...
!function (console, args, readFileFunc) {
    var fileName, env, fs, vm, path, exec, rhinoContext, dir, nodeRequire, nodeDefine, exists, reqMain,
        loadedOptimizedLib, existsForNode, Cc, Ci, version = "2.3.6", jsSuffixRegExp = /\.js$/, commandOption = "",
        useLibLoaded = {}, rhinoArgs = args, xpconnectArgs = args,
        readFile = void 0 !== readFileFunc ? readFileFunc : null;
    ...
    function loadLib() {
        ...
        if (!function () {
            ...
            return isJavaObj = function () {
                return !1
            }, "undefined" != typeof java && java.lang && java.lang.Object && "undefined" != typeof importPackage && (isJavaObj = function (obj) {
                return obj instanceof java.lang.Object
            }), lang = {
                backSlashRegExp: /\\/g, ostring: Object.prototype.toString, isArray: Array.isArray || function (it) {
                    return "[object Array]" === lang.ostring.call(it)
                }, isFunction: function (it) {
                    return "[object Function]" === lang.ostring.call(it)
                }, isRegExp: function (it) {
                    return it && it instanceof RegExp
                }, hasProp: hasProp, falseProp: function (obj, prop) {
                    return !hasProp(obj, prop) || !obj[prop]
                }, getOwn: function (obj, prop) {
                    return hasProp(obj, prop) && obj[prop]
                }, _mixin: function (dest, source, override) {
                    var name;
                    for (name in source) !source.hasOwnProperty(name) || !override && dest.hasOwnProperty(name) || (dest[name] = source[name]);
                    return dest
                }, mixin: function (dest) {
                    var override, i, l, parameters = Array.prototype.slice.call(arguments);
                    for (dest = dest || {}, 2 < parameters.length && "boolean" == typeof arguments[parameters.length - 1] && (override = parameters.pop()), i = 1, l = parameters.length; i < l; i++) lang._mixin(dest, parameters[i], override);
                    return dest
                }, deepMix: function (dest, source) {
                    return lang.eachProp(source, function (value, prop) {
                        "object" != typeof value || !value || lang.isArray(value) || lang.isFunction(value) || value instanceof RegExp ? dest[prop] = value : (dest[prop] || (dest[prop] = {}), lang.deepMix(dest[prop], value))
                    }), dest
                }, deeplikeCopy: function (obj, ignoredProps) {
                    var type, result;
                    return lang.isArray(obj) ? (result = [], obj.forEach(function (value) {
                        result.push(lang.deeplikeCopy(value, ignoredProps))
                    }), result) : (type = typeof obj, null == obj || "boolean" == type || "string" == type || "number" == type || lang.isFunction(obj) || lang.isRegExp(obj) || isJavaObj(obj) ? obj : (result = {}, lang.eachProp(obj, function (value, key) {
                        ignoredProps && hasProp(ignoredProps, key) || (result[key] = lang.deeplikeCopy(value, ignoredProps))
                    }), result))
                }, delegate: function (obj, props) {
                    TMP.prototype = obj;
                    var tmp = new TMP;
                    return TMP.prototype = null, props && lang.mixin(tmp, props), tmp
                }, each: function (ary, func) {
                    var i;
                    if (ary) for (i = 0; i < ary.length && !func(ary[i], i, ary); i += 1) ;
            ...
            function each(ary, func) {
                var i;
                if (ary) for (i = 0; i < ary.length; i += 1) ary[i] && func(ary[i], i, ary)
            }

            function check(p) {
                if (!hasProp(p, "e") && !hasProp(p, "v")) return 1;
                if (!prim.hideResolutionConflict) throw new Error("Prim promise already resolved: " + JSON.stringify(p))
            }
            ...
            (prim = function prim() {
                ...
                return p = {
                    callback: function (yes, no) {
                        no && p.errback(no), hasProp(p, "v") ? prim.nextTick(function () {
                            yes(p.v)
                        }) : ok.push(yes)
                    }, errback: function (no) {
                        hasProp(p, "e") ? prim.nextTick(function () {
                            no(p.e)
                        }) : fail.push(no)
                    }, finished: function () {
                        ...
                        return check(p) && (p.v = v, notify(ok, v)), p
                    }, reject: function (e) {
                        return check(p) && (p.e = e, notify(fail, e)), p
                    }, start: function (fn) {
                        return p.resolve(), p.promise.then(fn)
                    }, promise: {
                        then: function (yes, no) {
                            var next = prim();
                            return p.callback(function (v) {
                                try {
                                    yes && "function" == typeof yes && (v = yes(v)), v && v.then ? v.then(next.resolve, next.reject) : next.resolve(v)
                                } catch (e) {
                                    next.reject(e)
                                }
                            }, function (e) {
                                var err;
                                try {
                                    no && "function" == typeof no ? (err = no(e)) && err.then ? err.then(next.resolve, next.reject) : next.resolve(err) : next.reject(e)
                                } catch (e2) {
                                    next.reject(e2)
                                }
                            }), next.promise
                ...
                return each(ary, function (item) {
                    result = result.then(function () {
                        return item()
                    })
                }), result
            }, prim.nextTick = "function" == typeof setImmediate ? setImmediate : "undefined" != typeof process && process.nextTick ? process.nextTick : "undefined" != typeof setTimeout ? function (fn) {
                setTimeout(fn, 0)
            } : function (fn) {
                fn()
            }, "function" == typeof define && define.amd ? define("prim", function () {
                return prim
            }) : "undefined" != typeof module && module.exports && (module.exports = prim)
        }(), "browser" === env && define("browser/assert", function () {
            return {}
        }), "node" === env && define("node/assert", ["assert"], function (assert) {
            return assert
        }), "rhino" === env && define("rhino/assert", function () {
            return {}
        }), "xpconnect" === env && define("xpconnect/assert", function () {
            return {}
        }), "browser" === env && define("browser/args", function () {
            return []
        }), "node" === env && define("node/args", function () {
            var args = process.argv.slice(2);
            return args[0] && 0 === args[0].indexOf("-") && (args = args.slice(1)), args
        }), "rhino" === env) {
            var jsLibRhinoArgs = void 0 !== rhinoArgs && rhinoArgs || [].concat(Array.prototype.slice.call(arguments, 0));
        ...
        if ("xpconnect" === env) {
            var jsLibXpConnectArgs = void 0 !== xpconnectArgs && xpconnectArgs || [].concat(Array.prototype.slice.call(arguments, 0));
            define("xpconnect/args", function () {
                var args = jsLibXpConnectArgs;
                return args[0] && 0 === args[0].indexOf("-") && (args = args.slice(1)), args
            })
        }
        "browser" === env && define("browser/load", ["./file"], function (file) {
            function load(fileName) {
                eval(file.readFile(fileName))
            }

            return load
        }), "node" === env && define("node/load", ["fs"], function (fs) {
            return function (fileName) {
                var contents = fs.readFileSync(fileName, "utf8");
                process.compile(contents, fileName)
            }
        }), "rhino" === env && define("rhino/load", function () {
            return load
        }), "xpconnect" === env && define("xpconnect/load", function () {
            return load
        }), "browser" === env && define("browser/file", ["prim"], function (prim) {
            var file, currDirRegExp = /^\.(\/|$)/;
            return file = {
                backSlashRegExp: /\\/g, exclusionRegExp: /^\./, getLineSeparator: function () {
                    return "/"
                }, exists: function (fileName) {
                    return path = fileName, (xhr = new XMLHttpRequest).open("HEAD", path, !1), xhr.send(), 200 === (status = xhr.status) || 304 === status;
                    var path, status, xhr
                    ...
                    var dir;
                    return currDirRegExp.test(fileName) && (-1 !== (dir = location.href.replace(/\\/g, "/")).indexOf("/") && ((dir = dir.split("/")).splice(0, 3), dir.pop(), dir = "/" + dir.join("/")), fileName = dir + fileName.substring(1)), fileName
                    ...
                    return xhr.open("GET", path, !1), xhr.send(), xhr.responseText
                }, readFileAsync: function (path, encoding) {
                    var xhr = new XMLHttpRequest, d = prim();
                    return xhr.open("GET", path, !0), xhr.send(), xhr.onreadystatechange = function () {
                        4 === xhr.readyState && (400 < xhr.status ? d.reject(new Error("Status: " + xhr.status + ": " + xhr.statusText)) : d.resolve(xhr.responseText))
                    }, d.promise
        ...
        }), "node" === env && define("node/file", ["fs", "path", "prim"], function (fs, path, prim) {
            var file, isWindows = "win32" === process.platform, windowsDriveRegExp = /^[a-zA-Z]\:\/$/;
            ...
            function exists(path) {
                isWindows && "/" === path.charAt(path.length - 1) && ":" !== path.charAt(path.length - 2) && (path = path.substring(0, path.length - 1));
                try {
                    return fs.statSync(path), !0
                } catch (e) {
                    return !1
                }
            }
            ...
            function mkFullDir(dir) {
                var parts = dir.split("/"), currDir = "";
                parts.forEach(function (part) {
                    var dir;
                    currDir += part + "/", part && (exists(dir = currDir) || isWindows && windowsDriveRegExp.test(dir) || fs.mkdirSync(dir, 511))
                })
            }
            ...
            return file = {
                backSlashRegExp: /\\/g, exclusionRegExp: /^\./, getLineSeparator: function () {
                    ...
                    var topDir, regExpInclude, regExpExclude, dirFileArray, i, stat, filePath, ok, fileName, files = [];
                    if (topDir = startDir, regExpInclude = regExpFilters.include || regExpFilters, regExpExclude = regExpFilters.exclude || null, file.exists(topDir)) for (dirFileArray = fs.readdirSync(topDir), i = 0; i < dirFileArray.length; i++) fileName = dirFileArray[i], filePath = path.join(topDir, fileName), (stat = fs.statSync(filePath)).isFile() ? (makeUnixPaths && -1 === filePath.indexOf("/") && (filePath = frontSlash(filePath)), ok = !0, regExpInclude && (ok = filePath.match(regExpInclude)), ok && regExpExclude && (ok = !filePath.match(regExpExclude)), !ok || file.exclusionRegExp && file.exclusionRegExp.test(fileName) || files.push(filePath)) : !stat.isDirectory() || file.exclusionRegExp && file.exclusionRegExp.test(fileName) || this.getFilteredFileList(filePath, regExpFilters, makeUnixPaths).forEach(function (dirFile) {
                        files.push(dirFile)
                    });
                    return files
                }, copyDir: function (srcDir, destDir, regExpFilter, onlyCopyNew) {
                    regExpFilter = regExpFilter || /\w/, srcDir = frontSlash(path.normalize(srcDir)), destDir = frontSlash(path.normalize(destDir));
                    var i, srcFileName, destFileName, fileNames = file.getFilteredFileList(srcDir, regExpFilter, !0),
                        copiedFiles = [];
                    for (i = 0; i < fileNames.length; i++) destFileName = (srcFileName = fileNames[i]).replace(srcDir, destDir), file.copyFile(srcFileName, destFileName, onlyCopyNew) && copiedFiles.push(destFileName);
                    return copiedFiles.length ? copiedFiles : null
                }, copyFile: function (srcFileName, destFileName, onlyCopyNew) {
                    var parentDir;
                    return !(onlyCopyNew && file.exists(destFileName) && fs.statSync(destFileName).mtime.getTime() >= fs.statSync(srcFileName).mtime.getTime()) && (parentDir = path.dirname(destFileName), file.exists(parentDir) || mkFullDir(parentDir), fs.writeFileSync(destFileName, fs.readFileSync(srcFileName, "binary"), "binary"), !0)
                }, renameFile: function (from, to) {
                    return fs.renameSync(from, to)
                }, readFile: function (path, encoding) {
                    "utf-8" === encoding && (encoding = "utf8"), encoding = encoding || "utf8";
                    var text = fs.readFileSync(path, encoding);
                    return 0 === text.indexOf("\ufeff") && (text = text.substring(1, text.length)), text
                }, readFileAsync: function (path, encoding) {
                    var d = prim();
                    try {
                    ...
                    var parentDir;
                    "utf-8" === encoding && (encoding = "utf8"), encoding = encoding || "utf8", parentDir = path.dirname(fileName), file.exists(parentDir) || mkFullDir(parentDir), fs.writeFileSync(fileName, fileContents, encoding)
                }, deleteFile: function (fileName) {
                    var files, i;
                    if (file.exists(fileName)) if (fs.lstatSync(fileName).isDirectory()) {
                        for (files = fs.readdirSync(fileName), i = 0; i < files.length; i++) this.deleteFile(path.join(fileName, files[i]));
                        fs.rmdirSync(fileName)
                    } else fs.unlinkSync(fileName)
                }, deleteEmptyDirs: function (startDir) {
                    var dirFileArray, i, fileName, filePath;
                    if (file.exists(startDir)) {
                        for (dirFileArray = fs.readdirSync(startDir), i = 0; i < dirFileArray.length; i++) fileName = dirFileArray[i], filePath = path.join(startDir, fileName), fs.lstatSync(filePath).isDirectory() && file.deleteEmptyDirs(filePath);
                        0 === fs.readdirSync(startDir).length && file.deleteFile(startDir)
                    }
                }
            }
        }), "rhino" === env && define("rhino/file", ["prim"], function (prim) {
            var file = {
                backSlashRegExp: /\\/g, exclusionRegExp: /^\./, getLineSeparator: function () {
                    return file.lineSeparator
                }, lineSeparator: java.lang.System.getProperty("line.separator"), exists: function (fileName) {
                    ...
                    return "string" == typeof fileObj && (fileObj = new java.io.File(fileObj)), (fileObj.getCanonicalPath() + "").replace(file.backSlashRegExp, "/")
                }, getFilteredFileList: function (startDir, regExpFilters, makeUnixPaths, startDirIsJavaObject) {
                    var topDir, regExpInclude, regExpExclude, dirFileArray, i, fileObj, filePath, ok, files = [];
                    if (topDir = startDir, startDirIsJavaObject || (topDir = new java.io.File(startDir)), regExpInclude = regExpFilters.include || regExpFilters, regExpExclude = regExpFilters.exclude || null, topDir.exists()) for (dirFileArray = topDir.listFiles(), i = 0; i < dirFileArray.length; i++) (fileObj = dirFileArray[i]).isFile() ? (filePath = fileObj.getPath(), makeUnixPaths && -1 === (filePath = String(filePath)).indexOf("/") && (filePath = filePath.replace(/\\/g, "/")), ok = !0, regExpInclude && (ok = filePath.match(regExpInclude)), ok && regExpExclude && (ok = !filePath.match(regExpExclude)), !ok || file.exclusionRegExp && file.exclusionRegExp.test(fileObj.getName()) || files.push(filePath)) : !fileObj.isDirectory() || file.exclusionRegExp && file.exclusionRegExp.test(fileObj.getName()) || this.getFilteredFileList(fileObj, regExpFilters, makeUnixPaths, !0).forEach(function (dirFile) {
                        files.push(dirFile)
                    });
                    return files
                }, copyDir: function (srcDir, destDir, regExpFilter, onlyCopyNew) {
                    ...
                    var srcFile, parentDir, srcChannel, destChannel, destFile = new java.io.File(destFileName);
                    if (onlyCopyNew && (srcFile = new java.io.File(srcFileName), destFile.exists() && destFile.lastModified() >= srcFile.lastModified())) return !1;
                    if (!(parentDir = destFile.getParentFile()).exists() && !parentDir.mkdirs()) throw"Could not create directory: " + parentDir.getCanonicalPath();
                    return srcChannel = new java.io.FileInputStream(srcFileName).getChannel(), (destChannel = new java.io.FileOutputStream(destFileName).getChannel()).transferFrom(srcChannel, 0, srcChannel.size()), srcChannel.close(), destChannel.close(), !0
                }, renameFile: function (from, to) {
                    return new java.io.File(from).renameTo(new java.io.File(to))
                }, readFile: function (path, encoding) {
                    encoding = encoding || "utf-8";
                    var stringBuffer, line, fileObj = new java.io.File(path),
                        input = new java.io.BufferedReader(new java.io.InputStreamReader(new java.io.FileInputStream(fileObj), encoding));
                    try {
                        for (stringBuffer = new java.lang.StringBuffer, (line = input.readLine()) && line.length() && 65279 === line.charAt(0) && (line = line.substring(1)); null !== line;) stringBuffer.append(line), stringBuffer.append(file.lineSeparator), line = input.readLine();
                    ...
                    var outWriter, parentDir, os, outFile = new java.io.File(fileName);
                    if (!(parentDir = outFile.getAbsoluteFile().getParentFile()).exists() && !parentDir.mkdirs()) throw"Could not create directory: " + parentDir.getAbsolutePath();
                    outWriter = encoding ? new java.io.OutputStreamWriter(new java.io.FileOutputStream(outFile), encoding) : new java.io.OutputStreamWriter(new java.io.FileOutputStream(outFile)), os = new java.io.BufferedWriter(outWriter);
                    try {
                        "undefined" != typeof importPackage ? os.write(fileContents) : os.write(new java.lang.String(fileContents))
                    } finally {
                        os.close()
                    }
                }, deleteFile: function (fileName) {
                    var files, i, fileObj = new java.io.File(fileName);
                    if (fileObj.exists()) {
                        if (fileObj.isDirectory()) for (files = fileObj.listFiles(), i = 0; i < files.length; i++) this.deleteFile(files[i]);
                        fileObj.delete()
                    }
                }, deleteEmptyDirs: function (startDir, startDirIsJavaObject) {
                    var dirFileArray, i, fileObj, topDir = startDir;
                    if (startDirIsJavaObject || (topDir = new java.io.File(startDir)), topDir.exists()) {
                        for (dirFileArray = topDir.listFiles(), i = 0; i < dirFileArray.length; i++) (fileObj = dirFileArray[i]).isDirectory() && file.deleteEmptyDirs(fileObj, !0);
                        0 === topDir.listFiles().length && file.deleteFile(String(topDir.getPath()))
                    }
                }
            };
            return file
        }), "xpconnect" === env && define("xpconnect/file", ["prim"], function (prim) {
            var file, Cc = Components.classes, Ci = Components.interfaces, xpfile = xpcUtil.xpfile;
            return file = {
                backSlashRegExp: /\\/g,
                exclusionRegExp: /^\./,
                ...
                absPath: function (fileObj) {
                    return "string" == typeof fileObj && (fileObj = xpfile(fileObj)), fileObj.path
                },
                getFilteredFileList: function (startDir, regExpFilters, makeUnixPaths, startDirIsObject) {
                    var topDir, regExpInclude, regExpExclude, dirFileArray, fileObj, filePath, ok, files = [];
                    if (topDir = startDir, startDirIsObject || (topDir = xpfile(startDir)), regExpInclude = regExpFilters.include || regExpFilters, regExpExclude = regExpFilters.exclude || null, topDir.exists()) for (dirFileArray = topDir.directoryEntries; dirFileArray.hasMoreElements();) (fileObj = dirFileArray.getNext().QueryInterface(Ci.nsILocalFile)).isFile() ? (filePath = fileObj.path, makeUnixPaths && -1 === filePath.indexOf("/") && (filePath = filePath.replace(/\\/g, "/")), ok = !0, regExpInclude && (ok = filePath.match(regExpInclude)), ok && regExpExclude && (ok = !filePath.match(regExpExclude)), !ok || file.exclusionRegExp && file.exclusionRegExp.test(fileObj.leafName) || files.push(filePath)) : !fileObj.isDirectory() || file.exclusionRegExp && file.exclusionRegExp.test(fileObj.leafName) || this.getFilteredFileList(fileObj, regExpFilters, makeUnixPaths, !0).forEach(function (dirFile) {
                        files.push(dirFile)
                    });
                    return files
                },
                copyDir: function (srcDir, destDir, regExpFilter, onlyCopyNew) {
                    ...
                    var i, srcFileName, destFileName, fileNames = file.getFilteredFileList(srcDir, regExpFilter, !0),
                        copiedFiles = [];
                    for (i = 0; i < fileNames.length; i += 1) destFileName = (srcFileName = fileNames[i]).replace(srcDir, destDir), file.copyFile(srcFileName, destFileName, onlyCopyNew) && copiedFiles.push(destFileName);
                    return copiedFiles.length ? copiedFiles : null
                },
                copyFile: function (srcFileName, destFileName, onlyCopyNew) {
                    var destFile = xpfile(destFileName), srcFile = xpfile(srcFileName);
                    return !(onlyCopyNew && destFile.exists() && destFile.lastModifiedTime >= srcFile.lastModifiedTime) && (srcFile.copyTo(destFile.parent, destFile.leafName), !0)
                },
                renameFile: function (from, to) {
                ...
                saveFile: function (fileName, fileContents, encoding) {
                    var outStream, convertStream, dirObj, fileObj = xpfile(fileName);
                    (dirObj = fileObj.parent).exists() || dirObj.create(1, 511);
                    try {
                        (outStream = Cc["@mozilla.org/network/file-output-stream;1"].createInstance(Ci.nsIFileOutputStream)).init(fileObj, 42, 511, 0), (convertStream = Cc["@mozilla.org/intl/converter-output-stream;1"].createInstance(Ci.nsIConverterOutputStream)).init(outStream, encoding, 0, 0), convertStream.writeString(fileContents)
                    } catch (e) {
                        throw new Error((fileObj && fileObj.path || "") + ": " + e)
                    } finally {
                        convertStream && convertStream.close(), outStream && outStream.close()
                    }
                },
                deleteFile: function (fileName) {
                    var fileObj = xpfile(fileName);
                    fileObj.exists() && fileObj.remove(!0)
                },
                deleteEmptyDirs: function (startDir, startDirIsObject) {
                    var dirFileArray, fileObj, topDir = startDir;
                    if (startDirIsObject || (topDir = xpfile(startDir)), topDir.exists()) {
                        for (dirFileArray = topDir.directoryEntries; dirFileArray.hasMoreElements();) (fileObj = dirFileArray.getNext().QueryInterface(Ci.nsILocalFile)).isDirectory() && file.deleteEmptyDirs(fileObj, !0);
                        (dirFileArray = topDir.directoryEntries).hasMoreElements() || file.deleteFile(topDir.path)
                    }
                }
            }
        }), "browser" === env && define("browser/quit", function () {
            "use strict";
            return function (code) {
            }
        }), "node" === env && define("node/quit", function () {
            "use strict";
            return function (code) {
                function exit() {
                    0 === draining ? process.exit(code) : --draining
                }

                var draining = 0;
                process.stdout.bufferSize && (draining += 1, process.stdout.once("drain", exit)), process.stderr.bufferSize && (draining += 1, process.stderr.once("drain", exit)), exit()
            }
        }), "rhino" === env && define("rhino/quit", function () {
            "use strict";
            return function (code) {
                return quit(code)
            }
        }), "xpconnect" === env && define("xpconnect/quit", function () {
            "use strict";
            return function (code) {
                return quit(code)
            }
        }), "browser" === env && define("browser/print", function () {
            return function (msg) {
                console.log(msg)
            }
        }), "node" === env && define("node/print", function () {
            return function (msg) {
                console.log(msg)
            }
        }), "rhino" === env && define("rhino/print", function () {
            return print
        }), "xpconnect" === env && define("xpconnect/print", function () {
            return print
            ...
            return {
                ...
                trace: function (message) {
                    this.level <= this.TRACE && this._print(message)
                },
                info: function (message) {
                    this.level <= this.INFO && this._print(message)
                },
                warn: function (message) {
                    this.level <= this.WARN && this._print(message)
                },
                error: function (message) {
                    this.level <= this.ERROR && this._print(message)
                },
            ...
            return modules = [function (module, exports, __webpack_require__) {
                ...
                var comment_handler_1 = __webpack_require__(1), jsx_parser_1 = __webpack_require__(3),
                    parser_1 = __webpack_require__(8), tokenizer_1 = __webpack_require__(15);

                function parse(code, options, delegate) {
                    function proxyDelegate(node, metadata) {
                        delegate && delegate(node, metadata), commentHandler && commentHandler.visit(node, metadata)
                    }

                    var commentHandler = null, parserDelegate = "function" == typeof delegate ? proxyDelegate : null,
                        collectComment = !1;
                    if (options) {
                        ...
                        (collectComment || attachComment) && ((commentHandler = new comment_handler_1.CommentHandler).attach = attachComment, options.comment = !0, parserDelegate = proxyDelegate)
                    }
                    var parser, isModule = !1;
                    options && "string" == typeof options.sourceType && (isModule = "module" === options.sourceType), parser = options && "boolean" == typeof options.jsx && options.jsx ? new jsx_parser_1.JSXParser(code, options, parserDelegate) : new parser_1.Parser(code, options, parserDelegate);
                ...
                var syntax_1 = __webpack_require__(2), CommentHandler = function () {
                    ...
                    return CommentHandler.prototype.insertInnerComments = function (node, metadata) {
                        if (node.type === syntax_1.Syntax.BlockStatement && 0 === node.body.length) {
                            for (var innerComments = [], i = this.leading.length - 1; 0 <= i; --i) {
                        ...
                        if (0 < this.trailing.length) {
                            for (var i = this.trailing.length - 1; 0 <= i; --i) {
                        ...
                        for (var target, leadingComments = []; 0 < this.stack.length;) {
                            if (!((entry = this.stack[this.stack.length - 1]) && entry.start >= metadata.start.offset)) break;
                            target = entry.node, this.stack.pop()
                        }
                        if (target) {
                            for (var i = (target.leadingComments ? target.leadingComments.length : 0) - 1; 0 <= i; --i) {
                                var comment = target.leadingComments[i];
                                comment.range[1] <= metadata.start.offset && (leadingComments.unshift(comment), target.leadingComments.splice(i, 1))
                            }
                            return target.leadingComments && 0 === target.leadingComments.length && delete target.leadingComments, leadingComments
                        }
                        for (i = this.leading.length - 1; 0 <= i; --i) {
                            var entry;
                            (entry = this.leading[i]).start <= metadata.start.offset && (leadingComments.unshift(entry.comment), this.leading.splice(i, 1))
                        }
                        return leadingComments
                    }, CommentHandler.prototype.visitNode = function (node, metadata) {
                        if (!(node.type === syntax_1.Syntax.Program && 0 < node.body.length)) {
                            this.insertInnerComments(node, metadata);
                            var trailingComments = this.findTrailingComments(metadata),
                                leadingComments = this.findLeadingComments(metadata);
                            0 < leadingComments.length && (node.leadingComments = leadingComments), 0 < trailingComments.length && (node.trailingComments = trailingComments), this.stack.push({
                                node: node,
                                start: metadata.start.offset
                            })
                        }
                    }, CommentHandler.prototype.visitComment = function (node, metadata) {
                        var type = "L" === node.type[0] ? "Line" : "Block", comment = {type: type, value: node.value};
                        ...
                        "LineComment" === node.type || "BlockComment" === node.type ? this.visitComment(node, metadata) : this.attach && this.visitNode(node, metadata)
                    }, CommentHandler
                }();
                exports.CommentHandler = CommentHandler
            }, function (module, exports) {
                "use strict";
                Object.defineProperty(exports, "__esModule", {value: !0}), exports.Syntax = {
                ...
                "use strict";
                var extendStatics,
                    __extends = this && this.__extends || (extendStatics = Object.setPrototypeOf || {__proto__: []} instanceof Array && function (d, b) {
                        d.__proto__ = b
                    } || function (d, b) {
                ...
                Object.defineProperty(exports, "__esModule", {value: !0});
                var character_1 = __webpack_require__(4), JSXNode = __webpack_require__(5),
                    jsx_syntax_1 = __webpack_require__(6), Node = __webpack_require__(7),
                    parser_1 = __webpack_require__(8), token_1 = __webpack_require__(13),
                    xhtml_entities_1 = __webpack_require__(14);
                ...
                var JSXParser = function (_super) {
                    ...
                    return __extends(JSXParser, _super), JSXParser.prototype.parsePrimaryExpression = function () {
                        ...
                        for (var result = "&", valid = !0, terminated = !1, numeric = !1, hex = !1; !this.scanner.eof() && valid && !terminated;) {
                            ...
                            if (terminated = ";" === ch, result += ch, ++this.scanner.index, !terminated) switch (result.length) {
                                case 2:
                                    ...
                                    break;
                                case 3:
                                    numeric && (valid = (hex = "x" === ch) || character_1.Character.isDecimalDigit(ch.charCodeAt(0)), numeric = numeric && !hex);
                                    break;
                                default:
                        ...
                        if (valid && terminated && 2 < result.length) {
                            var str = result.substr(1, result.length - 2);
                            numeric && 1 < str.length ? result = String.fromCharCode(parseInt(str.substr(1), 10)) : hex && 2 < str.length ? result = String.fromCharCode(parseInt("0" + str.substr(1), 16)) : numeric || hex || !xhtml_entities_1.XHTMLEntities[str] || (result = xhtml_entities_1.XHTMLEntities[str])
                        }
                        return result
                        ...
                        if (46 === cp) {
                            var n1 = this.scanner.source.charCodeAt(this.scanner.index + 1),
                                n2 = this.scanner.source.charCodeAt(this.scanner.index + 2),
                                value = 46 === n1 && 46 === n2 ? "..." : ".";
                        ...
                        for (var start = this.scanner.index, text = ""; !this.scanner.eof();) {
                            var ch = this.scanner.source[this.scanner.index];
                            if ("{" === ch || "<" === ch) break;
                            ++this.scanner.index, text += ch, character_1.Character.isLineTerminator(ch.charCodeAt(0)) && (++this.scanner.lineNumber, "\r" === ch && "\n" === this.scanner.source[this.scanner.index] && ++this.scanner.index, this.scanner.lineStart = this.scanner.index)
                        }
                        ...
                        return 0 < text.length && this.config.tokens && this.tokens.push(this.convertToken(token)), token
                        ...
                        return 100 !== token.type && this.throwUnexpectedToken(token), this.finalize(node, new JSXNode.JSXIdentifier(token.value))
                        ...
                        var node = this.createJSXNode();
                        this.expectJSX("<");
                        var name = this.parseJSXElementName(), attributes = this.parseJSXAttributes(),
                            selfClosing = this.matchJSX("/");
                        ...
                        return this.expectJSX("{"), this.matchJSX("}") ? (expression = this.parseJSXEmptyExpression(), this.expectJSX("}")) : (this.finishJSX(), expression = this.parseAssignmentExpression(), this.reenterJSX()), this.finalize(node, new JSXNode.JSXExpressionContainer(expression))
                    }, JSXParser.prototype.parseJSXChildren = function () {
                        for (var children = []; !this.scanner.eof();) {
                            var node = this.createJSXChildNode(), token = this.nextJSXText();
                            if (token.start < token.end) {
                                var raw = this.getTokenRaw(token),
                                    child = this.finalize(node, new JSXNode.JSXText(token.value, raw));
                                children.push(child)
                            }
                        ...
                        for (var stack = []; !this.scanner.eof();) {
                            ...
                            if (element.type === jsx_syntax_1.JSXSyntax.JSXOpeningElement) {
                                ...
                                } else stack.push(el), el = {node: node, opening: opening, closing: null, children: []}
                            }
                            if (element.type === jsx_syntax_1.JSXSyntax.JSXClosingElement) {
                                el.closing = element;
                                var open_1 = getQualifiedElementName(el.opening.name);
                                if (open_1 !== getQualifiedElementName(el.closing.name) && this.tolerateError("Expected corresponding JSX closing tag for %0", open_1), !(0 < stack.length)) break;
                                child = this.finalize(el.node, new JSXNode.JSXElement(el.opening, el.children, el.closing));
                                (el = stack[stack.length - 1]).children.push(child), stack.pop()
                            }
                        }
                        return el
                    }, JSXParser.prototype.parseJSXElement = function () {
                        var node = this.createJSXNode(), opening = this.parseJSXOpeningElement(), children = [],
                            closing = null;
                        if (!opening.selfClosing) {
                ...
                exports.Character = {
                    fromCodePoint: function (cp) {
                        ...
                        return 32 === cp || 9 === cp || 11 === cp || 12 === cp || 160 === cp || 5760 <= cp && 0 <= [5760, 8192, 8193, 8194, 8195, 8196, 8197, 8198, 8199, 8200, 8201, 8202, 8239, 8287, 12288, 65279].indexOf(cp)
                    }, isLineTerminator: function (cp) {
                        return 10 === cp || 13 === cp || 8232 === cp || 8233 === cp
                    }, isIdentifierStart: function (cp) {
                        return 36 === cp || 95 === cp || 65 <= cp && cp <= 90 || 97 <= cp && cp <= 122 || 92 === cp || 128 <= cp && Regex.NonAsciiIdentifierStart.test(exports.Character.fromCodePoint(cp))
                    }, isIdentifierPart: function (cp) {
                        return 36 === cp || 95 === cp || 65 <= cp && cp <= 90 || 97 <= cp && cp <= 122 || 48 <= cp && cp <= 57 || 92 === cp || 128 <= cp && Regex.NonAsciiIdentifierPart.test(exports.Character.fromCodePoint(cp))
                ...
                "use strict";
                Object.defineProperty(exports, "__esModule", {value: !0}), exports.JSXSyntax = {
                ...
                "use strict";
                Object.defineProperty(exports, "__esModule", {value: !0});
                var assert_1 = __webpack_require__(9), error_handler_1 = __webpack_require__(10),
                    messages_1 = __webpack_require__(11), Node = __webpack_require__(7),
                    scanner_1 = __webpack_require__(12), syntax_1 = __webpack_require__(2),
                    token_1 = __webpack_require__(13), Parser = function () {
                        function Parser(code, options, delegate) {
                            ...
                            }, this.config.loc && options.source && null !== options.source && (this.config.source = String(options.source)), this.delegate = delegate, this.errorHandler = new error_handler_1.ErrorHandler, this.errorHandler.tolerant = this.config.tolerant, this.scanner = new scanner_1.Scanner(code, this.errorHandler), this.scanner.trackComment = this.config.comment, this.operatorPrecedence = {
                                ")": 0,
                                ";": 0,
                                ",": 0,
                                "=": 0,
                        ...
                        return Parser.prototype.throwError = function (messageFormat) {
                            ...
                            var args = Array.prototype.slice.call(arguments, 1),
                                ...
                                }), index = this.lastMarker.index, line = this.lastMarker.line,
                                column = this.lastMarker.column + 1;
                            ...
                            if (this.config.comment) {
                                ...
                                if (0 < comments.length && this.delegate) for (var i = 0; i < comments.length; ++i) {
                                    ...
                                    var metadata = {
                                        start: {
                                            ...
                                            offset: e.range[0]
                                        }, end: {line: e.loc.end.line, column: e.loc.end.column, offset: e.range[1]}
                                    };
                            ...
                            return this.hasLineTerminator = token.lineNumber !== next.lineNumber, next && this.context.strict && 3 === next.type && this.scanner.isStrictModeReservedWord(next.value) && (next.type = 4), this.lookahead = next, this.config.tokens && 2 !== next.type && this.tokens.push(this.convertToken(next)), token
                            ...
                            if (this.config.range && (node.range = [marker.index, this.lastMarker.index]), this.config.loc && (node.loc = {
                                ...
                                var metadata = {
                                    start: {line: marker.line, column: marker.column, offset: marker.index},
                                    end: {
                            ...
                            if (this.config.tolerant) {
                                var token = this.lookahead;
                                7 === token.type && "," === token.value ? this.nextToken() : 7 === token.type && ";" === token.value ? (this.nextToken(), this.tolerateUnexpectedToken(token)) : this.tolerateUnexpectedToken(token, messages_1.Messages.UnexpectedToken)
                            } else this.expect(",")
                            ...
                            var previousIsBindingElement = this.context.isBindingElement,
                                previousIsAssignmentTarget = this.context.isAssignmentTarget,
                            ...
                            return null !== this.context.firstCoverInitializedNameError && this.throwUnexpectedToken(this.context.firstCoverInitializedNameError), this.context.isBindingElement = previousIsBindingElement, this.context.isAssignmentTarget = previousIsAssignmentTarget, this.context.firstCoverInitializedNameError = previousFirstCoverInitializedNameError, result
                            ...
                            switch (this.lookahead.type) {
                                case 3:
                                    (this.context.isModule || this.context.await) && "await" === this.lookahead.value && this.tolerateUnexpectedToken(this.lookahead), expr = this.matchAsyncFunction() ? this.parseFunctionExpression() : this.finalize(node, new Node.Identifier(this.nextToken().value));
                                    break;
                                case 6:
                                case 8:
                                    ...
                                    break;
                                case 1:
                                    this.context.isAssignmentTarget = !1, this.context.isBindingElement = !1, token = this.nextToken(), raw = this.getTokenRaw(token), expr = this.finalize(node, new Node.Literal("true" === token.value, raw));
                                    break;
                                case 5:
                                ...
                                case 7:
                                    ...
                                    break;
                                case 4:
                                    expr = !this.context.strict && this.context.allowYield && this.matchKeyword("yield") ? this.parseIdentifierName() : !this.context.strict && this.matchKeyword("let") ? this.finalize(node, new Node.Identifier(this.nextToken().value)) : (this.context.isAssignmentTarget = !1, this.context.isBindingElement = !1, this.matchKeyword("function") ? this.parseFunctionExpression() : this.matchKeyword("this") ? (this.nextToken(), this.finalize(node, new Node.ThisExpression)) : this.matchKeyword("class") ? this.parseClassExpression() : this.throwUnexpectedToken(this.nextToken()));
                                    break;
                                default:
                                    expr = this.throwUnexpectedToken(this.nextToken())
                            }
                            ...
                            for (this.expect("["); !this.match("]");) if (this.match(",")) this.nextToken(), elements.push(null); else if (this.match("...")) {
                            ...
                            var previousStrict = this.context.strict,
                                previousAllowStrictDirective = this.context.allowStrictDirective;
                            ...
                            return this.context.allowYield = previousAllowYield, this.finalize(node, new Node.FunctionExpression(null, params.params, method, !1))
                        }, Parser.prototype.parsePropertyMethodAsyncFunction = function () {
                            var node = this.createNode(), previousAllowYield = this.context.allowYield,
                                previousAwait = this.context.await;
                            ...
                            var kind, node = this.createNode(), token = this.lookahead, key = null, value = null,
                                computed = !1, method = !1, shorthand = !1, isAsync = !1;
                            if (3 === token.type) {
                                var id = token.value;
                                this.nextToken(), computed = this.match("["), key = (isAsync = !(this.hasLineTerminator || "async" !== id || this.match(":") || this.match("(") || this.match("*") || this.match(","))) ? this.parseObjectPropertyKey() : this.finalize(node, new Node.Identifier(id))
                            } else this.match("*") ? this.nextToken() : (computed = this.match("["), key = this.parseObjectPropertyKey());
                            var lookaheadPropertyKey = this.qualifiedPropertyName(this.lookahead);
                            if (3 === token.type && !isAsync && "get" === token.value && lookaheadPropertyKey) kind = "get", computed = this.match("["), key = this.parseObjectPropertyKey(), this.context.allowYield = !1, value = this.parseGetterMethod(); else if (3 === token.type && !isAsync && "set" === token.value && lookaheadPropertyKey) kind = "set", computed = this.match("["), key = this.parseObjectPropertyKey(), value = this.parseSetterMethod(); else if (7 === token.type && "*" === token.value && lookaheadPropertyKey) kind = "init", computed = this.match("["), key = this.parseObjectPropertyKey(), value = this.parseGeneratorMethod(), method = !0; else if (key || this.throwUnexpectedToken(this.lookahead), kind = "init", this.match(":") && !isAsync) !computed && this.isPropertyKey(key, "__proto__") && (hasProto.value && this.tolerateError(messages_1.Messages.DuplicateProtoProperty), hasProto.value = !0), this.nextToken(), value = this.inheritCoverGrammar(this.parseAssignmentExpression); else if (this.match("(")) value = isAsync ? this.parsePropertyMethodAsyncFunction() : this.parsePropertyMethodFunction(), method = !0; else if (3 === token.type) {
                            ...
                            var node = this.createNode(), token = this.nextToken(), raw = token.value,
                                cooked = token.cooked;
                            ...
                            if (this.expect("("), this.match(")")) this.nextToken(), this.match("=>") || this.expect("=>"), expr = {
                                ...
                                if (this.match("...")) expr = this.parseRestElement(params), this.expect(")"), this.match("=>") || this.expect("=>"), expr = {
                                    ...
                                    if (!arrow) {
                                        if (this.expect(")"), this.match("=>") && (expr.type === syntax_1.Syntax.Identifier && "yield" === expr.name && (expr = {
                                            type: "ArrowParameterPlaceHolder",
                                            params: [expr],
                                            async: !(arrow = !0)
                                        }), !arrow)) {
                                            if (this.context.isBindingElement || this.throwUnexpectedToken(this.lookahead), expr.type === syntax_1.Syntax.SequenceExpression) for (i = 0; i < expr.expressions.length; i++) this.reinterpretExpressionAsPattern(expr.expressions[i]); else this.reinterpretExpressionAsPattern(expr);
                                            expr = {
                                                type: "ArrowParameterPlaceHolder",
                            ...
                            if (assert_1.assert("new" === id.name, "New expression must start with `new`"), this.match(".")) if (this.nextToken(), 3 === this.lookahead.type && this.context.inFunctionBody && "target" === this.lookahead.value) {
                                var property = this.parseIdentifierName();
                                expr = new Node.MetaProperty(id, property)
                            } else this.throwUnexpectedToken(this.lookahead); else {
                                var callee = this.isolateCoverGrammar(this.parseLeftHandSideExpression),
                                    args = this.match("(") ? this.parseArguments() : [];
                            ...
                            for (this.context.allowIn = !0, this.matchKeyword("super") && this.context.inFunctionBody ? (expr = this.createNode(), this.nextToken(), expr = this.finalize(expr, new Node.Super), this.match("(") || this.match(".") || this.match("[") || this.throwUnexpectedToken(this.lookahead)) : expr = this.inheritCoverGrammar(this.matchKeyword("new") ? this.parseNewExpression : this.parsePrimaryExpression); ;) if (this.match(".")) {
                                ...
                                if (expr = this.finalize(this.startNode(startToken), new Node.CallExpression(expr, args)), asyncArrow && this.match("=>")) {
                                    ...
                                    expr = {type: "ArrowParameterPlaceHolder", params: args, async: !0}
                                }
                            } else if (this.match("[")) {
                                this.context.isBindingElement = !1, this.context.isAssignmentTarget = !0, this.expect("[");
                            ...
                            } else if (expr = this.inheritCoverGrammar(this.parseLeftHandSideExpressionAllowCall), !this.hasLineTerminator && 7 === this.lookahead.type && (this.match("++") || this.match("--"))) {
                            ...
                            if (this.match("+") || this.match("-") || this.match("~") || this.match("!") || this.matchKeyword("delete") || this.matchKeyword("void") || this.matchKeyword("typeof")) {
                                ...
                                expr = this.inheritCoverGrammar(this.parseUnaryExpression), expr = this.finalize(node, new Node.UnaryExpression(token.value, expr)), this.context.strict && "delete" === expr.operator && expr.argument.type === syntax_1.Syntax.Identifier && this.tolerateError(messages_1.Messages.StrictDelete), this.context.isAssignmentTarget = !1, this.context.isBindingElement = !1
                            ...
                            var startToken = this.lookahead,
                                expr = this.inheritCoverGrammar(this.parseExponentiationExpression), token = this.lookahead,
                            ...
                            if (0 < prec) {
                                ...
                                for (var markers = [startToken, this.lookahead], left = expr, right = this.isolateCoverGrammar(this.parseExponentiationExpression), stack = [left, token.value, right], precedences = [prec]; !((prec = this.binaryPrecedence(this.lookahead)) <= 0);) {
                                    for (; 2 < stack.length && prec <= precedences[precedences.length - 1];) {
                                        right = stack.pop();
                                ...
                                expr = stack[i];
                                for (var lastMarker = markers.pop(); 1 < i;) {
                                    var marker = markers.pop(), lastLineStart = lastMarker && lastMarker.lineStart;
                                    node = this.startNode(marker, lastLineStart), operator = stack[i - 1];
                            ...
                            options = {simple: !0, paramSet: {}};
                            for (var i = 0; i < params.length; ++i) {
                                (param = params[i]).type === syntax_1.Syntax.AssignmentPattern ? param.right.type === syntax_1.Syntax.YieldExpression && (param.right.argument && this.throwUnexpectedToken(this.lookahead), param.right.type = syntax_1.Syntax.Identifier, param.right.name = "yield", delete param.right.argument, delete param.right.delegate) : asyncArrow && param.type === syntax_1.Syntax.Identifier && "await" === param.name && this.throwUnexpectedToken(this.lookahead), this.checkPatternParam(options, param), params[i] = param
                            }
                            ...
                            if (!this.context.allowYield && this.matchKeyword("yield")) expr = this.parseYieldExpression(); else {
                                ...
                                if (expr = this.parseConditionalExpression(), 3 === token.type && token.lineNumber === this.lookahead.lineNumber && "async" === token.value && (3 === this.lookahead.type || this.matchKeyword("yield"))) {
                                ...
                                if ("ArrowParameterPlaceHolder" === expr.type || this.match("=>")) {
                                    ...
                                    this.match("=") ? this.reinterpretExpressionAsPattern(expr) : (this.context.isAssignmentTarget = !1, this.context.isBindingElement = !1);
                                    var operator = (token = this.nextToken()).value,
                                        right = this.isolateCoverGrammar(this.parseAssignmentExpression);
                            ...
                            var startToken = this.lookahead,
                                expr = this.isolateCoverGrammar(this.parseAssignmentExpression);
                            if (this.match(",")) {
                                var expressions = [];
                                for (expressions.push(expr); 2 !== this.lookahead.type && this.match(",");) this.nextToken(), expressions.push(this.isolateCoverGrammar(this.parseAssignmentExpression));
                            ...
                            var node = this.createNode();
                            this.expect("[");
                            for (var elements = []; !this.match("]");) if (this.match(",")) this.nextToken(), elements.push(null); else {
                                if (this.match("...")) {
                            ...
                            if (3 === this.lookahead.type) {
                                ...
                                } else value = this.match(":") ? (this.expect(":"), this.parsePatternWithDefault(params, kind)) : (params.push(keyToken), shorthand = !0, init)
                            ...
                            return this.match("[") ? this.parseArrayPattern(params, kind) : this.match("{") ? this.parseObjectPattern(params, kind) : (!this.matchKeyword("let") || "const" !== kind && "let" !== kind || this.tolerateUnexpectedToken(this.lookahead, messages_1.Messages.LetInLexicalBinding), params.push(this.lookahead), this.parseVariableIdentifier(kind))
                            ...
                            for (list.push(this.parseVariableDeclaration(opt)); this.match(",");) this.nextToken(), list.push(this.parseVariableDeclaration(opt));
                            return list
                            ...
                            var test = this.parseExpression();
                            if (!this.match(")") && this.config.tolerant) this.tolerateUnexpectedToken(this.nextToken()), body = this.finalize(this.createNode(), new Node.EmptyStatement); else {
                                this.expect(")");
                                var previousInIteration = this.context.inIteration;
                            ...
                            var left, right, body, init = null, test = null, update = null, forIn = !0,
                            ...
                            if (this.expectKeyword("for"), this.expect("("), this.match(";")) this.nextToken(); else if (this.matchKeyword("var")) {
                                ...
                                } else 1 === declarations.length && null === declarations[0].init && this.matchContextualKeyword("of") ? (init = this.finalize(init, new Node.VariableDeclaration(declarations, "var")), this.nextToken(), left = init, right = this.parseAssignmentExpression(), init = null, forIn = !1) : (init = this.finalize(init, new Node.VariableDeclaration(declarations, "var")), this.expect(";"))
                                ...
                                if (this.context.strict || "in" !== this.lookahead.value) {
                                    ...
                                    this.context.allowIn = previousAllowIn, 1 === declarations.length && null === declarations[0].init && this.matchKeyword("in") ? (init = this.finalize(init, new Node.VariableDeclaration(declarations, kind)), this.nextToken(), left = init, right = this.parseExpression(), init = null) : 1 === declarations.length && null === declarations[0].init && this.matchContextualKeyword("of") ? (init = this.finalize(init, new Node.VariableDeclaration(declarations, kind)), this.nextToken(), left = init, right = this.parseAssignmentExpression(), init = null, forIn = !1) : (this.consumeSemicolon(), init = this.finalize(init, new Node.VariableDeclaration(declarations, kind)))
                                } else init = this.finalize(init, new Node.Identifier(kind)), this.nextToken(), left = init, right = this.parseExpression(), init = null
                            } else {
                                var initStartToken = this.lookahead;
                                previousAllowIn = this.context.allowIn;
                                if (this.context.allowIn = !1, init = this.inheritCoverGrammar(this.parseAssignmentExpression), this.context.allowIn = previousAllowIn, this.matchKeyword("in")) this.context.isAssignmentTarget && init.type !== syntax_1.Syntax.AssignmentExpression || this.tolerateError(messages_1.Messages.InvalidLHSInForIn), this.nextToken(), this.reinterpretExpressionAsPattern(init), left = init, right = this.parseExpression(), init = null; else if (this.matchContextualKeyword("of")) this.context.isAssignmentTarget && init.type !== syntax_1.Syntax.AssignmentExpression || this.tolerateError(messages_1.Messages.InvalidLHSInForLoop), this.nextToken(), this.reinterpretExpressionAsPattern(init), left = init, right = this.parseAssignmentExpression(), init = null, forIn = !1; else {
                                    if (this.match(",")) {
                            ...
                            if (void 0 === left && (this.match(";") || (test = this.parseExpression()), this.expect(";"), this.match(")") || (update = this.parseExpression())), !this.match(")") && this.config.tolerant) this.tolerateUnexpectedToken(this.nextToken()), body = this.finalize(this.createNode(), new Node.EmptyStatement); else {
                                this.expect(")");
                                var previousInIteration = this.context.inIteration;
                            ...
                            if (3 === this.lookahead.type && !this.hasLineTerminator) {
                                var id = this.parseVariableIdentifier(), key = "$" + (label = id).name;
                                Object.prototype.hasOwnProperty.call(this.context.labelSet, key) || this.throwError(messages_1.Messages.UnknownLabel, id.name)
                            }
                            return this.consumeSemicolon(), null !== label || this.context.inIteration || this.throwError(messages_1.Messages.IllegalContinue), this.finalize(node, new Node.ContinueStatement(label))
                            ...
                            return this.consumeSemicolon(), null !== label || this.context.inIteration || this.context.inSwitch || this.throwError(messages_1.Messages.IllegalBreak), this.finalize(node, new Node.BreakStatement(label))
                            ...
                            return body = !this.match(")") && this.config.tolerant ? (this.tolerateUnexpectedToken(this.nextToken()), this.finalize(this.createNode(), new Node.EmptyStatement)) : (this.expect(")"), this.parseStatement()), this.finalize(node, new Node.WithStatement(object, body))
                        }, Parser.prototype.parseSwitchCase = function () {
                            var test, node = this.createNode();
                            test = this.matchKeyword("default") ? (this.nextToken(), null) : (this.expectKeyword("case"), this.parseExpression()), this.expect(":");
                            ...
                            this.expect(")");
                            var previousInSwitch = this.context.inSwitch, cases = [],
                                defaultFound = !(this.context.inSwitch = !0);
                            for (this.expect("{"); !this.match("}");) {
                                var clause = this.parseSwitchCase();
                                null === clause.test && (defaultFound && this.throwError(messages_1.Messages.MultipleDefaultsInSwitch), defaultFound = !0), cases.push(clause)
                            }
                            ...
                            if (expr.type === syntax_1.Syntax.Identifier && this.match(":")) {
                                ...
                                var body = void 0;
                                if (this.matchKeyword("class")) this.tolerateUnexpectedToken(this.lookahead), body = this.parseClassDeclaration(); else if (this.matchKeyword("function")) {
                            ...
                            var block = this.parseBlock(),
                                handler = this.matchKeyword("catch") ? this.parseCatchClause() : null,
                                finalizer = this.matchKeyword("finally") ? this.parseFinallyClause() : null;
                            ...
                            switch (this.lookahead.type) {
                                ...
                                case 7:
                                    ...
                                    statement = "{" === value ? this.parseBlock() : "(" !== value && ";" === value ? this.parseEmptyStatement() : this.parseExpressionStatement();
                                    break;
                                case 3:
                            ...
                            var node = this.createNode();
                            this.expect("{");
                            var body = this.parseDirectivePrologues(), previousLabelSet = this.context.labelSet,
                                previousInIteration = this.context.inIteration, previousInSwitch = this.context.inSwitch,
                                previousInFunctionBody = this.context.inFunctionBody;
                            for (this.context.labelSet = {}, this.context.inIteration = !1, this.context.inSwitch = !1, this.context.inFunctionBody = !0; 2 !== this.lookahead.type && !this.match("}");) body.push(this.parseStatementListItem());
                            ...
                            }, this.expect("("), !this.match(")")) for (options.paramSet = {}; 2 !== this.lookahead.type && (this.parseFormalParameter(options), !this.match(")")) && (this.expect(","), !this.match(")"));) ;
                            return this.expect(")"), {
                                simple: options.simple,
                            ...
                            var formalParameters = this.parseFormalParameters(firstRestricted),
                                params = formalParameters.params, stricted = formalParameters.stricted;
                            ...
                            var message, isGenerator = !isAsync && this.match("*");
                            isGenerator && this.nextToken();
                            var firstRestricted, id = null, previousAllowAwait = this.context.await,
                            ...
                            var token = this.lookahead, node = this.createNode(), expr = this.parseExpression(),
                                directive = expr.type === syntax_1.Syntax.Literal ? this.getTokenRaw(token).slice(1, -1) : null;
                            ...
                            for (var firstRestricted = null, body = []; ;) {
                                var token = this.lookahead;
                                if (8 !== token.type) break;
                                var statement = this.parseDirective();
                                body.push(statement);
                                var directive = statement.directive;
                                if ("string" != typeof directive) break;
                                "use strict" === directive ? (this.context.strict = !0, firstRestricted && this.tolerateUnexpectedToken(firstRestricted, messages_1.Messages.StrictOctalLiteral), this.context.allowStrictDirective || this.tolerateUnexpectedToken(token, messages_1.Messages.IllegalLanguageModeDirective)) : !firstRestricted && token.octal && (firstRestricted = token)
                            }
                            return body
                            ...
                            0 < formalParameters.params.length && this.tolerateError(messages_1.Messages.BadGetterArity);
                            var method = this.parsePropertyMethod(formalParameters);
                            return this.context.allowYield = previousAllowYield, this.finalize(node, new Node.FunctionExpression(null, formalParameters.params, method, !1))
                            ...
                            return this.context.allowYield = previousAllowYield, this.finalize(node, new Node.FunctionExpression(null, params.params, method, !0))
                            ...
                            var id = identifierIsOptional && 3 !== this.lookahead.type ? null : this.parseVariableIdentifier(),
                                superClass = null;
                            ...
                            return 3 === this.lookahead.type ? (local = imported = this.parseVariableIdentifier(), this.matchContextualKeyword("as") && (this.nextToken(), local = this.parseVariableIdentifier())) : (local = imported = this.parseIdentifierName(), this.matchContextualKeyword("as") ? (this.nextToken(), local = this.parseVariableIdentifier()) : this.throwUnexpectedToken(this.nextToken())), this.finalize(node, new Node.ImportSpecifier(local, imported))
                            ...
                            if (this.expectKeyword("export"), this.matchKeyword("default")) if (this.nextToken(), this.matchKeyword("function")) {
                                ...
                                for (this.expect("{"); !this.match("}");) isExportFromIdentifier = isExportFromIdentifier || this.matchKeyword("default"), specifiers.push(this.parseExportSpecifier()), this.match("}") || this.expect(",");
                                if (this.expect("}"), this.matchContextualKeyword("from")) this.nextToken(), source = this.parseModuleSpecifier(), this.consumeSemicolon(); else if (isExportFromIdentifier) {
                ...
                "use strict";
                Object.defineProperty(exports, "__esModule", {value: !0}), exports.assert = function (condition, message) {
                ...
                "use strict";
                Object.defineProperty(exports, "__esModule", {value: !0}), exports.Messages = {
                ...
                "use strict";
                Object.defineProperty(exports, "__esModule", {value: !0});
                var assert_1 = __webpack_require__(9), character_1 = __webpack_require__(4),
                    messages_1 = __webpack_require__(11);
                ...
                var Scanner = function () {
                    function Scanner(code, handler) {
                        this.source = code, this.errorHandler = handler, this.trackComment = !1, this.isModule = !1, this.length = code.length, this.index = 0, this.lineNumber = 0 < code.length ? 1 : 0, this.lineStart = 0, this.curlyStack = []
                    }
                    ...
                    return Scanner.prototype.saveState = function () {
                        ...
                        return void 0 === message && (message = messages_1.Messages.UnexpectedTokenIllegal), this.errorHandler.throwError(this.index, this.lineNumber, this.index - this.lineStart + 1, message)
                    }, Scanner.prototype.tolerateUnexpectedToken = function (message) {
                        void 0 === message && (message = messages_1.Messages.UnexpectedTokenIllegal), this.errorHandler.tolerateError(this.index, this.lineNumber, this.index - this.lineStart + 1, message)
                        ...
                        for (this.trackComment && (comments = [], start = this.index - offset, loc = {
                            ...
                            if (++this.index, character_1.Character.isLineTerminator(ch)) {
                                ...
                                return 13 === ch && 10 === this.source.charCodeAt(this.index) && ++this.index, ++this.lineNumber, this.lineStart = this.index, comments
                            }
                        }
                        if (this.trackComment) {
                            entry = {
                                ...
                                }), slice: [start + offset, this.index], range: [start, this.index], loc: loc
                            };
                            comments.push(entry)
                        }
                        return comments
                        ...
                        for (this.trackComment && (comments = [], start = this.index - 2, loc = {
                            ...
                            var ch = this.source.charCodeAt(this.index);
                            if (character_1.Character.isLineTerminator(ch)) 13 === ch && 10 === this.source.charCodeAt(this.index + 1) && ++this.index, ++this.lineNumber, ++this.index, this.lineStart = this.index; else if (42 === ch) {
                        ...
                        for (var start = 0 === this.index; !this.eof();) {
                            var ch = this.source.charCodeAt(this.index);
                            if (character_1.Character.isWhiteSpace(ch)) ++this.index; else if (character_1.Character.isLineTerminator(ch)) ++this.index, 13 === ch && 10 === this.source.charCodeAt(this.index) && ++this.index, ++this.lineNumber, this.lineStart = this.index, start = !0; else if (47 === ch) if (47 === (ch = this.source.charCodeAt(this.index + 1))) {
                                this.index += 2;
                        ...
                        if (55296 <= cp && cp <= 56319) {
                            var second = this.source.charCodeAt(i + 1);
                            if (56320 <= second && second <= 57343) cp = 1024 * (cp - 55296) + second - 56320 + 65536
                        }
                        return cp
                        ...
                        for ("}" === ch && this.throwUnexpectedToken(); !this.eof() && (ch = this.source[this.index++], character_1.Character.isHexDigit(ch.charCodeAt(0)));) code = 16 * code + hexValue(ch);
                        return (1114111 < code || "}" !== ch) && this.throwUnexpectedToken(), character_1.Character.fromCodePoint(code)
                    }, Scanner.prototype.getIdentifier = function () {
                        for (var start = this.index++; !this.eof();) {
                            var ch = this.source.charCodeAt(this.index);
                            if (92 === ch) return this.index = start, this.getComplexIdentifier();
                            if (55296 <= ch && ch < 57343) return this.index = start, this.getComplexIdentifier();
                        ...
                        return !this.eof() && character_1.Character.isOctalDigit(this.source.charCodeAt(this.index)) && (octal = !0, code = 8 * code + octalValue(this.source[this.index++]), 0 <= "0123".indexOf(ch) && !this.eof() && character_1.Character.isOctalDigit(this.source.charCodeAt(this.index)) && (code = 8 * code + octalValue(this.source[this.index++]))), {
                            code: code,
                            octal: octal
                        }
                    }, Scanner.prototype.scanIdentifier = function () {
                        var type, start = this.index,
                            id = 92 === this.source.charCodeAt(start) ? this.getComplexIdentifier() : this.getIdentifier();
                        ...
                        switch (str) {
                            ...
                            case"~":
                                ...
                                break;
                            default:
                                ">>>=" === (str = this.source.substr(this.index, 4)) ? this.index += 4 : "===" === (str = str.substr(0, 3)) || "!==" === str || ">>>" === str || "<<=" === str || ">>=" === str || "**=" === str ? this.index += 3 : "&&" === (str = str.substr(0, 2)) || "||" === str || "==" === str || "!=" === str || "+=" === str || "-=" === str || "*=" === str || "/=" === str || "++" === str || "--" === str || "<<" === str || ">>" === str || "&=" === str || "|=" === str || "^=" === str || "%=" === str || "<=" === str || ">=" === str || "=>" === str || "**" === str ? this.index += 2 : (str = this.source[this.index], 0 <= "<>=!+-*%&|^/".indexOf(str) && ++this.index)
                        }
                        ...
                        return 0 === num.length && this.throwUnexpectedToken(), character_1.Character.isIdentifierStart(this.source.charCodeAt(this.index)) && this.throwUnexpectedToken(), {
                            type: 6,
                        ...
                        return 0 === num.length && this.throwUnexpectedToken(), this.eof() || (ch = this.source.charCodeAt(this.index), (character_1.Character.isIdentifierStart(ch) || character_1.Character.isDecimalDigit(ch)) && this.throwUnexpectedToken()), {
                            type: 6,
                        ...
                        return octal || 0 !== num.length || this.throwUnexpectedToken(), (character_1.Character.isIdentifierStart(this.source.charCodeAt(this.index)) || character_1.Character.isDecimalDigit(this.source.charCodeAt(this.index))) && this.throwUnexpectedToken(), {
                            type: 6,
                        ...
                        var num = "";
                        if ("." !== ch) {
                            if (num = this.source[this.index++], ch = this.source[this.index], "0" === num) {
                        ...
                        for (var octal = !1, str = ""; !this.eof();) {
                            ...
                            if ("\\" === ch) if ((ch = this.source[this.index++]) && character_1.Character.isLineTerminator(ch.charCodeAt(0))) ++this.lineNumber, "\r" === ch && "\n" === this.source[this.index] && ++this.index, this.lineStart = this.index; else switch (ch) {
                                case"u":
                                    if ("{" === this.source[this.index]) ++this.index, str += this.scanUnicodeCodePointEscape(); else {
                                        var unescaped_1 = this.scanHexEscape(ch);
                                        null === unescaped_1 && this.throwUnexpectedToken(), str += unescaped_1
                                    }
                                    break;
                                case"x":
                                    var unescaped = this.scanHexEscape(ch);
                                    null === unescaped && this.throwUnexpectedToken(messages_1.Messages.InvalidHexEscapeSequence), str += unescaped;
                                    break;
                                case"n":
                                    str += "\n";
                                    break;
                        ...
                        var cooked = "", terminated = !1, start = this.index, head = "`" === this.source[start],
                            tail = !1, rawOffset = 2;
                        for (++this.index; !this.eof();) {
                            var ch = this.source[this.index++];
                            if ("`" === ch) {
                                rawOffset = 1, terminated = tail = !0;
                                break
                            }
                            if ("$" === ch) {
                                ...
                                cooked += ch
                            } else if ("\\" === ch) if (ch = this.source[this.index++], character_1.Character.isLineTerminator(ch.charCodeAt(0))) ++this.lineNumber, "\r" === ch && "\n" === this.source[this.index] && ++this.index, this.lineStart = this.index; else switch (ch) {
                                case"n":
                                    cooked += "\n";
                                    break;
                                case"r":
                                ...
                                case"u":
                                    if ("{" === this.source[this.index]) ++this.index, cooked += this.scanUnicodeCodePointEscape(); else {
                                        ...
                                        null !== unescaped_2 ? cooked += unescaped_2 : (this.index = restore, cooked += ch)
                                    }
                                    break;
                                case"x":
                                    var unescaped = this.scanHexEscape(ch);
                                    null === unescaped && this.throwUnexpectedToken(messages_1.Messages.InvalidHexEscapeSequence), cooked += unescaped;
                                    break;
                                case"b":
                                    cooked += "\b";
                                    break;
                                ...
                                case"v":
                                    ...
                                    break;
                                default:
                                    "0" === ch ? (character_1.Character.isDecimalDigit(this.source.charCodeAt(this.index)) && this.throwUnexpectedToken(messages_1.Messages.TemplateOctalLiteral), cooked += "\0") : character_1.Character.isOctalDigit(ch.charCodeAt(0)) ? this.throwUnexpectedToken(messages_1.Messages.TemplateOctalLiteral) : cooked += ch
                            } else character_1.Character.isLineTerminator(ch.charCodeAt(0)) ? (++this.lineNumber, "\r" === ch && "\n" === this.source[this.index] && ++this.index, this.lineStart = this.index, cooked += "\n") : cooked += ch
                        }
                        ...
                        0 <= flags.indexOf("u") && (tmp = tmp.replace(/\\u\{([0-9a-fA-F]+)\}|\\u([a-fA-F0-9]{4})/g, function ($0, $1, $2) {
                            var codePoint = parseInt($1 || $2, 16);
                            return 1114111 < codePoint && self.throwUnexpectedToken(messages_1.Messages.InvalidRegExp), codePoint <= 65535 ? String.fromCharCode(codePoint) : "￿"
                        }).replace(/[\uD800-\uDBFF][\uDC00-\uDFFF]/g, "￿"));
                        try {
                            RegExp(tmp)
                        } catch (e) {
                        ...
                        for (var flags = ""; !this.eof();) {
                            ...
                            if (++this.index, "\\" !== ch || this.eof()) flags += ch, 0; else if ("u" === (ch = this.source[this.index])) {
                                ++this.index;
                                var restore = this.index, char = this.scanHexEscape("u");
                                if (null !== char) for (flags += char, 0; restore < this.index; ++restore) this.source[restore]; else this.index = restore, flags += "u", 0;
                ...
                "use strict";
                Object.defineProperty(exports, "__esModule", {value: !0});
                var error_handler_1 = __webpack_require__(10), scanner_1 = __webpack_require__(12),
                    token_1 = __webpack_require__(13), Reader = function () {
                        function Reader() {
                            this.values = [], this.curly = this.paren = -1
                        }

                        return Reader.prototype.beforeFunctionExpression = function (t) {
                            return 0 <= ["(", "{", "[", "in", "typeof", "instanceof", "new", "return", "case", "delete", "throw", "void", "=", "+=", "-=", "*=", "**=", "/=", "%=", "<<=", ">>=", ">>>=", "&=", "|=", "^=", ",", "+", "-", "*", "**", "/", "%", "++", "--", "<<", ">>", ">>>", "&", "|", "^", "!", "~", "&&", "||", "?", ":", "===", "==", ">=", "<=", "<", ">", "!=", "!=="].indexOf(t)
                            ...
                            7 === token.type || 4 === token.type ? ("{" === token.value ? this.curly = this.values.length : "(" === token.value && (this.paren = this.values.length), this.values.push(token.value)) : this.values.push(null)
                        }, Reader
                    }(), Tokenizer = function () {
                        function Tokenizer(code, config) {
                            this.errorHandler = new error_handler_1.ErrorHandler, this.errorHandler.tolerant = !!config && ("boolean" == typeof config.tolerant && config.tolerant), this.scanner = new scanner_1.Scanner(code, this.errorHandler), this.scanner.trackComment = !!config && ("boolean" == typeof config.comment && config.comment), this.trackRange = !!config && ("boolean" == typeof config.range && config.range), this.trackLoc = !!config && ("boolean" == typeof config.loc && config.loc), this.buffer = [], this.reader = new Reader
                        }
                        ...
                        return Tokenizer.prototype.errors = function () {
                            ...
                            if (0 === this.buffer.length) {
                                ...
                                if (this.scanner.trackComment) for (var i = 0; i < comments.length; ++i) {
                                    var e = comments[i], value = this.scanner.source.slice(e.slice[0], e.slice[1]),
                                        comment = {type: e.multiLine ? "BlockComment" : "LineComment", value: value};
            ...
            }], installedModules = {}, __webpack_require__.m = modules, __webpack_require__.c = installedModules, __webpack_require__.p = "", __webpack_require__(0);
        ...
        }, "function" == typeof define && define.amd ? define("esprima", [], factory) : "object" == typeof exports && "object" == typeof module ? module.exports = factory() : "object" == typeof exports ? exports.esprima = factory() : root.esprima = factory(), define("esprimaAdapter", ["./esprima", "env"], function (esprima, env) {
            return "xpconnect" === env.get() && "undefined" != typeof Reflect ? Reflect : esprima
        }), function (root, factory) {
            "function" == typeof define && define.amd ? define("source-map", [], factory) : root.sourceMap = factory()
        }(this, function () {
            return modules = [function (module, exports, __webpack_require__) {
                exports.SourceMapGenerator = __webpack_require__(1).SourceMapGenerator, exports.SourceMapConsumer = __webpack_require__(7).SourceMapConsumer, exports.SourceNode = __webpack_require__(10).SourceNode
            }, function (module, exports, __webpack_require__) {
                var base64VLQ = __webpack_require__(2), util = __webpack_require__(4),
                    ArraySet = __webpack_require__(5).ArraySet, MappingList = __webpack_require__(6).MappingList;
                ...
                SourceMapGenerator.prototype._version = 3, SourceMapGenerator.fromSourceMap = function (aSourceMapConsumer) {
                    var sourceRoot = aSourceMapConsumer.sourceRoot,
                        generator = new SourceMapGenerator({file: aSourceMapConsumer.file, sourceRoot: sourceRoot});
                    ...
                    var generated = util.getArg(aArgs, "generated"), original = util.getArg(aArgs, "original", null),
                        source = util.getArg(aArgs, "source", null), name = util.getArg(aArgs, "name", null);
                    ...
                    this._mappings.unsortedForEach(function (mapping) {
                        ...
                        var content = aSourceMapConsumer.sourceContentFor(sourceFile);
                        null != content && (null != aSourceMapPath && (sourceFile = util.join(aSourceMapPath, sourceFile)), null != sourceRoot && (sourceFile = util.relative(sourceRoot, sourceFile)), this.setSourceContent(sourceFile, content))
                    }, this)
                    ...
                    for (var next, mapping, nameIdx, sourceIdx, previousGeneratedColumn = 0, previousGeneratedLine = 1, previousOriginalColumn = 0, previousOriginalLine = 0, previousName = 0, previousSource = 0, result = "", mappings = this._mappings.toArray(), i = 0, len = mappings.length; i < len; i++) {
                        if (next = "", (mapping = mappings[i]).generatedLine !== previousGeneratedLine) for (previousGeneratedColumn = 0; mapping.generatedLine !== previousGeneratedLine;) next += ";", previousGeneratedLine++; else if (0 < i) {
                ...
                exports.encode = function (aValue) {
                    for (var digit, encoded = "", vlq = function (aValue) {
                        return aValue < 0 ? 1 + (-aValue << 1) : aValue << 1
                    }(aValue); digit = 31 & vlq, 0 < (vlq >>>= 5) && (digit |= 32), encoded += base64.encode(digit), 0 < vlq;) ;
                    return encoded
                }, exports.decode = function (aStr, aIndex, aOutParam) {
                    var continuation, digit, aValue, shifted, strLen = aStr.length, result = 0, shift = 0;
                    do {
                        if (strLen <= aIndex) throw new Error("Expected more digits in base 64 VLQ value.");
                    ...
                    aOutParam.value = (shifted = (aValue = result) >> 1, 1 == (1 & aValue) ? -shifted : shifted), aOutParam.rest = aIndex
                }
            }, function (module, exports) {
                ...
                function normalize(aPath) {
                    ...
                    for (var part, isAbsolute = exports.isAbsolute(path), parts = path.split(/\/+/), up = 0, i = parts.length - 1; 0 <= i; i--) "." === (part = parts[i]) ? parts.splice(i, 1) : ".." === part ? up++ : 0 < up && ("" === part ? (parts.splice(i + 1, up), up = 0) : (parts.splice(i, 2), up--));
                    return "" === (path = parts.join("/")) && (path = isAbsolute ? "/" : "."), url ? (url.path = path, urlGenerate(url)) : path
                }

                exports.urlParse = urlParse, exports.urlGenerate = urlGenerate, exports.normalize = normalize, exports.join = function (aRoot, aPath) {
                ...
                function isProtoString(s) {
                    if (s) {
                        var length = s.length;
                        if (!(length < 9) && 95 === s.charCodeAt(length - 1) && 95 === s.charCodeAt(length - 2) && 111 === s.charCodeAt(length - 3) && 116 === s.charCodeAt(length - 4) && 111 === s.charCodeAt(length - 5) && 114 === s.charCodeAt(length - 6) && 112 === s.charCodeAt(length - 7) && 95 === s.charCodeAt(length - 8) && 95 === s.charCodeAt(length - 9)) {
                            for (var i = length - 10; 0 <= i; i--) if (36 !== s.charCodeAt(i)) return;
                            return 1
                        }
                    }
                }
                ...
                function strcmp(aStr1, aStr2) {
                    return aStr1 === aStr2 ? 0 : aStr2 < aStr1 ? 1 : -1
                }
                ...
                var util = __webpack_require__(4), has = Object.prototype.hasOwnProperty;
                ...
                ArraySet.fromArray = function (aArray, aAllowDuplicates) {
                    ...
                    var sStr = util.toSetString(aStr), isDuplicate = has.call(this._set, sStr),
                    ...
                    if (0 <= aIdx && aIdx < this._array.length) return this._array[aIdx];
                ...
                var util = __webpack_require__(4), binarySearch = __webpack_require__(8),
                    ArraySet = __webpack_require__(5).ArraySet, base64VLQ = __webpack_require__(2),
                    quickSort = __webpack_require__(9).quickSort;

                function SourceMapConsumer(aSourceMap) {
                    var sourceMap = aSourceMap;
                    return "string" == typeof aSourceMap && (sourceMap = JSON.parse(aSourceMap.replace(/^\)\]\}'/, ""))), new (null != sourceMap.sections ? IndexedSourceMapConsumer : BasicSourceMapConsumer)(sourceMap)
                }

                function BasicSourceMapConsumer(aSourceMap) {
                    var sourceMap = aSourceMap;
                    "string" == typeof aSourceMap && (sourceMap = JSON.parse(aSourceMap.replace(/^\)\]\}'/, "")));
                    var version = util.getArg(sourceMap, "version"), sources = util.getArg(sourceMap, "sources"),
                        ...
                        mappings = util.getArg(sourceMap, "mappings"), file = util.getArg(sourceMap, "file", null);
                    if (version != this._version) throw new Error("Unsupported version: " + version);
                    sources = sources.map(String).map(util.normalize).map(function (source) {
                        return sourceRoot && util.isAbsolute(sourceRoot) && util.isAbsolute(source) ? util.relative(sourceRoot, source) : source
                    }), this._names = ArraySet.fromArray(names.map(String), !0), this._sources = ArraySet.fromArray(sources, !0), this.sourceRoot = sourceRoot, this.sourcesContent = sourcesContent, this._mappings = mappings, this.file = file
                }
                ...
                function IndexedSourceMapConsumer(aSourceMap) {
                    ...
                    this._sections = sections.map(function (s) {
                        ...
                        var offset = util.getArg(s, "offset"), offsetLine = util.getArg(offset, "line"),
                ...
                SourceMapConsumer.fromSourceMap = function (aSourceMap) {
                    ...
                    if (null != this.sourceRoot && (needle.source = util.relative(this.sourceRoot, needle.source)), !this._sources.has(needle.source)) return [];
                    ...
                    return mappings
                }, exports.SourceMapConsumer = SourceMapConsumer, (BasicSourceMapConsumer.prototype = Object.create(SourceMapConsumer.prototype)).consumer = SourceMapConsumer, BasicSourceMapConsumer.fromSourceMap = function (aSourceMap) {
                    ...
                    for (var mapping, str, segment, end, value, generatedLine = 1, previousGeneratedColumn = 0, previousOriginalLine = 0, previousOriginalColumn = 0, previousSource = 0, previousName = 0, length = aStr.length, index = 0, cachedSegments = {}, temp = {}, originalMappings = [], generatedMappings = []; index < length;) if (";" === aStr.charAt(index)) generatedLine++, index++, previousGeneratedColumn = 0; else if ("," === aStr.charAt(index)) index++; else {
                        ...
                        mapping.generatedColumn = previousGeneratedColumn + segment[0], previousGeneratedColumn = mapping.generatedColumn, 1 < segment.length && (mapping.source = previousSource + segment[1], previousSource += segment[1], mapping.originalLine = previousOriginalLine + segment[2], previousOriginalLine = mapping.originalLine, mapping.originalLine += 1, mapping.originalColumn = previousOriginalColumn + segment[3], previousOriginalColumn = mapping.originalColumn, 4 < segment.length && (mapping.name = previousName + segment[4], previousName += segment[4])), generatedMappings.push(mapping), "number" == typeof mapping.originalLine && originalMappings.push(mapping)
                    }
                    ...
                    if (null != this.sourceRoot && (aSource = util.relative(this.sourceRoot, aSource)), this._sources.has(aSource)) return this.sourcesContent[this._sources.indexOf(aSource)];
                    var url;
                    ...
                    if (null != this.sourceRoot && (source = util.relative(this.sourceRoot, source)), !this._sources.has(source)) return {
                        line: null,
                        column: null,
                        lastColumn: null
                    };
                ...
                }, exports.BasicSourceMapConsumer = BasicSourceMapConsumer, (IndexedSourceMapConsumer.prototype = Object.create(SourceMapConsumer.prototype)).constructor = SourceMapConsumer, IndexedSourceMapConsumer.prototype._version = 3, Object.defineProperty(IndexedSourceMapConsumer.prototype, "sources", {
                    get: function () {
                    ...
                    for (var i = 0; i < this._sections.length; i++) for (var section = this._sections[i], sectionMappings = section.consumer._generatedMappings, j = 0; j < sectionMappings.length; j++) {
                        ...
                        null !== section.consumer.sourceRoot && (source = util.join(section.consumer.sourceRoot, source)), this._sources.add(source), source = this._sources.indexOf(source);
                ...
                exports.GREATEST_LOWER_BOUND = 1, exports.LEAST_UPPER_BOUND = 2, exports.search = function (aNeedle, aHaystack, aCompare, aBias) {
                    ...
                    var index = function recursiveSearch(aLow, aHigh, aNeedle, aHaystack, aCompare, aBias) {
                        ...
                        return 0 === cmp ? mid : 0 < cmp ? 1 < aHigh - mid ? recursiveSearch(mid, aHigh, aNeedle, aHaystack, aCompare, aBias) : aBias == exports.LEAST_UPPER_BOUND ? aHigh < aHaystack.length ? aHigh : -1 : mid : 1 < mid - aLow ? recursiveSearch(aLow, mid, aNeedle, aHaystack, aCompare, aBias) : aBias == exports.LEAST_UPPER_BOUND ? mid : aLow < 0 ? -1 : aLow
                ...
                var SourceMapGenerator = __webpack_require__(1).SourceMapGenerator, util = __webpack_require__(4),
                ...
                SourceNode.fromStringWithSourceMap = function (aGeneratedCode, aSourceMapConsumer, aRelativePath) {
                    ...
                    var node = new SourceNode, remainingLines = aGeneratedCode.split(REGEX_NEWLINE),
                        lastGeneratedLine = 1, lastGeneratedColumn = 0, lastMapping = null;
                    return aSourceMapConsumer.eachMapping(function (mapping) {
                        if (null !== lastMapping) {
                            if (!(lastGeneratedLine < mapping.generatedLine)) {
                                var code = (nextLine = remainingLines[0]).substr(0, mapping.generatedColumn - lastGeneratedColumn);
                                return remainingLines[0] = nextLine.substr(mapping.generatedColumn - lastGeneratedColumn), lastGeneratedColumn = mapping.generatedColumn, addMappingWithCode(lastMapping, code), void (lastMapping = mapping)
                            }
                        ...
                        if (lastGeneratedColumn < mapping.generatedColumn) {
                            ...
                            node.add(nextLine.substr(0, mapping.generatedColumn)), remainingLines[0] = nextLine.substr(mapping.generatedColumn), lastGeneratedColumn = mapping.generatedColumn
                        }
                        lastMapping = mapping
                    }, this), 0 < remainingLines.length && (lastMapping && addMappingWithCode(lastMapping, shiftNextLine()), node.add(remainingLines.join(""))), aSourceMapConsumer.sources.forEach(function (sourceFile) {
                        var content = aSourceMapConsumer.sourceContentFor(sourceFile);
                    ...
                    var generated = {code: "", line: 1, column: 0}, map = new SourceMapGenerator(aArgs),
                        sourceMappingActive = !1, lastOriginalSource = null, lastOriginalLine = null,
                        lastOriginalColumn = null, lastOriginalName = null;
                    return this.walk(function (chunk, original) {
                        ...
                        for (var idx = 0, length = chunk.length; idx < length; idx++) 10 === chunk.charCodeAt(idx) ? (generated.line++, generated.column = 0, idx + 1 === length ? (lastOriginalSource = null, sourceMappingActive = !1) : sourceMappingActive && map.addMapping({
                            source: original.source,
                            original: {line: original.line, column: original.column},
                            generated: {line: generated.line, column: generated.column},
                            name: original.name
            ...
            ((DefaultsError.prototype = Object.create(Error.prototype)).constructor = DefaultsError).prototype.name = "DefaultsError", configure_error_stack(DefaultsError), DefaultsError.croak = function (msg, defs) {
            ...
            function remove(array, el) {
                for (var i = array.length; 0 <= --i;) array[i] === el && array.splice(i, 1)
            }
            ...
            function all(array, predicate) {
                for (var i = array.length; 0 <= --i;) if (!predicate(array[i])) return;
                return 1
            }
            ...
            function DEFNODE(type, props, methods, base) {
                ...
                for (var code = "return function AST_" + type + "(props){ if (props) { ", i = props.length; 0 <= --i;) code += "this." + props[i] + " = props." + props[i] + ";";
                var proto = base && new base;
                (proto && proto.initialize || methods && methods.initialize) && (code += "this.initialize();"), code += "}}";
                var ctor = new Function(code)();
                if (proto && (ctor.prototype = proto, ctor.BASE = base), base && base.SUBCLASSES.push(ctor), (ctor.prototype.CTOR = ctor).PROPS = props || null, ctor.SELF_PROPS = self_props, ctor.SUBCLASSES = [], type && (ctor.prototype.TYPE = ctor.TYPE = type), methods) for (i in methods) HOP(methods, i) && (/^\$/.test(i) ? ctor[i.substr(1)] = methods[i] : ctor.prototype[i] = methods[i]);
                return ctor.DEFMETHOD = function (name, method) {
                    this.prototype[name] = method
                }, void 0 !== exports && (exports["AST_" + type] = ctor), ctor
            }
            ...
            var AST_Statement = DEFNODE("Statement", null, {$documentation: "Base class of all statements"}),
                AST_Debugger = DEFNODE("Debugger", null, {$documentation: "Represents a debugger statement"}, AST_Statement),
                AST_Directive = DEFNODE("Directive", "value scope quote", {
            ...
            function walk_body(node, visitor) {
                var body = node.body;
                if (body instanceof AST_Statement) body._walk(visitor); else for (var i = 0, len = body.length; i < len; i++) body[i]._walk(visitor)
            }
            ...
            var AST_Block = DEFNODE("Block", "body", {
                ...
                AST_BlockStatement = DEFNODE("BlockStatement", null, {$documentation: "A block statement"}, AST_Block),
                    ...
                    clone: function (deep) {
                        ...
                        if (deep) {
                            ...
                            node.walk(new TreeWalker(function (node) {
                                node instanceof AST_LoopControl && node.label && node.label.thedef === def && (node.label.thedef = label).references.push(node)
                            }))
                        }
                        return node
                    }
                }, AST_StatementWithBody),
                AST_IterationStatement = DEFNODE("IterationStatement", null, {$documentation: "Internal class.  All loops inherit from it."}, AST_StatementWithBody),
                ...
                AST_Scope = DEFNODE("Scope", "directives variables functions uses_with uses_eval parent_scope enclosed cname", {
                    ...
                    wrap_enclose: function (arg_parameter_pairs) {
                        ...
                        return wrapped_tl = (wrapped_tl = parse(wrapped_tl)).transform(new TreeTransformer(function (node) {
                            if (node instanceof AST_Directive && "$ORIG" == node.value) return MAP.splice(self.body)
                        }))
                    },
                    wrap_commonjs: function (name, export_all) {
                        ...
                        return wrapped_tl = (wrapped_tl = parse(wrapped_tl)).transform(new TreeTransformer(function (node) {
                            if (node instanceof AST_Directive) switch (node.value) {
                                case"$ORIG":
                ...
                AST_Accessor = DEFNODE("Accessor", null, {$documentation: "A setter/getter function.  The `name` property is always null."}, AST_Lambda),
                AST_Function = DEFNODE("Function", null, {$documentation: "A function expression"}, AST_Lambda),
                AST_Defun = DEFNODE("Defun", null, {$documentation: "A function definition"}, AST_Lambda),
                AST_Jump = DEFNODE("Jump", null, {$documentation: "Base class for “jumps” (for now that's `return`, `throw`, `break` and `continue`)"}, AST_Statement),
                ...
                }, AST_Jump), AST_Return = DEFNODE("Return", null, {$documentation: "A `return` statement"}, AST_Exit),
                AST_Throw = DEFNODE("Throw", null, {$documentation: "A `throw` statement"}, AST_Exit),
                ...
                AST_Break = DEFNODE("Break", null, {$documentation: "A `break` statement"}, AST_LoopControl),
                AST_Continue = DEFNODE("Continue", null, {$documentation: "A `continue` statement"}, AST_LoopControl),
                ...
                AST_SwitchBranch = DEFNODE("SwitchBranch", null, {$documentation: "Base class for `switch` branches"}, AST_Block),
                AST_Default = DEFNODE("Default", null, {$documentation: "A `default` switch branch"}, AST_SwitchBranch),
                ...
                AST_Finally = DEFNODE("Finally", null, {$documentation: "A `finally` node; only makes sense as part of a `try` statement"}, AST_Block),
                ...
                AST_Var = DEFNODE("Var", null, {$documentation: "A `var` statement"}, AST_Definitions),
                AST_Const = DEFNODE("Const", null, {$documentation: "A `const` statement"}, AST_Definitions),
                ...
                AST_New = DEFNODE("New", null, {$documentation: "An object instantiation.  Derives from a function call since it has exactly the same properties"}, AST_Call),
                AST_Seq = DEFNODE("Seq", "car cdr", {
                    ...
                    $from_array: function (array) {
                        ...
                        for (var list = null, i = array.length; 0 <= --i;) list = AST_Seq.cons(array[i], list);
                        for (var p = list; p;) {
                            if (p.cdr && !p.cdr.cdr) {
                                p.cdr = p.cdr.car;
                                break
                    ...
                    len: function () {
                        return this.cdr instanceof AST_Seq ? this.cdr.len() + 1 : 2
                    },
                    _walk: function (visitor) {
                        return visitor._visit(this, function () {
                ...
                AST_UnaryPrefix = DEFNODE("UnaryPrefix", null, {$documentation: "Unary prefix expression, i.e. `typeof i` or `++i`"}, AST_Unary),
                AST_UnaryPostfix = DEFNODE("UnaryPostfix", null, {$documentation: "Unary postfix expression, i.e. `i++`"}, AST_Unary),
                ...
                AST_Assign = DEFNODE("Assign", null, {$documentation: "An assignment expression — `a = b + 5`"}, AST_Binary),
                ...
                AST_ObjectSetter = DEFNODE("ObjectSetter", null, {$documentation: "An object setter property"}, AST_ObjectProperty),
                AST_ObjectGetter = DEFNODE("ObjectGetter", null, {$documentation: "An object getter property"}, AST_ObjectProperty),
                ...
                AST_SymbolAccessor = DEFNODE("SymbolAccessor", null, {$documentation: "The name of a property accessor (setter/getter function)"}, AST_Symbol),
                AST_SymbolDeclaration = DEFNODE("SymbolDeclaration", "init", {$documentation: "A declaration symbol (symbol in var/const, function name or argument, symbol in catch)"}, AST_Symbol),
                AST_SymbolVar = DEFNODE("SymbolVar", null, {$documentation: "Symbol defining a variable"}, AST_SymbolDeclaration),
                AST_SymbolConst = DEFNODE("SymbolConst", null, {$documentation: "A constant declaration"}, AST_SymbolDeclaration),
                AST_SymbolFunarg = DEFNODE("SymbolFunarg", null, {$documentation: "Symbol naming a function argument"}, AST_SymbolVar),
                AST_SymbolDefun = DEFNODE("SymbolDefun", null, {$documentation: "Symbol defining a function"}, AST_SymbolDeclaration),
                AST_SymbolLambda = DEFNODE("SymbolLambda", null, {$documentation: "Symbol naming a function expression"}, AST_SymbolDeclaration),
                AST_SymbolCatch = DEFNODE("SymbolCatch", null, {$documentation: "Symbol naming the exception in catch"}, AST_SymbolDeclaration),
                ...
                AST_SymbolRef = DEFNODE("SymbolRef", null, {$documentation: "Reference to some symbol (not definition/declaration)"}, AST_Symbol),
                AST_LabelRef = DEFNODE("LabelRef", null, {$documentation: "Reference to a label symbol"}, AST_Symbol),
                ...
                }, AST_Constant), AST_Atom = DEFNODE("Atom", null, {$documentation: "Base class for atoms"}, AST_Constant),
                AST_Null = DEFNODE("Null", null, {$documentation: "The `null` atom", value: null}, AST_Atom),
                AST_NaN = DEFNODE("NaN", null, {$documentation: "The impossible value", value: NaN}, AST_Atom),
                AST_Undefined = DEFNODE("Undefined", null, {
                    $documentation: "The `undefined` value",
                    value: void 0
                }, AST_Atom),
                AST_Hole = DEFNODE("Hole", null, {$documentation: "A hole in an array", value: void 0}, AST_Atom),
                AST_Infinity = DEFNODE("Infinity", null, {
                    $documentation: "The `Infinity` value",
                    value: 1 / 0
                }, AST_Atom),
                AST_Boolean = DEFNODE("Boolean", null, {$documentation: "Base class for booleans"}, AST_Atom),
                AST_False = DEFNODE("False", null, {$documentation: "The `false` atom", value: !1}, AST_Boolean),
                AST_True = DEFNODE("True", null, {$documentation: "The `true` atom", value: !0}, AST_Boolean);
            ...
            TreeWalker.prototype = {
                _visit: function (node, descend) {
                    ...
                    return !ret && descend && descend.call(node), this.pop(node), ret
                }, parent: function (n) {
                    return this.stack[this.stack.length - 2 - (n || 0)]
                }, push: function (node) {
                    node instanceof AST_Lambda ? this.directives = Object.create(this.directives) : node instanceof AST_Directive && !this.directives[node.value] && (this.directives[node.value] = node), this.stack.push(node)
                }, pop: function (node) {
                    this.stack.pop(), node instanceof AST_Lambda && (this.directives = Object.getPrototypeOf(this.directives))
                }, self: function () {
                    return this.stack[this.stack.length - 1]
                }, find_parent: function (type) {
                    for (var stack = this.stack, i = stack.length; 0 <= --i;) {
                        var x = stack[i];
                    ...
                    if (node instanceof AST_Scope) for (var i = 0; i < node.body.length; ++i) {
                        var st = node.body[i];
                        if (!(st instanceof AST_Directive)) break;
                        if (st.value == type) return st
                    }
                }, in_boolean_context: function () {
                    for (var stack = this.stack, i = stack.length, self = stack[--i]; 0 < i;) {
                        var p = stack[--i];
            ...
            var KEYWORDS = "break case catch const continue debugger default delete do else finally for function if in instanceof new return switch throw try typeof var void while with",
                ...
                RESERVED_WORDS = "abstract boolean byte char class double enum export extends final float goto implements import int interface let long native package private protected public short static super synchronized this throws transient volatile yield " + KEYWORDS_ATOM + " " + KEYWORDS,
            ...
            var OPERATOR_CHARS = makePredicate(characters("+-*&%=<>!?|~^")), RE_HEX_NUMBER = /^0x[0-9a-f]+$/i,
                RE_OCT_NUMBER = /^0[0-7]+$/,
                OPERATORS = makePredicate(["in", "instanceof", "typeof", "new", "void", "delete", "++", "--", "+", "-", "!", "~", "&", "|", "^", "*", "/", "%", ">>", "<<", ">>>", "<", ">", "<=", ">=", "==", "===", "!=", "!==", "?", "=", "+=", "-=", "/=", "*=", "%=", ">>=", "<<=", ">>>=", "|=", "^=", "&=", "&&", "||"]),
                WHITESPACE_CHARS = makePredicate(characters("  \n\r\t\f\v​           \u2028\u2029  　\ufeff")),
                NEWLINE_CHARS = makePredicate(characters("\n\r\u2028\u2029")),
                PUNC_BEFORE_EXPRESSION = makePredicate(characters("[{(,;:")),
                PUNC_CHARS = makePredicate(characters("[]{}(),;:")), UNICODE = (makePredicate(characters("gmsiy")), {
            ...
            function is_letter(code) {
                return 97 <= code && code <= 122 || 65 <= code && code <= 90 || 170 <= code && UNICODE.letter.test(String.fromCharCode(code))
            }
            ...
            function is_identifier_char(ch) {
                ...
                return is_identifier_start(code) || is_digit(code) || 8204 == code || 8205 == code || function (ch) {
            ...
            function JS_Parse_Error(message, filename, line, col, pos) {
                this.message = message, this.filename = filename, this.line = line, this.col = col, this.pos = pos
            }
            ...
            ((JS_Parse_Error.prototype = Object.create(Error.prototype)).constructor = JS_Parse_Error).prototype.name = "SyntaxError", configure_error_stack(JS_Parse_Error);
            var EX_EOF = {};
            ...
            function tokenizer($TEXT, filename, html5_comments, shebang) {
                ...
                function next(signal_eof, in_string) {
                    ...
                    return NEWLINE_CHARS(ch) ? (S.newline_before = S.newline_before || !in_string, ++S.line, S.col = 0, in_string || "\r" != ch || "\n" != peek() || (++S.pos, ch = "\n")) : ++S.col, ch
                }
                ...
                function token(type, value, is_comment) {
                    S.regex_allowed = "operator" == type && !UNARY_POSTFIX(value) || "keyword" == type && KEYWORDS_BEFORE_EXPRESSION(value) || "punc" == type && PUNC_BEFORE_EXPRESSION(value), "punc" == type && "." == value ? prev_was_dot = !0 : is_comment || (prev_was_dot = !1);
                    var ret = {
                        type: type,
                        value: value,
                        line: S.tokline,
                    ...
                    if (/^(?:num|string|regexp)$/i.test(type) && (ret.raw = $TEXT.substring(ret.pos, ret.endpos)), !is_comment) {
                        ret.comments_before = S.comments_before, S.comments_before = [];
                        for (var i = 0, len = ret.comments_before.length; i < len; i++) ret.nlb = ret.nlb || ret.comments_before[i].nlb
                    }
                ...
                function read_escaped_char(in_string) {
                    ...
                    return "0" <= ch && ch <= "7" ? function (ch) {
                        var p = peek();
                        "0" <= p && p <= "7" && (ch += next(!0))[0] <= "3" && "0" <= (p = peek()) && p <= "7" && (ch += next(!0));
                        if ("0" === ch) return "\0";
                        0 < ch.length && next_token.has_directive("use strict") && parse_error("Legacy octal escape sequences are not allowed in strict mode");
                ...
                var skip_multiline_comment = with_eof_error("Unterminated multiline comment", function () {
                    var regex_allowed = S.regex_allowed, i = function (what, signal_eof) {
                        var pos = S.text.indexOf(what, S.pos);
                        if (signal_eof && -1 == pos) throw EX_EOF;
                        return pos
                    }("*/", !0), text = S.text.substring(S.pos, i).replace(/\r\n|\r|\u2028|\u2029/g, "\n");
                    return forward(text.length + 2), S.comments_before.push(token("comment2", text, !0)), S.regex_allowed = regex_allowed, next_token
                });

                function read_name() {
                    for (var ch, hex, backslash = !1, name = "", escaped = !1; null != (ch = peek());) if (backslash) "u" != ch && parse_error("Expecting UnicodeEscapeSequence -- uXXXX"), is_identifier_char(ch = read_escaped_char()) || parse_error("Unicode char: " + ch.charCodeAt(0) + " is not valid in identifier"), name += ch, backslash = !1; else if ("\\" == ch) escaped = backslash = !0, next(); else {
                ...
                var read_regexp = with_eof_error("Unterminated regular expression", function (regexp) {
                    for (var ch, prev_backslash = !1, in_class = !1; ch = next(!0);) if (NEWLINE_CHARS(ch)) parse_error("Unexpected line terminator"); else if (prev_backslash) regexp += "\\" + ch, prev_backslash = !1; else if ("[" == ch) in_class = !0, regexp += ch; else if ("]" == ch && in_class) in_class = !1, regexp += ch; else {
                ...
                function next_token(force_regexp) {
                    ...
                    for (shebang && 0 == S.pos && looking_at("#!") && (start_token(), forward(2), skip_line_comment("comment5")); ;) {
                        ...
                        if (92 == code || is_identifier_start(code)) return word = read_name(), prev_was_dot ? token("name", word) : KEYWORDS_ATOM(word) ? token("atom", word) : KEYWORDS(word) ? OPERATORS(word) ? token("operator", word) : token("keyword", word) : token("name", word);
                        break
                    }
                    var word;
                ...
                return next_token.context = function (nc) {
                    ...
                    return void 0 !== S.directives[directive] && 0 < S.directives[directive]
                }, next_token
            }
            ...
            var UNARY_PREFIX = makePredicate(["typeof", "void", "delete", "--", "++", "!", "~", "-", "+"]),
                ...
                ATOMIC_START_TOKEN = (array_to_hash(["for", "do", "while", "switch"]), array_to_hash(["atom", "num", "string", "regexp", "name"]));
            ...
            function parse($TEXT, options) {
                ...
                var statement = embed_tokens(function () {
                    switch (handle_regexp(), S.token.type) {
                        case"string":
                            if (S.in_directives) {
                                var token = peek();
                                -1 == S.token.raw.indexOf("\\") && (token.nlb || is_token(token, "eof") || is_token(token, "punc", ";") || is_token(token, "punc", "}")) ? S.input.add_directive(S.token.value) : S.in_directives = !1
                            }
                        ...
                        case"keyword":
                            switch (S.token.value) {
                                ...
                                case"do":
                                    ...
                                    return semicolon(!0), new AST_Do({body: body, condition: condition});
                                case"while":
                                ...
                                case"for":
                                    return next(), function () {
                                        expect("(");
                                        var init = null;
                                        if (!is("punc", ";") && (init = is("keyword", "var") ? (next(), var_(!0)) : expression(!0, !0), is("operator", "in"))) return init instanceof AST_Var && 1 < init.definitions.length && croak("Only one variable declaration allowed in for..in loop"), next(), function (init) {
                                ...
                                case"function":
                                    return next(), function_(AST_Defun);
                                case"if":
                                    return next(), function () {
                                ...
                                case"return":
                                    ...
                                    var value = null;
                                    return is("punc", ";") ? next() : can_insert_semicolon() || (value = expression(!0), semicolon()), new AST_Return({value: value});
                                case"switch":
                                ...
                                case"throw":
                                    next(), S.token.nlb && croak("Illegal newline after 'throw'");
                                    value = expression(!0);
                                    return semicolon(), new AST_Throw({value: value});
                                case"try":
                                    return next(), function () {
                ...
                function simple_statement(tmp) {
                    return new AST_SimpleStatement({body: (tmp = expression(!0), semicolon(), tmp)})
                }
                ...
                function break_cont(type) {
                    ...
                    can_insert_semicolon() || (label = as_symbol(AST_LabelRef, !0)), null != label ? ((ldef = find_if(function (l) {
                        return l.name == label.name
                    }, S.labels)) || croak("Undefined label " + label.name), label.thedef = ldef) : 0 == S.in_loop && croak(type.TYPE + " not inside a loop or switch"), semicolon();
                ...
                var function_ = function (ctor) {
                    var in_statement = ctor === AST_Defun,
                        name = is("name") ? as_symbol(in_statement ? AST_SymbolDefun : AST_SymbolLambda) : null;
                    return in_statement && !name && unexpected(), expect("("), new ctor({
                        ...
                        body: function (loop, labels) {
                            ++S.in_function, S.in_directives = !0, S.input.push_directives_stack(), S.in_loop = 0, S.labels = [];
                            var a = block_();
                            return S.input.pop_directives_stack(), --S.in_function, S.in_loop = loop, S.labels = labels, a
                        }(S.in_loop, S.labels)
                    })
                };
                ...
                var var_ = function (no_in) {
                    return new AST_Var({start: prev(), definitions: vardefs(no_in, !1), end: prev()})
                }, const_ = function () {
                    return new AST_Const({start: prev(), definitions: vardefs(!1, !0), end: prev()})
                };
                var expr_atom = function (allow_calls) {
                    if (is("operator", "new")) return function (allow_calls) {
                        ...
                        return args = is("punc", "(") ? (next(), expr_list(")")) : [], subscripts(new AST_New({
                            start: start,
                            expression: newexp,
                            args: args,
                            end: prev()
                    ...
                    if (ATOMIC_START_TOKEN[S.token.type]) return subscripts(function () {
                        var ret, tok = S.token;
                        switch (tok.type) {
                            case"name":
                            case"keyword":
                            ...
                            case"atom":
                                ...
                                break;
                            case"operator":
                                is_identifier_string(tok.value) || croak("Invalid getter/setter name: " + tok.value, tok.line, tok.col, tok.pos), ret = _make_symbol(AST_SymbolRef)
                        }
                        return next(), ret
                    }(), allow_calls);
                    unexpected()
                ...
                var array_ = embed_tokens(function () {
                    return expect("["), new AST_Array({elements: expr_list("]", !options.strict, !0)})
                }), create_accessor = embed_tokens(function () {
                    ...
                    return next(), new AST_Object({properties: a})
                });
                ...
                function _make_symbol(type) {
                    ...
                    return new ("this" == name ? AST_This : type)({name: String(name), start: S.token, end: S.token})
                }
                ...
                var subscripts = function (expr, allow_calls) {
                    ...
                    if (is("punc", ".")) return next(), subscripts(new AST_Dot({
                        start: start,
                        expression: expr,
                        property: ("name" != (tmp = S.token).type && unexpected(), next(), tmp.value),
                        end: prev()
                    }), allow_calls);
                    if (is("punc", "[")) {
                        next();
                    ...
                    for (var val = expr_atom(allow_calls); is("operator") && UNARY_POSTFIX(S.token.value) && !S.token.nlb;) (val = make_unary(AST_UnaryPostfix, S.token, val)).start = start, val.end = S.token, next();
                    return val
                };

                function make_unary(ctor, token, expr) {
                    var op = token.value;
                    return "++" != op && "--" != op || is_assignable(expr) || croak("Invalid use of " + op + " operator", token.line, token.col, token.pos), new ctor({
                        operator: op,
                        expression: expr
                    })
                }
                ...
                function is_assignable(expr) {
                    return options.cli || (expr instanceof AST_PropAccess || expr instanceof AST_SymbolRef)
                }
            ...
            function _(node, descend) {
                node.DEFMETHOD("transform", function (tw, in_list) {
                    var x, y;
                    return tw.push(this), tw.before && (x = tw.before(this, descend, in_list)), void 0 === x && (tw.after ? (tw.stack[tw.stack.length - 1] = x = this, descend(x, tw), void 0 !== (y = tw.after(x, in_list)) && (x = y)) : descend(x = this, tw)), tw.pop(this), x
                })
            }
            ...
            function SymbolDef(scope, index, orig) {
                this.name = orig.name, this.orig = [orig], this.scope = scope, this.references = [], this.global = !1, this.mangled_name = null, this.undeclared = !1, this.index = index, this.id = SymbolDef.next_id++
            }

            TreeTransformer.prototype = new TreeWalker, _(AST_Node, noop), _(AST_LabeledStatement, function (self, tw) {
                ...
                unmangleable: function (options) {
                    ...
                    if (this.global && cache && cache.has(this.name)) this.mangled_name = cache.get(this.name); else if (!this.mangled_name && !this.unmangleable(options)) {
                        ...
                        !options.screw_ie8 && sym instanceof AST_SymbolLambda && (s = s.parent_scope), this.defun && (def = this.defun.variables.get(this.name)) ? this.mangled_name = def.mangled_name || def.name : this.mangled_name = s.next_mangled(options, this), this.global && cache && cache.set(this.name, this.mangled_name)
                    }
                }
            }, AST_Toplevel.DEFMETHOD("figure_out_scope", function (options) {
                options = defaults(options, {cache: null, screw_ie8: !0});
                var self = this, scope = self.parent_scope = null, labels = new Dictionary, defun = null,
                    tw = new TreeWalker(function (node, descend) {
                        if (node instanceof AST_Catch) {
                            var save_scope = scope;
                            return (scope = new AST_Scope(node)).init_scope_vars(save_scope), descend(), scope = save_scope, !0
                        }
                        if (node instanceof AST_Scope) {
                            node.init_scope_vars(scope);
                            save_scope = scope;
                            var save_defun = defun, save_labels = labels;
                            return defun = scope = node, labels = new Dictionary, descend(), scope = save_scope, defun = save_defun, labels = save_labels, !0
                        }
                        ...
                        if (node instanceof AST_With) for (var s = scope; s; s = s.parent_scope) s.uses_with = !0; else if (node instanceof AST_Symbol && (node.scope = scope), node instanceof AST_Label && ((node.thedef = node).references = []), node instanceof AST_SymbolLambda) defun.def_function(node); else if (node instanceof AST_SymbolDefun) (node.scope = defun.parent_scope).def_function(node); else if (node instanceof AST_SymbolVar || node instanceof AST_SymbolConst) {
                            if (defun.def_variable(node), defun !== scope) {
                ...
                self.walk(tw);
                var func = null;
                self.globals = new Dictionary, tw = new TreeWalker(function (node, descend) {
                    ...
                    if (node instanceof AST_SymbolRef) {
                        ...
                        return node.scope instanceof AST_Lambda && "arguments" == name && (node.scope.uses_arguments = !0), sym = sym || self.def_global(node), node.thedef = sym, node.reference(options), !0
                    }
                });
                self.walk(tw), options.screw_ie8 || self.walk(new TreeWalker(function (node, descend) {
                    if (node instanceof AST_SymbolCatch) {
                        var name = node.name, refs = node.thedef.references, scope = node.thedef.defun,
                            def = scope.find_variable(name) || self.globals.get(name) || scope.def_variable(node);
                        return refs.forEach(function (ref) {
                            ref.thedef = def, ref.reference(options)
                        }), node.thedef = def, !0
                    }
                })), options.cache && (this.cname = options.cache.cname)
                ...
                this.variables = new Dictionary, this.functions = new Dictionary, this.uses_with = !1, this.uses_eval = !1, this.parent_scope = parent_scope, this.enclosed = [], this.cname = -1
            }), AST_Lambda.DEFMETHOD("init_scope_vars", function () {
                AST_Scope.prototype.init_scope_vars.apply(this, arguments), this.uses_arguments = !1, this.def_variable(new AST_SymbolVar({
                    name: "arguments",
                    start: this.start,
                    end: this.end
                }))
            }), AST_Symbol.DEFMETHOD("mark_enclosed", function (options) {
                for (var def = this.definition(), s = this.scope; s && (push_uniq(s.enclosed, def), options.keep_fnames && s.functions.each(function (d) {
                ...
                var def;
                return this.variables.has(symbol.name) ? (def = this.variables.get(symbol.name)).orig.push(symbol) : (def = new SymbolDef(this, this.variables.size(), symbol), this.variables.set(symbol.name, def), def.global = !this.parent_scope), symbol.thedef = def
            }), AST_Scope.DEFMETHOD("next_mangled", function (options) {
                var ext = this.enclosed;
                out:for (; ;) {
                    var m = base54(++this.cname);
                    if (is_identifier(m) && !(0 <= options.except.indexOf(m))) {
                        for (var i = ext.length; 0 <= --i;) {
                            var sym = ext[i];
                ...
                return defaults(options, {eval: !1, except: [], keep_fnames: !1, screw_ie8: !0, sort: !1, toplevel: !1})
            }), AST_Toplevel.DEFMETHOD("mangle_names", function (options) {
                (options = this._default_mangler_options(options)).except.push("arguments");
                ...
                var tw = new TreeWalker(function (node, descend) {
                    ...
                    if (node instanceof AST_Label) {
                        for (var name; !is_identifier(name = base54(++lname));) ;
                        return node.mangled_name = name, !0
                    }
                    options.screw_ie8 && node instanceof AST_SymbolCatch && to_mangle.push(node.definition())
                });
            ...
            var base54 = function () {
                ...
                function reset() {
                    frequency = Object.create(null), (chars = "abcdefghijklmnopqrstuvwxyzABCDEFGHIJKLMNOPQRSTUVWXYZ$_0123456789".split("").map(function (ch) {
                ...
                function base54(num) {
                    var ret = "", base = 54;
                    for (num++; num--, ret += String.fromCharCode(chars[num % base]), num = Math.floor(num / base), base = 64, 0 < num;) ;
                    return ret
                }

                return base54.consider = function (str) {
                    for (var i = str.length; 0 <= --i;) {
                ...
                }, (base54.reset = reset)(), base54.get = function () {
                    return chars
            ...
            var use_asm, in_directive, EXPECT_DIRECTIVE = /^$|[;{][\s\n]*$/;
            ...
            function OutputStream(options) {
                ...
                function encode_string(str, quote) {
                    var ret = function (str, quote) {
                        ...
                        switch (str = str.replace(/[\\\b\f\n\r\v\t\x22\x27\u2028\u2029\0\ufeff]/g, function (s, i) {
                            ...
                            default:
                                return (sq < dq ? quote_single : quote_double)()
                        }
                    }(str, quote);
                    return options.inline_script && (ret = (ret = (ret = ret.replace(/<\x2fscript([>\/\t\n\f\r ])/gi, "<\\/script$1")).replace(/\x3c!--/g, "\\x3c!--")).replace(/--\x3e/g, "--\\x3e")), ret
                }
                ...
                var might_need_space = !1, might_need_semicolon = !1, might_add_newline = 0, last = "",
                    ensure_line_len = options.max_line_len ? function () {
                ...
                function print(str) {
                    var ch = (str = String(str)).charAt(0), prev = last.charAt(last.length - 1);
                    if (might_need_semicolon && (might_need_semicolon = !1, (":" == prev && "}" == ch || (!ch || ";}".indexOf(ch) < 0) && ";" != prev) && (options.semicolons || requireSemicolonChars(ch) ? (OUTPUT += ";", current_col++, current_pos++) : (ensure_line_len(), OUTPUT += "\n", current_pos++, current_line++, current_col = 0, /^\s+$/.test(str) && (might_need_semicolon = !0)), options.beautify || (might_need_space = !1))), !options.beautify && options.preserve_line && stack[stack.length - 1]) for (var target_line = stack[stack.length - 1].start.line; current_line < target_line;) ensure_line_len(), OUTPUT += "\n", current_pos++, current_line++, current_col = 0, might_need_space = !1;
                    might_need_space && ((is_identifier_char(prev) && (is_identifier_char(ch) || "\\" == ch) || "/" == ch && ch == prev || ("+" == ch || "-" == ch) && ch == last) && (OUTPUT += " ", current_col++, current_pos++), might_need_space = !1), OUTPUT += str, current_pos += str.length;
                    var a = str.split(/\r?\n/), n = a.length - 1;
                    current_line += n, current_col += a[0].length, 0 < n && (ensure_line_len(), current_col = a[n].length), last = str
                }
                ...
                return {
                    ...
                    print_string: function (str, quote, escape_directive) {
                        ...
                        !0 === escape_directive && -1 === encoded.indexOf("\\") && (EXPECT_DIRECTIVE.test(OUTPUT) || force_semicolon(), force_semicolon()), print(encoded)
                    },
            ...
            function display_body(body, is_toplevel, output, allow_directives) {
                ...
                in_directive = allow_directives, body.forEach(function (stmt, i) {
                    !0 !== in_directive || stmt instanceof AST_Directive || stmt instanceof AST_EmptyStatement || stmt instanceof AST_SimpleStatement && stmt.body instanceof AST_String || (in_directive = !1), stmt instanceof AST_EmptyStatement || (output.indent(), stmt.print(output), i == last && is_toplevel || (output.newline(), is_toplevel && output.newline())), !0 === in_directive && stmt instanceof AST_SimpleStatement && stmt.body instanceof AST_String && (in_directive = !1)
                }), in_directive = !1
            }

            function print_bracketed(body, output, allow_directives) {
                0 < body.length ? output.with_block(function () {
                    display_body(body, !1, output, allow_directives)
                }) : output.print("{}")
            }
            ...
            function print_property_name(key, quote, output) {
                output.option("quote_keys") ? output.print_string(key + "") : ("number" == typeof key || !output.option("beautify") && +key + "" == key) && 0 <= parseFloat(key) ? output.print(make_num(key)) : (RESERVED_WORDS(key) ? !output.option("screw_ie8") : !is_identifier_string(key)) || quote && output.option("keep_quoted_props") ? output.print_string(key, quote) : output.print_name(key)
            }
            ...
            function make_num(num) {
                ...
                return Math.floor(num) === num ? (0 <= num ? a.push("0x" + num.toString(16).toLowerCase(), "0" + num.toString(8)) : a.push("-0x" + (-num).toString(16).toLowerCase(), "-0" + (-num).toString(8)), (m = /^(.*?)(0+)$/.exec(num)) && a.push(m[1] + "e" + m[2].length)) : (m = /^0?\.(0+)(.*)$/.exec(num)) && a.push(m[2] + "e-" + (m[1].length + m[2].length), str.substr(str.indexOf("."))), function (a) {
            ...
            function Compressor(options, false_by_default) {
                ...
                top_retain instanceof RegExp ? this.top_retain = function (def) {
                    ...
                    return 0 <= top_retain.indexOf(def.name)
                });
                var sequences = this.options.sequences;
                this.sequences_limit = 1 == sequences ? 200 : 0 | sequences, this.warnings_produced = {}
            }

            function readReservedFile(filename, reserved) {
                reserved = reserved || {vars: [], props: []};
                var data = rjsFile.readFile(filename, "utf8");
                return (data = JSON.parse(data)).vars && data.vars.forEach(function (name) {
                    exports.push_uniq(reserved.vars, name)
                }), data.props && data.props.forEach(function (name) {
                    exports.push_uniq(reserved.props, name)
                }), reserved
            }

            in_directive = use_asm = !1, AST_Node.DEFMETHOD("print", function (stream, force_parens) {
                ...
                self instanceof AST_Directive && "use asm" == self.value && stream.parent() instanceof AST_Scope && (use_asm = !0), stream.push_node(self), force_parens || self.needs_parens(stream) ? stream.with_parens(doit) : doit(), stream.pop_node(), self instanceof AST_Scope && (use_asm = prev_use_asm)
            }), AST_Node.DEFMETHOD("print_to_string", function (options) {
                var s = OutputStream(options);
                return options || (s._readonly = !0), this.print(s), s.get()
                ...
                if (!output._readonly) {
                    ...
                    if (start && !start._comments_dumped) {
                        ...
                        if (this instanceof AST_Exit && this.value && this.value.walk(new TreeWalker(function (node) {
                            if (node.start && node.start.comments_before && (comments = comments.concat(node.start.comments_before), node.start.comments_before = []), node instanceof AST_Function || node instanceof AST_Array || node instanceof AST_Object) return !0
                        })), 0 == output.pos()) {
                            0 < comments.length && output.option("shebang") && "comment5" == comments[0].type && (output.print("#!" + comments.shift().value + "\n"), output.indent());
                            var preamble = output.option("preamble");
                            preamble && output.print(preamble.replace(/\r\n?|[\n\u2028\u2029]|\s*$/g, "\n"))
                        }
                ...
                if (p instanceof AST_Binary) {
                    ...
                    if (sp < pp || pp == sp && this === p.right) return !0
                }
                ...
                var p = output.parent();
                return p instanceof AST_Unary || (p instanceof AST_Binary && !(p instanceof AST_Assign) || (p instanceof AST_Call && p.expression === this || (p instanceof AST_Conditional && p.condition === this || (p instanceof AST_PropAccess && p.expression === this || void 0))))
                ...
                output.print("do"), output.space(), make_block(self.body, output), output.space(), output.print("while"), output.space(), output.with_parens(function () {
                ...
                }), output.space(), print_bracketed(self.body, output, !0)
                ...
                output.print("if"), output.space(), output.with_parens(function () {
                    ...
                    var b = self.body;
                    if (output.option("bracketize") || !output.option("screw_ie8") && b instanceof AST_Do) return make_block(b, output);
                    if (!b) return output.force_semicolon();
                    for (; ;) if (b instanceof AST_If) {
                        if (!b.alternative) return make_block(self.body, output);
                        b = b.alternative
                    } else {
                ...
                last < 0 ? output.print("{}") : output.with_block(function () {
                    self.body.forEach(function (branch, i) {
                        output.indent(!0), branch.print(output), i < last && 0 < branch.body.length && output.newline()
                    })
                })
                ...
                self.expression.print(output), self instanceof AST_New && !need_constructor_parens(self, output) || output.with_parens(function () {
                ...
                expr.print(output), expr instanceof AST_Number && 0 <= expr.getValue() && (/[xa-f.)]/i.test(output.last()) || output.print(".")), output.print("."), output.add_mapping(self.end), output.print_name(self.property)
            }), DEFPRINT(AST_Sub, function (self, output) {
                self.expression.print(output), output.print("["), self.property.print(output), output.print("]")
            }), DEFPRINT(AST_UnaryPrefix, function (self, output) {
                var op = self.operator;
                output.print(op), (/^[a-z]/i.test(op) || /[+-]$/.test(op) && self.expression instanceof AST_UnaryPrefix && /^[+-]/.test(self.expression.operator)) && output.space(), self.expression.print(output)
            }), DEFPRINT(AST_UnaryPostfix, function (self, output) {
                self.expression.print(output), output.print(self.operator)
            }), DEFPRINT(AST_Binary, function (self, output) {
                var op = self.operator;
                self.left.print(output), ">" == op[0] && self.left instanceof AST_UnaryPostfix && "--" == self.left.operator ? output.print(" ") : output.space(), output.print(op), ("<" == op || "<<" == op) && self.right instanceof AST_UnaryPrefix && "!" == self.right.operator && self.right.expression instanceof AST_UnaryPrefix && "--" == self.right.expression.operator ? output.print(" ") : output.space(), self.right.print(output)
                ...
                output.with_square(function () {
                    ...
                    0 < len && output.space(), a.forEach(function (exp, i) {
                        i && output.comma(), exp.print(output), i === len - 1 && exp instanceof AST_Hole && output.comma()
                    }), 0 < len && output.space()
                })
            }), DEFPRINT(AST_Object, function (self, output) {
                0 < self.properties.length ? output.with_block(function () {
                ...
                output.print(type), output.space(), print_property_name(this.key.name, this.quote, output), this.value._do_print(output, !0)
                ...
                output.option("ascii_only") ? str = output.to_ascii(str) : output.option("unescape_regexps") && (str = str.split("\\\\").map(function (str) {
                    return str.replace(/\\u[0-9a-fA-F]{4}|\\x[0-9a-fA-F]{2}/g, function (s) {
                        ...
                        }(code) ? String.fromCharCode(code) : s
                    })
            ...
            }), merge(Compressor.prototype = new TreeTransformer, {
                option: function (key) {
                    return this.options[key]
                }, compress: function (node) {
                    this.option("expression") && (node = node.process_expression(!0));
                    for (var passes = +this.options.passes || 1, pass = 0; pass < passes && pass < 3; ++pass) (0 < pass || this.option("reduce_vars")) && node.reset_opt_flags(this, !0), node = node.transform(this);
                    return this.option("expression") && (node = node.process_expression(!1)), node
                }, info: function () {
                    ...
                    var was_scope = !1;
                    node instanceof AST_Scope && (node = node.hoist_declarations(this), was_scope = !0), descend(node, this), descend(node, this);
                    var opt = node.optimize(this);
                    return was_scope && opt instanceof AST_Scope && (opt.drop_unused(this), descend(opt, this)), opt === node && (opt._squeezed = !0), opt
                }
            }), function () {
                ...
                function is_reference_const(ref) {
                    if (ref instanceof AST_SymbolRef) for (var orig = ref.definition().orig, i = orig.length; 0 <= --i;) if (orig[i] instanceof AST_SymbolConst) return 1
                }
                ...
                function make_node_from_constant(val, orig) {
                    switch (typeof val) {
                        ...
                        case"number":
                            ...
                            }) : make_node(AST_Number, orig, {value: val}) : val < 0 ? make_node(AST_UnaryPrefix, orig, {
                                operator: "-",
                        ...
                        default:
                            if (null === val) return make_node(AST_Null, orig, {value: null});
                            if (val instanceof RegExp) return make_node(AST_RegExp, orig, {value: val});
                            throw new Error(string_template("Can't handle constant of type: {type}", {type: typeof val}))
                    }
                }

                function maintain_this_binding(parent, orig, val) {
                    return parent instanceof AST_UnaryPrefix && "delete" == parent.operator || parent instanceof AST_Call && parent.expression === orig && (val instanceof AST_PropAccess || val instanceof AST_SymbolRef && "eval" == val.name) ? make_node(AST_Seq, orig, {
                ...
                function loop_body(x) {
                    return !(x instanceof AST_Switch) && (x instanceof AST_For || x instanceof AST_ForIn || x instanceof AST_DWLoop) && x.body instanceof AST_BlockStatement ? x.body : x
                }

                function is_iife_call(node) {
                    return node instanceof AST_Call && !(node instanceof AST_New) && (node.expression instanceof AST_Function || is_iife_call(node.expression))
                }

                function tighten_body(statements, compressor) {
                    for (var CHANGED, max_iter = 10; CHANGED = !1, compressor.option("angular") && (statements = process_for_angular(statements)), statements = eliminate_spurious_blocks(statements), compressor.option("dead_code") && (statements = eliminate_dead_code(statements, compressor)), compressor.option("if_return") && (statements = handle_if_return(statements, compressor)), 0 < compressor.sequences_limit && (statements = sequencesize(statements, compressor)), compressor.option("join_vars") && (statements = join_consecutive_vars(statements)), compressor.option("collapse_vars") && (statements = collapse_single_use_vars(statements, compressor)), CHANGED && 0 < max_iter--;) ;
                    return statements;

                    function collapse_single_use_vars(statements, compressor) {
                        for (var self = compressor.self(), var_defs_removed = !1, toplevel = compressor.option("toplevel"), stat_index = statements.length; 0 <= --stat_index;) {
                            var stat = statements[stat_index];
                            if (!(stat instanceof AST_Definitions)) {
                                if ([stat, stat.body, stat.alternative, stat.bcatch, stat.bfinally].forEach(function (node) {
                                    node && node.body && collapse_single_use_vars(node.body, compressor)
                                }), stat_index <= 0) break;
                                var prev_stat_index = stat_index - 1, prev_stat = statements[prev_stat_index];
                                if (prev_stat instanceof AST_Definitions) {
                                    var var_defs = prev_stat.definitions;
                                    if (null != var_defs) for (var var_names_seen = {}, side_effects_encountered = !1, lvalues_encountered = !1, lvalues = {}, var_defs_index = var_defs.length; 0 <= --var_defs_index;) {
                                        var var_decl = var_defs[var_defs_index];
                                        if (null == var_decl.value) break;
                                        var var_name = var_decl.name.name;
                                        if (!var_name || !var_name.length) break;
                                        if (var_name in var_names_seen) break;
                                        var_names_seen[var_name] = !0;
                                        var def = self.find_variable && self.find_variable(var_name);
                                        if (def && def.references && 1 === def.references.length && "arguments" != var_name && (toplevel || !def.global)) {
                                            var ref = def.references[0];
                                            if (ref.scope.uses_eval || ref.scope.uses_with) break;
                                            if (var_decl.value.is_constant()) {
                                                var ctt = new TreeTransformer(function (node) {
                                                    var parent = ctt.parent();
                                                    return parent instanceof AST_IterationStatement && (parent.condition === node || parent.init === node) ? node : node === ref ? replace_var(node, parent, !0) : void 0
                                                });
                                                stat.transform(ctt)
                                            } else if (!(side_effects_encountered |= lvalues_encountered)) {
                                                var value_has_side_effects = var_decl.value.has_side_effects(compressor);
                                                if (ref.scope === self) {
                                                    var tw = new TreeWalker(function (node) {
                                                        node instanceof AST_SymbolRef && is_lvalue(node, tw.parent()) && (lvalues[node.name] = lvalues_encountered = !0)
                                                    });
                                                    var_decl.value.walk(tw);
                                                    var unwind = !1, tt = new TreeTransformer(function (node) {
                                                        if (unwind) return node;
                                                        var parent = tt.parent();
                                                        if (node instanceof AST_Lambda || node instanceof AST_Try || node instanceof AST_With || node instanceof AST_Case || node instanceof AST_IterationStatement || parent instanceof AST_If && node !== parent.condition || parent instanceof AST_Conditional && node !== parent.condition || node instanceof AST_SymbolRef && value_has_side_effects && !function (def, scope) {
                                                            if (1 === def.orig.length && def.orig[0] instanceof AST_SymbolDefun) return 1;
                                                            if (def.scope !== scope) return;
                                                            for (var refs = def.references, i = 0, len = refs.length; i < len; i++) if (refs[i].scope !== scope) return;
                                                            return 1
                                                        }(node.definition(), self) || parent instanceof AST_Binary && ("&&" == parent.operator || "||" == parent.operator) && node === parent.right || parent instanceof AST_Switch && node !== parent.expression) return side_effects_encountered = unwind = !0, node
                                                    }, function (node) {
                                                        return unwind ? node : node === ref ? (unwind = !0, replace_var(node, tt.parent(), !1)) : (side_effects_encountered |= node.has_side_effects(compressor)) ? (unwind = !0, node) : lvalues_encountered && node instanceof AST_SymbolRef && node.name in lvalues ? (unwind = side_effects_encountered = !0, node) : void 0
                                                    });
                                                    stat.transform(tt)
                                                } else side_effects_encountered |= value_has_side_effects
                                            }
                                        } else side_effects_encountered = !0
                                    }
                                }
                            }
                        }
                        if (var_defs_removed) for (var i = statements.length; 0 <= --i;) 1 < statements.length && statements[i] instanceof AST_EmptyStatement && statements.splice(i, 1);
                        return statements;
                        ...
                        function replace_var(node, parent, is_constant) {
                            ...
                            return var_decl.value = null, var_defs.splice(var_defs_index, 1), 0 === var_defs.length && (statements[prev_stat_index] = make_node(AST_EmptyStatement, self), var_defs_removed = !0), stat.reset_opt_flags(compressor), compressor.info("Collapsing " + (is_constant ? "constant" : "variable") + " " + var_name + " [{file}:{line},{col}]", node.start), CHANGED = !0, value
                        }
                    }
                    ...
                    function process_for_angular(statements) {
                        ...
                        return statements.reduce(function (a, stat) {
                            if (a.push(stat), stat.body && stat.body.args) !function check_expression(body) {
                                ...
                                if (comments && 0 < comments.length) has_inject(comments.pop()) && (stat instanceof AST_Defun ? a.push(make_injector(stat, stat.name)) : stat instanceof AST_Definitions ? stat.definitions.forEach(function (def) {
                    ...
                    function handle_if_return(statements, compressor) {
                        var self = compressor.self(), multiple_if_returns = function (statements) {
                            for (var n = 0, i = statements.length; 0 <= --i;) {
                                var stat = statements[i];
                                if (stat instanceof AST_If && stat.body instanceof AST_Return && 1 < ++n) return !0
                            }
                            return !1
                        }(statements), in_lambda = self instanceof AST_Lambda, ret = [];
                        loop:for (var i = statements.length; 0 <= --i;) {
                            var stat = statements[i];
                            switch (!0) {
                                ...
                                case stat instanceof AST_If:
                                    if (stat.body instanceof AST_Return) {
                                        ...
                                        if (ret[0] instanceof AST_Return && stat.body.value && ret[0].value && !stat.alternative) {
                                            CHANGED = !0, (stat = stat.clone()).alternative = ret[0], ret[0] = stat.transform(compressor);
                                            continue loop
                                        }
                                        if (multiple_if_returns && (0 == ret.length || ret[0] instanceof AST_Return) && stat.body.value && !stat.alternative && in_lambda) {
                                            CHANGED = !0, (stat = stat.clone()).alternative = ret[0] || make_node(AST_Return, stat, {value: null}), ret[0] = stat.transform(compressor);
                                            continue loop
                                        }
                                        if (!stat.body.value && in_lambda) {
                                            CHANGED = !0, (stat = stat.clone()).condition = stat.condition.negate(compressor);
                                            var funs = extract_functions_from_statement_array(body = as_statement_array(stat.alternative).concat(ret));
                                            stat.body = make_node(AST_BlockStatement, stat, {body: body}), stat.alternative = null, ret = funs.concat([stat.transform(compressor)]);
                                            continue loop
                                        }
                                        if (compressor.option("sequences") && 0 < i && statements[i - 1] instanceof AST_If && statements[i - 1].body instanceof AST_Return && 1 == ret.length && in_lambda && ret[0] instanceof AST_SimpleStatement && !stat.alternative) {
                                            CHANGED = !0, ret.push(make_node(AST_Return, ret[0], {value: null}).transform(compressor)), ret.unshift(stat);
                                            continue loop
                                        }
                                    }
                                    var lct = (ab = aborts(stat.body)) instanceof AST_LoopControl ? compressor.loopcontrol_target(ab) : null;
                                    if (ab && (ab instanceof AST_Return && !ab.value && in_lambda || ab instanceof AST_Continue && self === loop_body(lct) || ab instanceof AST_Break && lct instanceof AST_BlockStatement && self === lct)) {
                                        ...
                                        (stat = stat.clone()).condition = stat.condition.negate(compressor), stat.body = make_node(AST_BlockStatement, stat, {body: as_statement_array(stat.alternative).concat(ret)}), stat.alternative = make_node(AST_BlockStatement, stat, {body: body}), ret = [stat.transform(compressor)];
                                        continue loop
                                    }
                                    var ab;
                                    lct = (ab = aborts(stat.alternative)) instanceof AST_LoopControl ? compressor.loopcontrol_target(ab) : null;
                                    if (ab && (ab instanceof AST_Return && !ab.value && in_lambda || ab instanceof AST_Continue && self === loop_body(lct) || ab instanceof AST_Break && lct instanceof AST_BlockStatement && self === lct)) {
                        ...
                        return statements = statements.reduce(function (a, stat) {
                            if (has_quit) extract_declarations_from_unreachable_code(compressor, stat, a); else {
                                if (stat instanceof AST_LoopControl) {
                                    ...
                                    stat instanceof AST_Break && !(lct instanceof AST_IterationStatement) && loop_body(lct) === self || stat instanceof AST_Continue && loop_body(lct) === self ? stat.label && remove(stat.label.thedef.references, stat) : a.push(stat)
                                } else a.push(stat);
                    ...
                    function sequencesize(statements, compressor) {
                        ...
                        function push_seq() {
                            (seq = AST_Seq.from_array(seq)) && ret.push(make_node(AST_SimpleStatement, seq, {body: seq})), seq = []
                        }
                            ...
                            if (stat instanceof AST_SimpleStatement) {
                                (function (a) {
                                    for (var len = 0, i = 0; i < a.length; ++i) {
                                        var stat = a[i];
                                        stat instanceof AST_Seq ? len += stat.len() : len++
                                    }
                                    return len
                                })(seq) >= compressor.sequences_limit && push_seq();
                                var body = stat.body;
                                0 < seq.length && (body = body.drop_side_effect_free(compressor)), body && seq.push(body)
                            } else push_seq(), ret.push(stat)
                        }), push_seq(), ret = function (statements, compressor) {
                            ...
                            return statements.forEach(function (stat) {
                                if (prev) if (stat instanceof AST_For) {
                                    ...
                                    try {
                                        ...
                                        })), !stat.init || stat.init instanceof AST_Definitions ? stat.init || (stat.init = prev.body.drop_side_effect_free(compressor), ret.pop()) : stat.init = cons_seq(stat.init)
                                    } catch (ex) {
                        ...
                        }(ret, compressor), CHANGED = ret.length != statements.length, ret
                    }

                    function join_consecutive_vars(statements) {
                        var prev = null;
                        return statements.reduce(function (a, stat) {
                            return stat instanceof AST_Definitions && prev && prev.TYPE == stat.TYPE ? (prev.definitions = prev.definitions.concat(stat.definitions), CHANGED = !0) : stat instanceof AST_For && prev instanceof AST_Var && (!stat.init || stat.init.TYPE == prev.TYPE) ? (CHANGED = !0, a.pop(), stat.init ? stat.init.definitions = prev.definitions.concat(stat.init.definitions) : stat.init = prev, a.push(stat), prev = stat) : (prev = stat, a.push(stat)), a
                        }, [])
                    }
                }

                function extract_functions_from_statement_array(statements) {
                    for (var funs = [], i = statements.length - 1; 0 <= i; --i) {
                        var stat = statements[i];
                        stat instanceof AST_Defun && (statements.splice(i, 1), funs.unshift(stat))
                    }
                    return funs
                }

                function extract_declarations_from_unreachable_code(compressor, stat, target) {
                    stat instanceof AST_Defun || compressor.warn("Dropping unreachable code [{file}:{line},{col}]", stat.start), stat.walk(new TreeWalker(function (node) {
                        return node instanceof AST_Definitions ? (compressor.warn("Declarations in unreachable code! [{file}:{line},{col}]", node.start), node.remove_initializers(), target.push(node), !0) : node instanceof AST_Defun ? (target.push(node), !0) : node instanceof AST_Scope || void 0
                    }))
                }
                ...
                OPT(AST_Node, function (self, compressor) {
                    ...
                    var self = this, tt = new TreeTransformer(function (node) {
                        if (insert && node instanceof AST_SimpleStatement) return make_node(AST_Return, node, {value: node.body});
                        ...
                        if (node instanceof AST_Lambda && node !== self) return node;
                        if (node instanceof AST_Block) {
                            var index = node.body.length - 1;
                            0 <= index && (node.body[index] = node.body[index].transform(tt))
                        }
                        return node instanceof AST_If && (node.body = node.body.transform(tt), node.alternative && (node.alternative = node.alternative.transform(tt))), node instanceof AST_With && (node.body = node.body.transform(tt)), node
                    });
                    return self.transform(tt)
                }), AST_Node.DEFMETHOD("reset_opt_flags", function (compressor, rescan) {
                    var reduce_vars = rescan && compressor.option("reduce_vars"),
                        toplevel = compressor.option("toplevel"), safe_ids = Object.create(null),
                        suppressor = new TreeWalker(function (node) {
                            if (node instanceof AST_Symbol) {
                                var d = node.definition();
                                node instanceof AST_SymbolRef && d.references.push(node), d.fixed = !1
                            }
                        }), tw = new TreeWalker(function (node, descend) {
                            if (node._squeezed = !1, node._optimized = !1, reduce_vars) {
                                var iife;
                                if (node instanceof AST_Toplevel && node.globals.each(reset_def), node instanceof AST_Scope && node.variables.each(reset_def), node instanceof AST_SymbolRef) if ((d = node.definition()).references.push(node), void 0 === d.fixed || !is_safe(d) || function is_modified(node, level, func) {
                                    var parent = tw.parent(level);
                                    {
                                        if (is_lhs(node, parent) || !func && parent instanceof AST_Call && parent.expression === node) return !0;
                                        if (parent instanceof AST_PropAccess && parent.expression === node) return !func && is_modified(parent, level + 1)
                                    }
                                }(node, 0, node.fixed_value() instanceof AST_Lambda)) d.fixed = !1; else {
                                    var parent = tw.parent();
                                    (parent instanceof AST_Assign && "=" == parent.operator && node === parent.right || parent instanceof AST_Call && node !== parent.expression || parent instanceof AST_Return && node === parent.value && node.scope !== d.scope || parent instanceof AST_VarDef && node === parent.value) && (d.escaped = !0)
                                }
                                if (node instanceof AST_SymbolCatch && (node.definition().fixed = !1), node instanceof AST_VarDef) {
                                    if (null == (d = node.name.definition()).fixed) return node.value ? (d.fixed = function () {
                                        return node.value
                                    }, mark(d, !1), descend()) : d.fixed = null, mark(d, !0), !0;
                                    node.value && (d.fixed = !1)
                                }
                                if (node instanceof AST_Defun) {
                                    var d = node.name.definition();
                                    !toplevel && d.global || is_safe(d) ? d.fixed = !1 : (d.fixed = node, mark(d, !0));
                                    var save_ids = safe_ids;
                                    return safe_ids = Object.create(null), descend(), safe_ids = save_ids, !0
                                }
                                if (node instanceof AST_Function) return push(), !node.name && (iife = tw.parent()) instanceof AST_Call && iife.expression === node && node.argnames.forEach(function (arg, i) {
                                ...
                                if (node instanceof AST_Binary && ("&&" == node.operator || "||" == node.operator)) return node.left.walk(tw), push(), node.right.walk(tw), pop(), !0;
                                if (node instanceof AST_Conditional) return node.condition.walk(tw), push(), node.consequent.walk(tw), pop(), push(), node.alternative.walk(tw), pop(), !0;
                                if (node instanceof AST_If || node instanceof AST_DWLoop) return node.condition.walk(tw), push(), node.body.walk(tw), pop(), node.alternative && (push(), node.alternative.walk(tw), pop()), !0;
                                if (node instanceof AST_LabeledStatement) return push(), node.body.walk(tw), pop(), !0;
                                if (node instanceof AST_For) return node.init && node.init.walk(tw), push(), node.condition && node.condition.walk(tw), node.body.walk(tw), node.step && node.step.walk(tw), pop(), !0;
                                if (node instanceof AST_ForIn) return node.init.walk(suppressor), node.object.walk(tw), push(), node.body.walk(tw), pop(), !0;
                                if (node instanceof AST_Try) return push(), walk_body(node, tw), pop(), node.bcatch && (push(), node.bcatch.walk(tw), pop()), node.bfinally && node.bfinally.walk(tw), !0;
                                if (node instanceof AST_SwitchBranch) return push(), descend(), pop(), !0
                            }
                        });
                    ...
                    function is_safe(def) {
                        if (safe_ids[def.id]) {
                            if (null == def.fixed) {
                                var orig = def.orig[0];
                                if (orig instanceof AST_SymbolFunarg || "arguments" == orig.name) return;
                    ...
                    function reset_def(def) {
                        def.escaped = !1, !def.scope.uses_eval && (toplevel || !def.global || def.orig[0] instanceof AST_SymbolConst) ? def.fixed = void 0 : def.fixed = !1, def.references = [], def.should_replace = void 0
                    }
                ...
                }), def(AST_Node, is_strict), def(AST_Null, return_true), def(AST_Undefined, return_true), def(AST_Constant, return_false), def(AST_Array, return_false), def(AST_Object, function (pure_getters) {
                    if (!is_strict(pure_getters)) return !1;
                    for (var i = this.properties.length; 0 <= --i;) if (this.properties[i].value instanceof AST_Accessor) return !0;
                    return !1
                }), def(AST_Function, return_false), def(AST_UnaryPostfix, return_false), def(AST_UnaryPrefix, function () {
                    ...
                    def(AST_Node, return_false), def(AST_UnaryPrefix, function () {
                    ...
                    }), def(AST_True, return_true), def(AST_False, return_true)
                }(function (node, func) {
                    ...
                    def(AST_Node, return_false), def(AST_String, return_true), def(AST_UnaryPrefix, function () {
                ...
                !function (def) {
                    AST_Node.DEFMETHOD("resolve_defines", function (compressor) {
                        if (compressor.option("global_defs")) {
                            ...
                            if (def) {
                                for (var node, parent = this, level = 0; node = parent, (parent = compressor.parent(level++)) instanceof AST_PropAccess && parent.expression === node;) ;
                            ...
                            if (defines && HOP(defines, name = this.name + suffix)) {
                                var node = function to_node(value, orig) {
                                    ...
                                    if (value && "object" == typeof value) {
                                        var props = [];
                                        for (var key in value) props.push(make_node(AST_ObjectKeyVal, orig, {
                                            key: key,
                    ...
                    AST_Node.DEFMETHOD("is_constant", function () {
                        ...
                        if (this instanceof AST_Constant && !(this instanceof AST_RegExp)) return this.value;
                        ...
                        if (compressor.option("unsafe")) return this.elements.map(function (element) {
                        ...
                        if (compressor.option("unsafe")) {
                            for (var val = {}, i = 0, len = this.properties.length; i < len; i++) {
                                var prop = this.properties[i], key = prop.key;
                                if (key instanceof AST_Symbol ? key = key.name : key instanceof AST_Node && (key = ev(key, compressor)), "function" == typeof Object.prototype[key]) throw def;
                        ...
                        switch (this.operator) {
                            ...
                            case"typeof":
                                ...
                                if ((e = ev(e, compressor)) instanceof RegExp) throw def;
                                return typeof e;
                            case"void":
                        ...
                        var result, left = this.left, right = this.right;
                        switch (this.operator) {
                            case"&&":
                        ...
                        if (isNaN(result) && c.find_parent(AST_With)) throw def;
                        return result
                    }), def(AST_Conditional, function (compressor) {
                        ...
                        try {
                            ...
                            if (HOP(fixed, "_eval") || (fixed._eval = function () {
                                return value
                            }), value && "object" == typeof value && this.definition().escaped) throw def;
                            return value
                        } finally {
                            this._evaluating = !1
                        }
                        ...
                        if (compressor.option("unsafe")) {
                            ...
                            key instanceof AST_Node && (key = ev(key, compressor));
                            var val = ev(this.expression, compressor);
                            if (val && HOP(val, key)) return val[key]
                        }
                        throw def
                    })
                }(function (node, func) {
                    ...
                    def(AST_Node, function () {
                        ...
                        var self = this.clone();
                        return self.consequent = self.consequent.negate(compressor), self.alternative = self.alternative.negate(compressor), best(this, self, first_in_statement)
                        ...
                        switch (op) {
                            ...
                            case"!==":
                                return self.operator = "===", self;
                            case"&&":
                                return self.operator = "||", self.left = self.left.negate(compressor, first_in_statement), self.right = self.right.negate(compressor), best(this, self, first_in_statement);
                            case"||":
                                return self.operator = "&&", self.left = self.left.negate(compressor, first_in_statement), self.right = self.right.negate(compressor), best(this, self, first_in_statement)
                        }
                        return basic_negation(this)
                    ...
                    if (!compressor.option("side_effects")) return !1;
                    if (void 0 !== this.pure) return this.pure;
                    var comments, last_comment, pure = !1;
                    return this.start && (comments = this.start.comments_before) && comments.length && /[@#]__PURE__/.test((last_comment = comments[comments.length - 1]).value) && (pure = last_comment), this.pure = pure
                }), function (def) {
                    function any(list, compressor) {
                        for (var i = list.length; 0 <= --i;) if (list[i].has_side_effects(compressor)) return !0;
                        return !1
                    }

                    def(AST_Node, return_true), def(AST_EmptyStatement, return_false), def(AST_Constant, return_false), def(AST_This, return_false), def(AST_Call, function (compressor) {
                        if (!this.has_pure_annotation(compressor) && compressor.pure_funcs(this)) return !0;
                        ...
                        return this.body.has_side_effects(compressor)
                    }), def(AST_Defun, return_true), def(AST_Function, return_false), def(AST_Binary, function (compressor) {
                    ...
                    function block_aborts() {
                        ...
                        return 0 < n && aborts(this.body[n - 1])
                    }

                    def(AST_Statement, return_null), def(AST_Jump, return_this), def(AST_BlockStatement, block_aborts), def(AST_SwitchBranch, block_aborts), def(AST_If, function () {
                    ...
                    if (compressor.option("unused") && (!(self instanceof AST_Toplevel) || toplevel) && !self.uses_eval && !self.uses_with) {
                        var assign_as_unused = !/keep_assign/.test(compressor.option("unused")),
                            drop_funcs = /funcs/.test(toplevel), drop_vars = /vars/.test(toplevel);
                        self instanceof AST_Toplevel && 1 != toplevel || (drop_funcs = drop_vars = !0);
                        var in_use = [], in_use_ids = Object.create(null);
                        self instanceof AST_Toplevel && compressor.top_retain && self.variables.each(function (def) {
                            !compressor.top_retain(def) || def.id in in_use_ids || (in_use_ids[def.id] = !0, in_use.push(def))
                        });
                        var initializations = new Dictionary, scope = this,
                            tw = new TreeWalker(function (node, descend) {
                                if (node !== self) {
                                    if (node instanceof AST_Defun) {
                                        if (!drop_funcs && scope === self) (node_def = node.name.definition()).id in in_use_ids || (in_use_ids[node_def.id] = !0, in_use.push(node_def));
                                        return initializations.add(node.name.name, node), !0
                                    }
                                    if (node instanceof AST_Definitions && scope === self) return node.definitions.forEach(function (def) {
                                        if (!drop_vars) {
                                            var node_def = def.name.definition();
                                            node_def.id in in_use_ids || (in_use_ids[node_def.id] = !0, in_use.push(node_def))
                                        }
                                        def.value && (initializations.add(def.name.name, def.value), def.value.has_side_effects(compressor) && def.value.walk(tw))
                                    }), !0;
                                    if (assign_as_unused && node instanceof AST_Assign && "=" == node.operator && node.left instanceof AST_SymbolRef && !is_reference_const(node.left) && scope === self) return node.right.walk(tw), !0;
                                    var node_def;
                                    if (node instanceof AST_SymbolRef) return (node_def = node.definition()).id in in_use_ids || (in_use_ids[node_def.id] = !0, in_use.push(node_def)), !0;
                                    if (node instanceof AST_Scope) {
                                        var save_scope = scope;
                        ...
                        var tt = new TreeTransformer(function (node, descend, in_list) {
                            node instanceof AST_Function && node.name && !compressor.option("keep_fnames") && ((def = node.name.definition()).id in in_use_ids && !(1 < def.orig.length) || (node.name = null));
                            if (node instanceof AST_Lambda && !(node instanceof AST_Accessor)) for (var trim = !compressor.option("keep_fargs"), a = node.argnames, i = a.length; 0 <= --i;) {
                                var sym = a[i];
                                sym.definition().id in in_use_ids ? trim = !1 : (sym.__unused = !0, trim && (a.pop(), compressor[sym.unreferenced() ? "warn" : "info"]("Dropping unused function argument {name} [{file}:{line},{col}]", {
                                    name: sym.name,
                                    file: sym.start.file,
                                    line: sym.start.line,
                                    col: sym.start.col
                            ...
                            if (drop_funcs && node instanceof AST_Defun && node !== self) return node.name.definition().id in in_use_ids ? node : (compressor[node.name.unreferenced() ? "warn" : "info"]("Dropping unused function {name} [{file}:{line},{col}]", {
                                name: node.name.name,
                            ...
                            if (drop_vars && node instanceof AST_Definitions && !(tt.parent() instanceof AST_ForIn && tt.parent().init === node)) {
                                var def;
                                def = mergeSort(def = node.definitions.filter(function (def) {
                                    def.value && (def.value = def.value.transform(tt));
                                    var sym = def.name.definition();
                                    if (sym.id in in_use_ids) return !0;
                                    if (sym.orig[0] instanceof AST_SymbolCatch) return def.value = def.value && def.value.drop_side_effect_free(compressor), !0;
                                    var w = {
                                        name: def.name.name,
                                    ...
                                    return def.value && (def._unused_side_effects = def.value.drop_side_effect_free(compressor)) ? (compressor.warn("Side effects in initialization of unused variable {name} [{file}:{line},{col}]", w), !0) : (compressor[def.name.unreferenced() ? "warn" : "info"]("Dropping unused variable {name} [{file}:{line},{col}]", w), !1)
                                }), function (a, b) {
                                    return !a.value && b.value ? -1 : !b.value && a.value ? 1 : 0
                                });
                                var side_effects = [];
                                for (i = 0; i < def.length;) {
                                    var x = def[i];
                                    x._unused_side_effects ? (side_effects.push(x._unused_side_effects), def.splice(i, 1)) : (0 < side_effects.length && (side_effects.push(x.value), x.value = AST_Seq.from_array(side_effects), side_effects = []), ++i)
                                }
                                return (side_effects = 0 < side_effects.length ? make_node(AST_BlockStatement, node, {body: [make_node(AST_SimpleStatement, node, {body: AST_Seq.from_array(side_effects)})]}) : null, 0 != def.length || side_effects) ? 0 == def.length ? in_list ? MAP.splice(side_effects.body) : side_effects : (node.definitions = def, side_effects ? (side_effects.body.unshift(node), in_list ? MAP.splice(side_effects.body) : side_effects) : node) : make_node(AST_EmptyStatement, node)
                            }
                            if (drop_vars && assign_as_unused && node instanceof AST_Assign && "=" == node.operator && node.left instanceof AST_SymbolRef && (!((def = node.left.definition()).id in in_use_ids) && self.variables.get(def.name) === def)) return maintain_this_binding(tt.parent(), node, node.right.transform(tt));
                            if (node instanceof AST_For) {
                                if (descend(node, this), node.init instanceof AST_BlockStatement) {
                                    var block = node.init;
                                    return node.init = block.body.pop(), block.body.push(node), in_list ? MAP.splice(block.body) : block
                                }
                                return is_empty(node.init) && (node.init = null), node
                            }
                    ...
                    if (hoist_funs || hoist_vars) {
                        ...
                        self.walk(new TreeWalker(function (node) {
                            return node instanceof AST_Scope && node !== self || (node instanceof AST_Var ? (++var_decl, !0) : void 0)
                        })), hoist_vars = hoist_vars && 1 < var_decl;
                            ...
                            if (node !== self) {
                                ...
                                if (node instanceof AST_Var && hoist_vars) {
                                    ...
                                    if (p instanceof AST_ForIn && p.init === node) {
                                        if (null != seq) return seq;
                                        var def = node.definitions[0].name;
                                        return make_node(AST_SymbolRef, def, def)
                                    }
                                    return p instanceof AST_For && p.init === node ? seq : seq ? make_node(AST_SimpleStatement, node, {body: seq}) : make_node(AST_EmptyStatement, node)
                                }
                                if (node instanceof AST_Scope) return node
                            }
                        });
                        if (self = self.transform(tt), 0 < vars_found) {
                            var defs = [];
                            if (vars.each(function (def, name) {
                                self instanceof AST_Lambda && find_if(function (x) {
                                    return x.name == def.name.name
                                }, self.argnames) ? vars.del(name) : ((def = def.clone()).value = null, defs.push(def), vars.set(name, def))
                            }), 0 < defs.length) {
                    ...
                    function trim(nodes, compressor, first_in_statement) {
                        for (var ret = [], changed = !1, i = 0, len = nodes.length; i < len; i++) {
                    ...
                    def(AST_Node, return_this), def(AST_Constant, return_null), def(AST_This, return_null), def(AST_Call, function (compressor, first_in_statement) {
                        if (!this.has_pure_annotation(compressor) && compressor.pure_funcs(this)) {
                            if (!(this.expression instanceof AST_Function) || this.expression.name && this.expression.name.definition().references.length) return this;
                            var node = this.clone();
                            return node.expression = node.expression.process_expression(!1, compressor), node
                        }
                        this.pure && (compressor.warn("Dropping __PURE__ call [{file}:{line},{col}]", this.start), this.pure.value = this.pure.value.replace(/[@#]__PURE__/g, " "));
                        var args = trim(this.args, compressor, first_in_statement);
                        return args && AST_Seq.from_array(args)
                    }), def(AST_Accessor, return_null), def(AST_Function, return_null), def(AST_Binary, function (compressor, first_in_statement) {
                        ...
                        switch (this.operator) {
                            ...
                            default:
                                ...
                                return left ? make_node(AST_Seq, this, {
                                    car: left,
                                    cdr: right
                                }) : this.right.drop_side_effect_free(compressor, first_in_statement)
                        }
                    }), def(AST_Assign, return_this), def(AST_Conditional, function (compressor) {
                        var consequent = this.consequent.drop_side_effect_free(compressor),
                            alternative = this.alternative.drop_side_effect_free(compressor);
                        ...
                        return first_in_statement && this instanceof AST_UnaryPrefix && is_iife_call(expression) ? expression === this.expression && 1 === this.operator.length ? this : make_node(AST_UnaryPrefix, this, {
                        ...
                        return property ? make_node(AST_Seq, this, {car: expression, cdr: property}) : expression
                    }), def(AST_Seq, function (compressor) {
                        var cdr = this.cdr.drop_side_effect_free(compressor);
                        return cdr === this.cdr ? this : cdr ? make_node(AST_Seq, this, {
                            car: this.car,
                            cdr: cdr
                        }) : this.car
                    })
                    ...
                    if (compressor.option("side_effects")) {
                        ...
                        if (!node) return compressor.warn("Dropping side-effect-free statement [{file}:{line},{col}]", self.start), make_node(AST_EmptyStatement, self);
                        if (node !== body) return make_node(AST_SimpleStatement, self, {body: node})
                    }
                    return self
                    ...
                    if (cond !== self.condition) {
                        if (cond) return make_node(AST_For, self, {body: self.body});
                        if (compressor.option("dead_code") && self instanceof AST_While) {
                            var a = [];
                            return extract_declarations_from_unreachable_code(compressor, self.body, a), make_node(AST_BlockStatement, self, {body: a}).optimize(compressor)
                        }
                        if (self instanceof AST_Do) {
                            ...
                            if ((parent instanceof AST_LabeledStatement ? parent : self).walk(tw), !has_loop_control) return self.body
                        }
                    }
                    return self instanceof AST_While ? make_node(AST_For, self, self).optimize(compressor) : self
                    ...
                    if (self.condition) {
                        ...
                        if (compressor.option("dead_code") && !cond) {
                            var a = [];
                            return self.init instanceof AST_Statement ? a.push(self.init) : self.init && a.push(make_node(AST_SimpleStatement, self.init, {body: self.init})), extract_declarations_from_unreachable_code(compressor, self.body, a), make_node(AST_BlockStatement, self, {body: a}).optimize(compressor)
                        }
                        cond !== self.condition && (cond = make_node_from_constant(cond, self.condition).transform(compressor), self.condition = best_of_expression(cond, self.condition))
                    }
                    return function if_break_in_loop(self, compressor) {
                        function drop_it(rest) {
                            rest = as_statement_array(rest), self.body instanceof AST_BlockStatement ? (self.body = self.body.clone(), self.body.body = rest.concat(self.body.body.slice(1)), self.body = self.body.transform(compressor)) : self.body = make_node(AST_BlockStatement, self.body, {body: rest}).transform(compressor), if_break_in_loop(self, compressor)
                        }

                        var first = self.body instanceof AST_BlockStatement ? self.body.body[0] : self.body;
                        first instanceof AST_If && (first.body instanceof AST_Break && compressor.loopcontrol_target(first.body) === compressor.self() ? (self.condition ? self.condition = make_node(AST_Binary, self.condition, {
                            left: self.condition,
                            operator: "&&",
                            right: first.condition.negate(compressor)
                        }) : self.condition = first.condition.negate(compressor), drop_it(first.alternative)) : first.alternative instanceof AST_Break && compressor.loopcontrol_target(first.alternative) === compressor.self() && (self.condition ? self.condition = make_node(AST_Binary, self.condition, {
                            left: self.condition,
                            operator: "&&",
                            right: first.condition
                        }) : self.condition = first.condition, drop_it(first.body)))
                    ...
                    if (cond !== self.condition) {
                        if (cond) {
                            if (compressor.warn("Condition always true [{file}:{line},{col}]", self.condition.start), compressor.option("dead_code")) {
                                var a = [];
                                return self.alternative && extract_declarations_from_unreachable_code(compressor, self.alternative, a), a.push(self.body), make_node(AST_BlockStatement, self, {body: a}).optimize(compressor)
                            }
                        } else if (compressor.warn("Condition always false [{file}:{line},{col}]", self.condition.start), compressor.option("dead_code")) {
                            a = [];
                            return extract_declarations_from_unreachable_code(compressor, self.body, a), self.alternative && a.push(self.alternative), make_node(AST_BlockStatement, self, {body: a}).optimize(compressor)
                        }
                        cond = make_node_from_constant(cond, self.condition).transform(compressor), self.condition = best_of_expression(cond, self.condition)
                    }
                    var negated = self.condition.negate(compressor),
                        self_condition_length = self.condition.print_to_string().length,
                        negated_length = negated.print_to_string().length,
                        negated_is_best = negated_length < self_condition_length;
                    ...
                    if (is_empty(self.body) && is_empty(self.alternative)) return make_node(AST_SimpleStatement, self.condition, {body: self.condition.clone()}).optimize(compressor);
                    ...
                    if (is_empty(self.alternative) && self.body instanceof AST_SimpleStatement) return self_condition_length === negated_length && !negated_is_best && self.condition instanceof AST_Binary && "||" == self.condition.operator && (negated_is_best = !0), negated_is_best ? make_node(AST_SimpleStatement, self, {
                    ...
                    if (self.body instanceof AST_If && !self.body.alternative && !self.alternative && (self = make_node(AST_If, self, {
                        ...
                        }), body: self.body.body, alternative: null
                    })), aborts(self.body) && self.alternative) {
                        var alt = self.alternative;
                        return self.alternative = null, make_node(AST_BlockStatement, self, {body: [self, alt]}).optimize(compressor)
                    }
                    if (aborts(self.alternative)) {
                        var body = self.body;
                        return self.body = self.alternative, self.condition = negated_is_best ? negated : self.condition.negate(compressor), self.alternative = null, make_node(AST_BlockStatement, self, {body: [self, body]}).optimize(compressor)
                    }
                    return self
                    ...
                    for (var default_branch, exact_match, decl = [], body = [], i = 0, len = self.body.length; i < len && !exact_match; i++) {
                        if ((branch = self.body[i]) instanceof AST_Default) default_branch ? eliminate_branch(branch, body[body.length - 1]) : default_branch = branch; else if (value !== self.expression) {
                            var exp = branch.expression.evaluate(compressor);
                            if (exp === value) {
                                if (exact_match = branch, default_branch) {
                    ...
                    for (0 < body.length && (body[0].body = decl.concat(body[0].body)), self.body = body; branch = body[body.length - 1];) {
                        var stat = branch.body[branch.body.length - 1];
                        if (stat instanceof AST_Break && compressor.loopcontrol_target(stat) === self && branch.body.pop(), branch.body.length || branch instanceof AST_Case && (default_branch || branch.expression.has_side_effects(compressor))) break;
                        body.pop() === default_branch && (default_branch = null)
                    }
                    if (0 == body.length) return make_node(AST_BlockStatement, self, {body: decl.concat(make_node(AST_SimpleStatement, self.expression, {body: self.expression}))}).optimize(compressor);
                    if (1 == body.length && (body[0] === exact_match || body[0] === default_branch)) {
                        ...
                        if (self.walk(tw), !has_break) return (body = body[0].body.slice()).unshift(make_node(AST_SimpleStatement, self.expression, {body: self.expression})), make_node(AST_BlockStatement, self, {body: body}).optimize(compressor)
                    }
                    return self;
                    ...
                    if (self.body = tighten_body(self.body, compressor), self.bcatch && self.bfinally && all(self.bfinally.body, is_empty) && (self.bfinally = null), all(self.body, is_empty)) {
                        var body = [];
                        return self.bcatch && extract_declarations_from_unreachable_code(compressor, self.bcatch, body), self.bfinally && (body = body.concat(self.bfinally.body)), make_node(AST_BlockStatement, self, {body: body}).optimize(compressor)
                    }
                    return self
                    ...
                    var reduce_vars = compressor.option("reduce_vars"),
                        assignments = this.definitions.reduce(function (a, def) {
                            if (def.value) {
                    ...
                    if (compressor.option("reduce_vars") && exp instanceof AST_SymbolRef) {
                        var def = exp.definition(), fixed = exp.fixed_value();
                        fixed instanceof AST_Defun && (def.fixed = fixed = make_node(AST_Function, fixed, fixed).clone(!0)), fixed instanceof AST_Function && (exp = fixed, !compressor.option("unused") || 1 != def.references.length || def.scope.uses_arguments && def.orig[0] instanceof AST_SymbolFunarg || def.scope.uses_eval || compressor.find_parent(AST_Scope) !== def.scope || (self.expression = exp))
                    }
                    ...
                    if (compressor.option("unsafe")) if (exp instanceof AST_SymbolRef && exp.undeclared()) switch (exp.name) {
                        case"Array":
                            if (1 != self.args.length) return make_node(AST_Array, self, {elements: self.args}).optimize(compressor);
                            break;
                        case"Object":
                            if (0 == self.args.length) return make_node(AST_Object, self, {properties: []});
                            break;
                        case"String":
                            if (0 == self.args.length) return make_node(AST_String, self, {value: ""});
                            ...
                            break;
                        case"Number":
                            if (0 == self.args.length) return make_node(AST_Number, self, {value: 0});
                        ...
                        case"Boolean":
                            ...
                            break;
                        case"Function":
                            if (0 == self.args.length) return make_node(AST_Function, self, {argnames: [], body: []});
                            if (all(self.args, function (x) {
                                return x instanceof AST_String
                            })) try {
                                var ast = parse(code = "(function(" + self.args.slice(0, -1).map(function (arg) {
                                    return arg.value
                                }).join(",") + "){" + self.args[self.args.length - 1].value + "})()");
                                ast.figure_out_scope({screw_ie8: compressor.option("screw_ie8")});
                                var fun, comp = new Compressor(compressor.options);
                                (ast = ast.transform(comp)).figure_out_scope({screw_ie8: compressor.option("screw_ie8")}), ast.mangle_names();
                                try {
                                ...
                                return AST_BlockStatement.prototype._codegen.call(fun, fun, code), code = code.toString().replace(/^\{|\}$/g, ""), args.push(make_node(AST_String, self.args[self.args.length - 1], {value: code})), self.args = args, self
                            } catch (ex) {
                        ...
                        if (exp instanceof AST_Dot && exp.expression instanceof AST_Array && "join" == exp.property) {
                            if (!(0 < self.args.length && (separator = self.args[0].evaluate(compressor)) === self.args[0])) {
                                var first, node, elements = [], consts = [];
                                return (exp.expression.elements.forEach(function (el) {
                                    var value = el.evaluate(compressor);
                                    value !== el ? consts.push(value) : (0 < consts.length && (elements.push(make_node(AST_String, self, {value: consts.join(separator)})), consts.length = 0), elements.push(el))
                                }), 0 < consts.length && elements.push(make_node(AST_String, self, {value: consts.join(separator)})), 0 == elements.length) ? make_node(AST_String, self, {value: ""}) : 1 == elements.length ? elements[0].is_string(compressor) ? elements[0] : make_node(AST_Binary, elements[0], {
                                    operator: "+",
                                    left: make_node(AST_String, self, {value: ""}),
                                    right: elements[0]
                                }) : "" == separator ? (first = elements[0].is_string(compressor) || elements[1].is_string(compressor) ? elements.shift() : make_node(AST_String, self, {value: ""}), elements.reduce(function (prev, el) {
                                    return make_node(AST_Binary, el, {operator: "+", left: prev, right: el})
                                }, first).optimize(compressor)) : ((node = self.clone()).expression = node.expression.clone(), node.expression.expression = node.expression.expression.clone(), node.expression.expression.elements = elements, best_of(compressor, self, node))
                            }
                        } else if (exp instanceof AST_Dot && exp.expression.is_string(compressor) && "charAt" == exp.property) {
                    ...
                    if (exp instanceof AST_Function) {
                        if (exp.body[0] instanceof AST_Return) {
                            ...
                            if (!value || value.is_constant()) {
                                args = self.args.concat(value || make_node(AST_Undefined, self));
                    ...
                    if (compressor.option("drop_console") && exp instanceof AST_PropAccess) {
                        ...
                        if (name instanceof AST_SymbolRef && "console" == name.name && name.undeclared()) return make_node(AST_Undefined, self).optimize(compressor)
                    }
                    return compressor.option("negate_iife") && compressor.parent() instanceof AST_SimpleStatement && is_iife_call(self) ? self.negate(compressor, !0) : self
                    ...
                    if (self.car = self.car.drop_side_effect_free(compressor, first_in_statement(compressor)), !self.car) return maintain_this_binding(compressor.parent(), self, self.cdr);
                    var left;
                    if (compressor.option("cascade") && (self.car instanceof AST_Assign && !self.car.left.has_side_effects(compressor) ? left = self.car.left : self.car instanceof AST_Unary && ("++" == self.car.operator || "--" == self.car.operator) && (left = self.car.expression), left && !(left instanceof AST_SymbolRef && (left.definition().orig[0] instanceof AST_SymbolLambda || is_reference_const(left))))) for (var parent, field, cdr = self.cdr; ;) {
                        if (cdr.equivalent_to(left)) {
                            ...
                            return parent ? (parent[field] = car, self.cdr) : car
                        }
                        if (cdr instanceof AST_Binary && !(cdr instanceof AST_Assign)) if (cdr.left.is_constant()) {
                            if ("||" == cdr.operator || "&&" == cdr.operator) break;
                            field = "right"
                        } else field = "left"; else {
                            if (!(cdr instanceof AST_Call || cdr instanceof AST_Unary && !unary_side_effects(cdr.operator))) break;
                            field = "expression"
                        }
                        cdr = (parent = cdr)[field]
                    }
                    return is_undefined(self.cdr, compressor) ? make_node(AST_UnaryPrefix, self, {
                        operator: "void",
                        expression: self.car
                    }) : self
                }), AST_Unary.DEFMETHOD("lift_sequences", function (compressor) {
                    if (compressor.option("sequences") && this.expression instanceof AST_Seq) {
                        var seq = this.expression, x = seq.to_array(), e = this.clone();
                        return e.expression = x.pop(), x.push(e), seq = AST_Seq.from_array(x).transform(compressor)
                    }
                    return this
                    ...
                    if ("delete" == self.operator && !(e instanceof AST_SymbolRef || e instanceof AST_PropAccess || e instanceof AST_NaN || e instanceof AST_Infinity || e instanceof AST_Undefined)) return e instanceof AST_Seq ? ((e = e.to_array()).push(make_node(AST_True, self)), AST_Seq.from_array(e).optimize(compressor)) : make_node(AST_Seq, self, {
                        car: e,
                        cdr: make_node(AST_True, self)
                    }).optimize(compressor);
                    var seq = self.lift_sequences(compressor);
                    if (seq !== self) return seq;
                    if (compressor.option("side_effects") && "void" == self.operator) return (e = e.drop_side_effect_free(compressor)) ? (self.expression = e, self) : make_node(AST_Undefined, self).optimize(compressor);
                    if (compressor.option("booleans") && compressor.in_boolean_context()) switch (self.operator) {
                        case"!":
                            ...
                            break;
                        case"typeof":
                            return compressor.warn("Boolean expression always true [{file}:{line},{col}]", self.start), (e instanceof AST_SymbolRef ? make_node(AST_True, self) : make_node(AST_Seq, self, {
                                car: e,
                                cdr: make_node(AST_True, self)
                            })).optimize(compressor)
                    }
                    if ("-" == self.operator && e instanceof AST_Infinity && (e = e.transform(compressor)), e instanceof AST_Binary && ("+" == self.operator || "-" == self.operator) && ("*" == e.operator || "/" == e.operator || "%" == e.operator)) return make_node(AST_Binary, self, {
                        operator: e.operator,
                        left: make_node(AST_UnaryPrefix, e.left, {operator: self.operator, expression: e.left}),
                        right: e.right
                    });
                    ...
                    if (compressor.option("sequences")) {
                        if (this.left instanceof AST_Seq) {
                            var x = (seq = this.left).to_array();
                            return (e = this.clone()).left = x.pop(), x.push(e), AST_Seq.from_array(x).optimize(compressor)
                        }
                        if (this.right instanceof AST_Seq && !this.left.has_side_effects(compressor)) {
                            for (var cursor, e, assign = "=" == this.operator && this.left instanceof AST_SymbolRef, root = this.right.clone(), seq = root; (assign || !seq.car.has_side_effects(compressor)) && (cursor = seq).cdr instanceof AST_Seq;) seq = seq.cdr = seq.cdr.clone();
                            if (cursor) return (e = this.clone()).right = cursor.cdr, cursor.cdr = e, root.optimize(compressor)
                        }
                    }
                    return this
                });
                ...
                OPT(AST_Binary, function (self, compressor) {
                    ...
                    if (commutativeOperators(self.operator) && self.right.is_constant() && !self.left.is_constant() && (self.left instanceof AST_Binary && PRECEDENCE[self.left.operator] >= PRECEDENCE[self.operator] || reverse()), self = self.lift_sequences(compressor), compressor.option("comparisons")) switch (self.operator) {
                        case"===":
                        case"!==":
                            (self.left.is_string(compressor) && self.right.is_string(compressor) || self.left.is_number(compressor) && self.right.is_number(compressor) || self.left.is_boolean() && self.right.is_boolean()) && (self.operator = self.operator.substr(0, 2));
                        case"==":
                        case"!=":
                            if (self.left instanceof AST_String && "undefined" == self.left.value && self.right instanceof AST_UnaryPrefix && "typeof" == self.right.operator) {
                                var expr = self.right.expression;
                                (expr instanceof AST_SymbolRef ? expr.undeclared() : expr instanceof AST_PropAccess && !compressor.option("screw_ie8")) || (self.right = expr, self.left = make_node(AST_Undefined, self.left).optimize(compressor), 2 == self.operator.length && (self.operator += "="))
                            }
                    }
                    if (compressor.option("booleans") && "+" == self.operator && compressor.in_boolean_context()) {
                        ...
                        if (ll && "string" == typeof ll) return compressor.warn("+ in boolean context always true [{file}:{line},{col}]", self.start), make_node(AST_Seq, self, {
                            car: self.right,
                            cdr: make_node(AST_True, self)
                        }).optimize(compressor);
                        if (rr && "string" == typeof rr) return compressor.warn("+ in boolean context always true [{file}:{line},{col}]", self.start), make_node(AST_Seq, self, {
                            car: self.left,
                            cdr: make_node(AST_True, self)
                        }).optimize(compressor)
                    }
                    ...
                    if ("+" == self.operator) {
                        ...
                        if (self.left instanceof AST_Binary && "+" == self.left.operator && self.left.left instanceof AST_String && "" == self.left.left.getValue() && self.right.is_string(compressor)) return self.left = self.left.right, self.transform(compressor)
                    }
                    if (compressor.option("evaluate")) {
                        switch (self.operator) {
                            case"&&":
                                if (!(ll = self.left.evaluate(compressor))) return compressor.warn("Condition left of && always false [{file}:{line},{col}]", self.start), maintain_this_binding(compressor.parent(), self, self.left).optimize(compressor);
                                if (ll !== self.left) return compressor.warn("Condition left of && always true [{file}:{line},{col}]", self.start), maintain_this_binding(compressor.parent(), self, self.right).optimize(compressor);
                                if (compressor.option("booleans") && compressor.in_boolean_context()) {
                                    if (!(rr = self.right.evaluate(compressor))) return compressor.warn("Boolean && always false [{file}:{line},{col}]", self.start), make_node(AST_Seq, self, {
                                        car: self.left,
                                        cdr: make_node(AST_False, self)
                                    }).optimize(compressor);
                                    if (rr !== self.right) return compressor.warn("Dropping side-effect-free && in boolean context [{file}:{line},{col}]", self.start), self.left.optimize(compressor)
                                }
                                break;
                            case"||":
                                if (!(ll = self.left.evaluate(compressor))) return compressor.warn("Condition left of || always false [{file}:{line},{col}]", self.start), maintain_this_binding(compressor.parent(), self, self.right).optimize(compressor);
                                if (ll !== self.left) return compressor.warn("Condition left of || always true [{file}:{line},{col}]", self.start), maintain_this_binding(compressor.parent(), self, self.left).optimize(compressor);
                                if (compressor.option("booleans") && compressor.in_boolean_context()) {
                                    if (!(rr = self.right.evaluate(compressor))) return compressor.warn("Dropping side-effect-free || in boolean context [{file}:{line},{col}]", self.start), self.left.optimize(compressor);
                                    if (rr !== self.right) return compressor.warn("Boolean || always true [{file}:{line},{col}]", self.start), make_node(AST_Seq, self, {
                                        car: self.left,
                                        cdr: make_node(AST_True, self)
                                    }).optimize(compressor)
                                }
                    ...
                    if (self.right instanceof AST_Binary && self.right.operator == self.operator && ("&&" == self.operator || "||" == self.operator || "+" == self.operator && (self.right.left.is_string(compressor) || self.left.is_string(compressor) && self.right.right.is_string(compressor)))) return self.left = make_node(AST_Binary, self.left, {
                        operator: self.operator,
                        left: self.left,
                        right: self.right.left
                    }), self.right = self.right.right, self.transform(compressor);
                    var ev = self.evaluate(compressor);
                    return ev !== self ? (ev = make_node_from_constant(ev, self).optimize(compressor), best_of(compressor, ev, self)) : self
                }), OPT(AST_SymbolRef, function (self, compressor) {
                    var def = self.resolve_defines(compressor);
                    if (def) return def.optimize(compressor);
                    if (compressor.option("screw_ie8") && self.undeclared() && (!self.scope.uses_with || !compressor.find_parent(AST_With))) switch (self.name) {
                        case"undefined":
                    ...
                    if (compressor.option("evaluate") && compressor.option("reduce_vars") && is_lhs(self, compressor.parent()) !== self) {
                        var d = self.definition(), fixed = self.fixed_value();
                        if (fixed) {
                            if (void 0 === d.should_replace) {
                                var init = fixed.evaluate(compressor);
                                if (init === fixed || !compressor.option("unsafe_regexp") && init instanceof RegExp) d.should_replace = !1; else {
                                    var fn,
                                        value = (init = make_node_from_constant(init, fixed)).optimize(compressor).print_to_string().length;
                                    fn = function (value) {
                                        var found;
                                    ...
                                    } : (value = Math.min(value, fixed.print_to_string().length), function () {
                                        var result = best_of_expression(init.optimize(compressor), fixed);
                                        return result === init || result === fixed ? result.clone(!0) : result
                                    });
                                    var name = d.name.length, overhead = 0;
                                    !compressor.option("unused") || d.global && !compressor.option("toplevel") || (overhead = (name + 2 + value) / d.references.length), d.should_replace = value <= name + overhead && fn
                                }
                            }
                    ...
                    var lhs = is_lhs(compressor.self(), compressor.parent());
                    return (!lhs || !is_atomic(lhs, self)) && (!compressor.option("keep_infinity") || lhs && !is_atomic(lhs, self) || find_variable(compressor, "Infinity")) ? make_node(AST_Binary, self, {
                        operator: "/",
                ...
                function literals_in_boolean_context(self, compressor) {
                    return compressor.option("booleans") && compressor.in_boolean_context() ? best_of(compressor, self, make_node(AST_Seq, self, {
                        car: self,
                        cdr: make_node(AST_True, self)
                ...
                OPT(AST_Assign, function (self, compressor) {
                    ...
                    var cond = self.condition.evaluate(compressor);
                    if (cond !== self.condition) return cond ? (compressor.warn("Condition always true [{file}:{line},{col}]", self.start), maintain_this_binding(compressor.parent(), self, self.consequent)) : (compressor.warn("Condition always false [{file}:{line},{col}]", self.start), maintain_this_binding(compressor.parent(), self, self.alternative));
                    ...
                    var condition = self.condition, consequent = self.consequent, alternative = self.alternative;
                    return condition instanceof AST_SymbolRef && consequent instanceof AST_SymbolRef && condition.definition() === consequent.definition() ? make_node(AST_Binary, self, {
                        operator: "||",
                        left: condition,
                        right: alternative
                    }) : consequent instanceof AST_Assign && alternative instanceof AST_Assign && consequent.operator == alternative.operator && consequent.left.equivalent_to(alternative.left) && (!self.condition.has_side_effects(compressor) || "=" == consequent.operator && !consequent.left.has_side_effects(compressor)) ? make_node(AST_Assign, self, {
                    ...
                    }) : consequent instanceof AST_Call && alternative.TYPE === consequent.TYPE && 1 == consequent.args.length && 1 == alternative.args.length && consequent.expression.equivalent_to(alternative.expression) && !consequent.expression.has_side_effects(compressor) ? (consequent.args[0] = make_node(AST_Conditional, self, {
                        condition: self.condition,
                        consequent: consequent.args[0],
                        alternative: alternative.args[0]
                    }), consequent) : consequent instanceof AST_Conditional && consequent.alternative.equivalent_to(alternative) ? make_node(AST_Conditional, self, {
                        condition: make_node(AST_Binary, self, {
                            left: self.condition,
                            operator: "&&",
                            right: consequent.condition
                        }), consequent: consequent.consequent, alternative: alternative
                    }) : consequent.equivalent_to(alternative) ? make_node(AST_Seq, self, {
                        car: self.condition,
                        cdr: consequent
                    }).optimize(compressor) : is_true(self.consequent) ? is_false(self.alternative) ? booleanize(self.condition) : make_node(AST_Binary, self, {
                        operator: "||",
                    ...
                    if (compressor.option("booleans")) {
                        ...
                        return p instanceof AST_Binary && ("==" == p.operator || "!=" == p.operator) ? (compressor.warn("Non-strict equality against boolean: {operator} {value} [{file}:{line},{col}]", {
                            ...
                            line: p.start.line,
                            col: p.start.col
                        }), make_node(AST_Number, self, {value: +self.value})) : make_node(AST_UnaryPrefix, self, {
                            operator: "!",
                    ...
                    if (prop instanceof AST_String && compressor.option("properties")) {
                        ...
                        isNaN(v) || v.toString() != prop || (self.property = make_node(AST_Number, self.property, {value: v}))
                    }
                    var ev = self.evaluate(compressor);
                    ...
                    if (def) return def.optimize(compressor);
                    var prop = self.property;
                    if (RESERVED_WORDS(prop) && !compressor.option("screw_ie8")) return make_node(AST_Sub, self, {
                        expression: self.expression,
                    ...
                    return self.value && is_undefined(self.value, compressor) && (self.value = null), self
                }), OPT(AST_VarDef, function (self, compressor) {
                    var defines = compressor.option("global_defs");
                    return defines && HOP(defines, self.name.name) && compressor.warn("global_defs " + self.name.name + " redefined [{file}:{line},{col}]", self.start), self
                })
            }(), function () {
                ...
                var MOZ_TO_ME = {
                    Program: function (M) {
                        ...
                        if (1 < handlers.length || M.guardedHandlers && M.guardedHandlers.length) throw new Error("Multiple catch clauses are not supported.");
                        return new AST_Try({
                            start: my_start_token(M),
                            end: my_end_token(M),
                        ...
                        return "init" == M.kind ? new AST_ObjectKeyVal(args) : (args.key = new AST_SymbolAccessor({name: args.key}), args.value = new AST_Accessor(args.value), "get" == M.kind ? new AST_ObjectGetter(args) : "set" == M.kind ? new AST_ObjectSetter(args) : void 0)
                        ...
                        switch (typeof val) {
                            ...
                            default:
                                var rx = M.regex;
                                return rx && rx.pattern ? args.value = new RegExp(rx.pattern, rx.flags).toString() : args.value = M.regex && M.raw ? M.raw : val, new AST_RegExp(args)
                        }
                ...
                function map(moztype, mytype, propmap) {
                    ...
                    }), moz_to_me += "\n})\n}", me_to_moz += "\n}\n}", moz_to_me = new Function("U2", "my_start_token", "my_end_token", "from_moz", "return(" + moz_to_me + ")")(exports, my_start_token, my_end_token, from_moz), me_to_moz = new Function("to_moz", "to_moz_block", "to_moz_scope", "return(" + me_to_moz + ")")(to_moz, to_moz_block, to_moz_scope), MOZ_TO_ME[moztype] = moz_to_me, def_to_moz(mytype, me_to_moz)
                }
                ...
                MOZ_TO_ME.UpdateExpression = MOZ_TO_ME.UnaryExpression = function (M) {
                    ...
                    var kind, key = {type: "Literal", value: M.key instanceof AST_SymbolAccessor ? M.key.name : M.key};
                    ...
                    return {
                        ...
                        value: value,
                        raw: value.toString(),
                        regex: {pattern: value.source, flags: value.toString().match(/[gimuy]*$/)[0]}
                    }
                    ...
                    } : {type: "Literal", value: value, raw: M.start.raw}
                ...
                function def_to_moz(mytype, handler) {
                    mytype.DEFMETHOD("to_mozilla_ast", function () {
                        return moznode = handler(mynode = this), start = mynode.start, end = mynode.end, null != start.pos && null != end.endpos && (moznode.range = [start.pos, end.endpos]), start.line && (moznode.loc = {
                            start: {
                                line: start.line,
                                column: start.col
            ...
            }(), exports.Compressor = Compressor, exports.DefaultsError = DefaultsError, exports.Dictionary = Dictionary, exports.JS_Parse_Error = JS_Parse_Error, exports.MAP = MAP,exports.OutputStream = OutputStream,exports.SourceMap = function (options) {
            ...
            },exports.TreeTransformer = TreeTransformer,exports.TreeWalker = TreeWalker,exports.base54 = base54,exports.defaults = defaults,exports.mangle_properties = function (ast, options) {
                ...
                null == reserved && (reserved = function () {
                ...
                null == cache && (cache = {cname: -1, props: new Dictionary});
                var debug_name_suffix, regex = options.regex, ignore_quoted = options.ignore_quoted,
                    debug = !1 !== options.debug;
                debug && (debug_name_suffix = !0 === options.debug ? "" : options.debug);
                var names_to_mangle = [], unmangleable = [], ignored = {};
                return ast.walk(new TreeWalker(function (node) {
                    node instanceof AST_ObjectKeyVal ? add(node.key, ignore_quoted && node.quote) : node instanceof AST_ObjectProperty ? add(node.key.name) : node instanceof AST_Dot ? add(node.property) : node instanceof AST_Sub && function (node, ignore) {
                        var out = {};
                        try {
                            !function walk(node) {
                                node.walk(new TreeWalker(function (node) {
                                    if (node instanceof AST_Seq) return walk(node.cdr), !0;
                                    if (node instanceof AST_String) return add(node.value, ignore), !0;
                                    if (node instanceof AST_Conditional) return walk(node.consequent), walk(node.alternative), !0;
                                    throw out
                                }))
                            }(node)
                        } catch (ex) {
                    ...
                    }(node.property, ignore_quoted)
                })), ast.transform(new TreeTransformer(function (node) {
                    node instanceof AST_ObjectKeyVal ? ignore_quoted && node.quote || (node.key = mangle(node.key)) : node instanceof AST_ObjectProperty ? node.key.name = mangle(node.key.name) : node instanceof AST_Dot ? node.property = mangle(node.property) : node instanceof AST_Sub && (ignore_quoted || (node.property = function mangleStrings(node) {
                        return node.transform(new TreeTransformer(function (node) {
                            return node instanceof AST_Seq ? node.cdr = mangleStrings(node.cdr) : node instanceof AST_String ? node.value = mangle(node.value) : node instanceof AST_Conditional && (node.consequent = mangleStrings(node.consequent), node.alternative = mangleStrings(node.alternative)), node
                        }))
                    }(node.property)))
                }));

                function can_mangle(name) {
                    if (!(0 <= unmangleable.indexOf(name) || 0 <= reserved.indexOf(name))) {
                        if (options.only_cache) return cache.props.has(name);
                        if (!/^-?[0-9]+(\.[0-9]+)?(e[+-][0-9]+)?$/.test(name)) return 1
                    }
                }

                function should_mangle(name) {
                    return !(ignore_quoted && name in ignored || regex && !regex.test(name) || 0 <= reserved.indexOf(name)) && (cache.props.has(name) || 0 <= names_to_mangle.indexOf(name))
                }

                function add(name, ignore) {
                    ignore ? ignored[name] = !0 : (can_mangle(name) && push_uniq(names_to_mangle, name), should_mangle(name) || push_uniq(unmangleable, name))
                }
                ...
                function mangle(name) {
                    ...
                    if (!mangled) {
                        if (debug) {
                            ...
                            !can_mangle(debug_mangled) || ignore_quoted && debug_mangled in ignored || (mangled = debug_mangled)
                        }
                        if (!mangled) for (; !can_mangle(mangled = base54(++cache.cname)) || ignore_quoted && mangled in ignored;) ;
                        cache.props.set(name, mangled)
                    }
                    return mangled
                }
            },exports.merge = merge,exports.parse = parse,exports.push_uniq = push_uniq,exports.string_template = string_template,exports.tokenizer = tokenizer,exports.is_identifier = is_identifier,exports.SymbolDef = SymbolDef,AST_Node.warn_function = function (txt) {
                ...
                "string" == typeof inMap && "inline" != inMap && (inMap = JSON.parse(rjsFile.readFile(inMap, "utf8")));
                var toplevel = null, sourcesContent = {};
                if (options.spidermonkey) {
                    if ("inline" == inMap) throw new Error("inline source map only works with built-in parser");
                    ...
                    var addFile = function (file, fileUrl) {
                        ...
                        "inline" == inMap && (inMap = function (code) {
                            var match = /\n\/\/# sourceMappingURL=data:application\/json(;.*?)?;base64,(.*)/.exec(code);
                            return match ? JSON.parse(new Buffer(match[2], "base64")) : (exports.AST_Node.warn("inline source map not found"), null)
                        }(code)), sourcesContent[fileUrl] = code, toplevel = exports.parse(code, {
                            filename: fileUrl,
                            toplevel: toplevel,
                            bare_returns: options.parse ? options.parse.bare_returns : void 0
                        })
                    };
                    if (!options.fromString && (files = exports.simple_glob(files), "inline" == inMap && 1 < files.length)) throw new Error("inline source map only works with singular input");
                    [].concat(files).forEach(function (files, i) {
                        if ("string" == typeof files) addFile(files, options.fromString ? i : files); else for (var fileUrl in files) addFile(files[fileUrl], fileUrl)
                    })
                }
                if (options.wrap && (toplevel = toplevel.wrap_commonjs(options.wrap, options.exportAll)), options.compress) {
                    var compress = {warnings: options.warnings};
                    exports.merge(compress, options.compress), toplevel.figure_out_scope(options.mangle);
                    var sq = exports.Compressor(compress);
                    toplevel = sq.compress(toplevel)
                }
                (options.mangleProperties || options.nameCache) && (options.mangleProperties.cache = exports.readNameCache(options.nameCache, "props"), toplevel = exports.mangle_properties(toplevel, options.mangleProperties), exports.writeNameCache(options.nameCache, "props", options.mangleProperties.cache)), options.mangle && (toplevel.figure_out_scope(options.mangle), toplevel.compute_char_frequency(options.mangle), toplevel.mangle_names(options.mangle));
                var output = {max_line_len: 32e3};
                if ((options.outSourceMap || options.sourceMapInline) && (output.source_map = exports.SourceMap({
                    file: options.outFileName || ("string" == typeof options.outSourceMap ? options.outSourceMap.replace(/\.map$/i, "") : null),
                    orig: inMap,
                    root: options.sourceRoot
                }), options.sourceMapIncludeSources)) for (var file in sourcesContent) sourcesContent.hasOwnProperty(file) && output.source_map.get().setSourceContent(file, sourcesContent[file]);
                options.output && exports.merge(output, options.output);
                var stream = exports.OutputStream(output);
                toplevel.print(stream);
                var source_map = output.source_map;
                source_map && (source_map += "");
                return options.sourceMapInline ? stream += "\n//# sourceMappingURL=data:application/json;charset=utf-8;base64," + new Buffer(source_map).toString("base64") : options.outSourceMap && "string" == typeof options.outSourceMap && !1 !== options.sourceMapUrl && (stream += "\n//# sourceMappingURL=" + ("string" == typeof options.sourceMapUrl ? options.sourceMapUrl : options.outSourceMap)), {
                    code: stream + "",
                    map: source_map
                }
            },exports.describe_ast = function () {
                var out = exports.OutputStream({beautify: !0});
                return function doitem(ctor) {
                    out.print("AST_" + ctor.TYPE);
                    var props = ctor.SELF_PROPS.filter(function (prop) {
                        return !/^\$/.test(prop)
                    });
                    0 < props.length && (out.space(), out.with_parens(function () {
                        props.forEach(function (prop, i) {
                            i && out.space(), out.print(prop)
                        })
                    })), ctor.documentation && (out.space(), out.print_string(ctor.documentation)), 0 < ctor.SUBCLASSES.length && (out.space(), out.with_block(function () {
                        ctor.SUBCLASSES.forEach(function (ctor, i) {
                            out.indent(), doitem(ctor), out.newline()
                        })
                    }))
                }(exports.AST_Node), out + ""
                ...
                var cache = null;
                if (filename) try {
                    cache = rjsFile.readFile(filename, "utf8");
                    if (!(cache = JSON.parse(cache)[key])) throw"init";
                    cache.props = exports.Dictionary.fromObject(cache.props)
                } catch (ex) {
                    cache = {cname: -1, props: new exports.Dictionary}
                }
                return cache
                ...
                if (filename) {
                    var data;
                    try {
                        data = rjsFile.readFile(filename, "utf8"), data = JSON.parse(data)
                    } catch (ex) {
                        data = {}
                    }
                    data[key] = {
                ...
                if (glob.match(/\*|\?/)) {
                    ...
                    if (entries) {
                        var pattern = "^" + path.basename(glob).replace(/[.+^$[\]\\(){}]/g, "\\$&").replace(/\*/g, "[^/\\\\]*").replace(/\?/g, "[^/\\\\]") + "$",
                            mod = "win32" === process.platform ? "i" : "", rx = new RegExp(pattern, mod),
                            results = entries.filter(function (name) {
                                return rx.test(name)
                            }).map(function (name) {
            ...
            function arrayToString(ary) {
                var output = "[";
                return ary && ary.forEach(function (item, i) {
                    output += (0 < i ? "," : "") + '"' + lang.jsEscape(item) + '"'
                }), output += "]"
            }
            ...
            function traverse(object, visitor) {
                var child;
                if (object) {
                    if (!1 === visitor.call(null, object)) return !1;
                    for (var i = 0, keys = Object.keys(object); i < keys.length; i++) if ("object" == typeof (child = object[keys[i]]) && null !== child && !1 === traverse(child, visitor)) return !1
                }
            }
            ...
            function traverseBroad(object, visitor) {
                ...
                if (object) {
                    if (!1 === visitor.call(null, object)) return !1;
                    for (var i = 0, keys = Object.keys(object); i < keys.length; i++) "object" == typeof (child = object[key]) && null !== child && traverseBroad(child, visitor)
                }
            }

            function getValidDeps(node) {
                if (node && "ArrayExpression" === node.type && node.elements) {
                    var deps = [];
                    return node.elements.some(function (elem) {
                        "Literal" === elem.type && deps.push(elem.value)
                    }), deps.length ? deps : void 0
                }
            }

            function isFnExpression(node) {
                return node && ("FunctionExpression" === node.type || "ArrowFunctionExpression" === node.type)
            }

            function parse(moduleName, fileName, fileContents, options) {
                options = options || {};
                var i, moduleCall, depString, moduleDeps = [], result = "", moduleList = [], needsDefine = !0,
                    astRoot = esprima.parse(fileContents);
                if (parse.recurse(astRoot, function (callName, config, name, deps, node, factoryIdentifier, fnExpScope) {
                    return deps = deps || [], "define" !== callName || name && name !== moduleName || (needsDefine = !1), name ? moduleList.push({
                        name: name,
                        deps: deps
                    }) : moduleDeps = moduleDeps.concat(deps), "define" === callName && factoryIdentifier && hasProp(fnExpScope, factoryIdentifier) ? factoryIdentifier : !!options.findNestedDependencies
                }, options), options.insertNeedsDefine && needsDefine && (result += 'require.needsDefine("' + moduleName + '");'), moduleDeps.length || moduleList.length) {
                    for (i = 0; i < moduleList.length; i++) result && (result += "\n"), (moduleCall = moduleList[i]).name === moduleName && (moduleCall.deps = moduleCall.deps.concat(moduleDeps), moduleDeps = []), depString = arrayToString(moduleCall.deps), result += 'define("' + moduleCall.name + '",' + depString + ");";
                    moduleDeps.length && (result && (result += "\n"), depString = arrayToString(moduleDeps), result += 'define("' + moduleName + '",' + depString + ");")
                }
                return result || null
            }

            return parse.traverse = traverse, parse.traverseBroad = traverseBroad, parse.isFnExpression = isFnExpression, parse.recurse = function (object, onMatch, options, fnExpScope) {
                var keys, child, result, i, params, param, tempObject, hasHas = options && options.has;
                if (fnExpScope = fnExpScope || emptyScope, object) if (hasHas && "IfStatement" === object.type && object.test.type && "Literal" === object.test.type) object.test.value ? this.recurse(object.consequent, onMatch, options, fnExpScope) : this.recurse(object.alternate, onMatch, options, fnExpScope); else {
                    if (!1 === (result = this.parseNode(object, onMatch, fnExpScope))) return;
                    if ("string" == typeof result) return result;
                    if ("ExpressionStatement" === object.type && object.expression && "CallExpression" === object.expression.type && object.expression.callee && isFnExpression(object.expression.callee) && (tempObject = object.expression.callee), "UnaryExpression" === object.type && object.argument && "CallExpression" === object.argument.type && object.argument.callee && isFnExpression(object.argument.callee) && (tempObject = object.argument.callee), tempObject && tempObject.params && tempObject.params.length) for (params = tempObject.params, fnExpScope = mixin({}, fnExpScope, !0), i = 0; i < params.length; i++) "Identifier" === (param = params[i]).type && (fnExpScope[param.name] = !0);
                    for (i = 0, keys = Object.keys(object); i < keys.length && ("object" != typeof (child = object[keys[i]]) || null === child || "string" != typeof (result = this.recurse(child, onMatch, options, fnExpScope)) || !hasProp(fnExpScope, result)); i++) ;
                    if ("string" == typeof result) return hasProp(fnExpScope, result) ? result : void 0
                }
            }, parse.definesRequire = function (fileName, fileContents) {
                var foundDefine = !1, foundDefineAmd = !1;
                return traverse(esprima.parse(fileContents), function (node) {
                    if ("Program" === node.type && node.body && node.body.length && (foundDefine = node.body.some(function (bodyNode) {
                        if ("VariableDeclaration" === bodyNode.type) {
                            var decls = bodyNode.declarations;
                            if (decls) if (decls.some(function (declNode) {
                                return "VariableDeclarator" === declNode.type && declNode.id && "Identifier" === declNode.id.type && "define" === declNode.id.name
                            })) return !0
                        }
                        if ("FunctionDeclaration" === bodyNode.type && bodyNode.id && "Identifier" === bodyNode.id.type && "define" === bodyNode.id.name) return !0
                    })), foundDefine && parse.hasDefineAmd(node)) return !(foundDefineAmd = !0)
                }), foundDefine && foundDefineAmd
            }, parse.getAnonDeps = function (fileName, fileContents) {
                var astRoot = "string" == typeof fileContents ? esprima.parse(fileContents) : fileContents,
                    defFunc = this.findAnonDefineFactory(astRoot);
                return parse.getAnonDepsFromNode(defFunc)
            }, parse.getAnonDepsFromNode = function (node) {
                var funcArgLength, deps = [];
                return node && (this.findRequireDepNames(node, deps), (funcArgLength = node.params && node.params.length) && (deps = (1 < funcArgLength ? ["require", "exports", "module"] : ["require"]).concat(deps))), deps
            }, parse.isDefineNodeWithArgs = function (node) {
                return node && "CallExpression" === node.type && node.callee && "Identifier" === node.callee.type && "define" === node.callee.name && node[argPropName]
            }, parse.findAnonDefineFactory = function (node) {
                var match;
                return traverse(node, function (node) {
                    var arg0, arg1;
                    if (parse.isDefineNodeWithArgs(node)) {
                        if (isFnExpression(arg0 = node[argPropName][0])) return match = arg0, !1;
                        if (arg1 = node[argPropName][1], "Literal" === arg0.type && isFnExpression(arg1)) return match = arg1, !1
                    }
                }), match
            }, parse.findConfig = function (fileContents) {
                var jsConfig, foundConfig, stringData, foundRange, quote, quoteMatch, quoteRegExp = /(:\s|\[\s*)(['"])/,
                    astRoot = esprima.parse(fileContents, {loc: !0});
                return traverse(astRoot, function (node) {
                    var arg, requireType = parse.hasRequire(node);
                    if (!requireType || "require" !== requireType && "requirejs" !== requireType && "requireConfig" !== requireType && "requirejsConfig" !== requireType) {
                        if (arg = parse.getRequireObjectLiteral(node)) return stringData = parse.nodeToString(fileContents, arg), jsConfig = stringData.value, foundRange = stringData.range, !1
                    } else if ((arg = node[argPropName] && node[argPropName][0]) && "ObjectExpression" === arg.type) return stringData = parse.nodeToString(fileContents, arg), jsConfig = stringData.value, foundRange = stringData.range, !1
                }), jsConfig && (quoteMatch = quoteRegExp.exec(jsConfig), quote = quoteMatch && quoteMatch[2] || '"', foundConfig = eval("(" + jsConfig + ")")), {
                    config: foundConfig,
                    range: foundRange,
                    quote: quote
                }
            }, parse.getRequireObjectLiteral = function (node) {
                if (node.id && "Identifier" === node.id.type && ("require" === node.id.name || "requirejs" === node.id.name) && node.init && "ObjectExpression" === node.init.type) return node.init
            }, parse.renameNamespace = function (fileContents, ns) {
                var lines, locs = [], astRoot = esprima.parse(fileContents, {loc: !0});
                return parse.recurse(astRoot, function (callName, config, name, deps, node) {
                    return locs.push(node.loc), "define" !== callName
                }, {}), locs.length && (lines = fileContents.split("\n"), locs.reverse(), locs.forEach(function (loc) {
                    var startIndex = loc.start.column, lineIndex = loc.start.line - 1, line = lines[lineIndex];
                ...
                var dependencies = [], astRoot = esprima.parse(fileContents);
                return parse.recurse(astRoot, function (callName, config, name, deps) {
                    deps && (dependencies = dependencies.concat(deps))
                }, options), dependencies
            }, parse.findCjsDependencies = function (fileName, fileContents) {
                var dependencies = [];
                return traverse(esprima.parse(fileContents), function (node) {
                    var arg;
                    node && "CallExpression" === node.type && node.callee && "Identifier" === node.callee.type && "require" === node.callee.name && node[argPropName] && 1 === node[argPropName].length && "Literal" === (arg = node[argPropName][0]).type && dependencies.push(arg.value)
                }), dependencies
            }, parse.hasDefDefine = function (node) {
                return "FunctionDeclaration" === node.type && node.id && "Identifier" === node.id.type && "define" === node.id.name
            }, parse.hasDefineAmd = function (node) {
                return node && "AssignmentExpression" === node.type && node.left && "MemberExpression" === node.left.type && node.left.object && "define" === node.left.object.name && node.left.property && "amd" === node.left.property.name
            }, parse.refsDefineAmd = function (node) {
                return node && "MemberExpression" === node.type && node.object && "define" === node.object.name && "Identifier" === node.object.type && node.property && "amd" === node.property.name && "Identifier" === node.property.type
            }, parse.hasRequire = function (node) {
                var callName, c = node && node.callee;
                return node && "CallExpression" === node.type && c && ("Identifier" !== c.type || "require" !== c.name && "requirejs" !== c.name ? "MemberExpression" === c.type && c.object && "Identifier" === c.object.type && ("require" === c.object.name || "requirejs" === c.object.name) && c.property && "config" === c.property.name && (callName = c.object.name + "Config") : callName = c.name), callName
            }, parse.hasDefine = function (node) {
                return node && "CallExpression" === node.type && node.callee && "Identifier" === node.callee.type && "define" === node.callee.name
            }, parse.getNamedDefine = function (fileContents) {
                var name;
                return traverse(esprima.parse(fileContents), function (node) {
                    if (node && "CallExpression" === node.type && node.callee && "Identifier" === node.callee.type && "define" === node.callee.name && node[argPropName] && node[argPropName][0] && "Literal" === node[argPropName][0].type) return name = node[argPropName][0].value, !1
                }), name
            }, parse.getAllNamedDefines = function (fileContents, excludeMap) {
                var names = [];
                return parse.recurse(esprima.parse(fileContents), function (callName, config, name, deps, node, factoryIdentifier, fnExpScope) {
                    return "define" === callName && name && (excludeMap.hasOwnProperty(name) || names.push(name)), "define" !== callName || !factoryIdentifier || !hasProp(fnExpScope, factoryIdentifier) || factoryIdentifier
                }, {}), names
            }, parse.usesAmdOrRequireJs = function (fileName, fileContents) {
                var uses;
                return traverse(esprima.parse(fileContents), function (node) {
                    var type, callName, arg;
                    parse.hasDefDefine(node) ? type = "declaresDefine" : parse.hasDefineAmd(node) ? type = "defineAmd" : (callName = parse.hasRequire(node)) ? !(arg = node[argPropName] && node[argPropName][0]) || "ObjectExpression" !== arg.type && "ArrayExpression" !== arg.type || (type = callName) : parse.hasDefine(node) && (type = "define"), type && ((uses = uses || {})[type] = !0)
                }), uses
            }, parse.usesCommonJs = function (fileName, fileContents) {
                var uses = null, assignsExports = !1;
                return traverse(esprima.parse(fileContents), function (node) {
                    var type, exp = node.expression || node.init;
                    "Identifier" !== node.type || "__dirname" !== node.name && "__filename" !== node.name ? "VariableDeclarator" === node.type && node.id && "Identifier" === node.id.type && "exports" === node.id.name ? type = "varExports" : exp && "AssignmentExpression" === exp.type && exp.left && "MemberExpression" === exp.left.type && exp.left.object ? "module" === exp.left.object.name && exp.left.property && "exports" === exp.left.property.name ? type = "moduleExports" : "exports" === exp.left.object.name && exp.left.property ? type = "exports" : "MemberExpression" === exp.left.object.type && "module" === exp.left.object.object.name && "exports" === exp.left.object.property.name && "Identifier" === exp.left.object.property.type && (type = "moduleExports") : node && "CallExpression" === node.type && node.callee && "Identifier" === node.callee.type && "require" === node.callee.name && node[argPropName] && 1 === node[argPropName].length && "Literal" === node[argPropName][0].type && (type = "require") : type = node.name.substring(2), type && ("varExports" === type ? assignsExports = !0 : "exports" === type && assignsExports || ((uses = uses || {})[type] = !0))
                }), uses
            }, parse.findRequireDepNames = function (node, deps) {
                traverse(node, function (node) {
                    var arg;
                    node && "CallExpression" === node.type && node.callee && "Identifier" === node.callee.type && "require" === node.callee.name && node[argPropName] && 1 === node[argPropName].length && "Literal" === (arg = node[argPropName][0]).type && deps.push(arg.value)
                })
                ...
                var name, deps, cjsDeps, arg, factory, exp, refsDefine, bodyNode, args = node && node[argPropName],
                    callName = parse.hasRequire(node), isUmd = !1;
                if ("require" === callName || "requirejs" === callName) {
                    if ((arg = node[argPropName] && node[argPropName][0]) && "ArrayExpression" !== arg.type && "ObjectExpression" === arg.type && (arg = node[argPropName][1]), !(deps = getValidDeps(arg))) return;
                    return onMatch("require", null, null, deps, node)
                }
                if (parse.hasDefine(node) && args && args.length) {
                    if (name = args[0], deps = args[1], factory = args[2], "ArrayExpression" === name.type ? (factory = deps, deps = name, name = null) : isFnExpression(name) ? (factory = name, name = deps = null) : "Identifier" === name.type && 1 === args.length && hasProp(fnExpScope, name.name) ? (isUmd = !0, factory = name, name = null) : "Literal" !== name.type && (name = deps = factory = null), name && "Literal" === name.type && deps && (isFnExpression(deps) ? (factory = deps, deps = null) : "ObjectExpression" === deps.type ? deps = factory = null : "Identifier" === deps.type && (2 === args.length ? deps = factory = null : 3 === args.length && isFnExpression(factory) && (deps = null))), deps && "ArrayExpression" === deps.type) deps = getValidDeps(deps); else if (isFnExpression(factory)) (cjsDeps = parse.getAnonDepsFromNode(factory)).length && (deps = cjsDeps); else if (deps || factory && !isUmd) return;
                    return name && "Literal" === name.type && (name = name.value), onMatch("define", null, name, deps, node, factory && "Identifier" === factory.type ? factory.name : void 0, fnExpScope)
                }
                if ("CallExpression" === node.type && node.callee && isFnExpression(node.callee) && node.callee.body && node.callee.body.body && 1 === node.callee.body.body.length && "IfStatement" === node.callee.body.body[0].type && (bodyNode = node.callee.body.body[0]).consequent && bodyNode.consequent.body && "ExpressionStatement" === (exp = bodyNode.consequent.body[0]).type && exp.expression && parse.hasDefine(exp.expression) && exp.expression.arguments && 1 === exp.expression.arguments.length && "Identifier" === exp.expression.arguments[0].type && (traverse(bodyNode.test, function (node) {
                    if (parse.refsDefineAmd(node)) return !(refsDefine = !0)
                }), refsDefine)) return onMatch("define", null, null, null, exp.expression, exp.expression.arguments[0].name, fnExpScope)
            }, parse.nodeToString = function (contents, node) {
                var extracted, loc = node.loc, lines = contents.split("\n"),
                    preamble = (1 < loc.start.line ? lines.slice(0, loc.start.line - 1).join("\n") + "\n" : "") + lines[loc.start.line - 1].substring(0, loc.start.column);
                return {
                    value: extracted = loc.start.line === loc.end.line ? lines[loc.start.line - 1].substring(loc.start.column, loc.end.column) : lines[loc.start.line - 1].substring(loc.start.column) + "\n" + lines.slice(loc.start.line, loc.end.line - 1).join("\n") + "\n" + lines[loc.end.line - 1].substring(0, loc.end.column),
                    range: [preamble.length, preamble.length + extracted.length]
                }
            }, parse.getLicenseComments = function (fileName, contents) {
                var commentNode, refNode, subNode, value, i, j, ast = esprima.parse(contents, {comment: !0, range: !0}),
                    result = "", existsMap = {}, lineEnd = -1 === contents.indexOf("\r") ? "\n" : "\r\n";
                if (ast.comments) for (i = 0; i < ast.comments.length; i++) {
                    if ("Line" === (commentNode = ast.comments[i]).type) if (value = "//" + commentNode.value + lineEnd, refNode = commentNode, i + 1 >= ast.comments.length) value += lineEnd; else {
                        for (j = i + 1; j < ast.comments.length && ("Line" === (subNode = ast.comments[j]).type && subNode.range[0] === refNode.range[1] + 1); j++) value += "//" + subNode.value + lineEnd, refNode = subNode;
                        value += lineEnd, i = j - 1
                    } else value = "/*" + commentNode.value + "*/" + lineEnd + lineEnd;
                    existsMap[value] || -1 === value.indexOf("license") && ("Block" !== commentNode.type || 0 !== value.indexOf("/*!")) && -1 === value.indexOf("opyright") && -1 === value.indexOf("(c)") || (result += value, existsMap[value] = !0)
                }
                return result
            }, parse
        }), define("transform", ["./esprimaAdapter", "./parse", "logger", "lang"], function (esprima, parse, logger, lang) {
            "use strict";
            var transform, baseIndentRegExp = /^([ \t]+)/, indentRegExp = /\{[\r\n]+([ \t]+)/,
                keyRegExp = /^[_A-Za-z]([A-Za-z\d_]*)$/, bulkIndentRegExps = {"\n": /\n/g, "\r\n": /\r\n/g};
            return transform = {
                toTransport: function (namespace, moduleName, path, contents, onFound, options) {
                    options = options || {};

                    function applySourceUrl(contents) {
                        return options.useSourceUrl && (contents = 'eval("' + lang.jsEscape(contents) + "\\n//# sourceURL=" + (0 === path.indexOf("/") ? "" : "/") + path + '");\n'), contents
                    }

                    var astRoot, contentLines, modLine, foundAnon, scanCount = 0, scanReset = !1, defineInfos = [];
                    try {
                        astRoot = esprima.parse(contents, {loc: !0})
                    } catch (e) {
                        return logger.trace("toTransport skipping " + path + ": " + e.toString()), contents
                    }
                    return parse.traverse(astRoot, function (node) {
                        var args, firstArg, firstArgLoc, factoryNode, needsId, depAction, init, range, namespaceExists;
                        if ("VariableDeclarator" === node.type && node.id && "define" === node.id.name && "Identifier" === node.id.type && !((init = node.init) && init.callee && "CallExpression" === init.callee.type && init.callee.callee && "Identifier" === init.callee.callee.type && "require" === init.callee.callee.name && init.callee.arguments && 1 === init.callee.arguments.length && "Literal" === init.callee.arguments[0].type && init.callee.arguments[0].value && -1 !== init.callee.arguments[0].value.indexOf("amdefine"))) return !1;
                        if ((namespaceExists = namespace && "CallExpression" === node.type && node.callee && node.callee.object && "Identifier" === node.callee.object.type && node.callee.object.name === namespace && "Identifier" === node.callee.property.type && "define" === node.callee.property.name) || parse.isDefineNodeWithArgs(node)) {
                            if (!(args = node.arguments) || !args.length) return;
                            if (firstArgLoc = (firstArg = args[0]).loc, 1 === args.length) "Identifier" === firstArg.type ? (needsId = !0, depAction = "empty") : parse.isFnExpression(firstArg) ? (factoryNode = firstArg, needsId = !0, depAction = "scan") : "ObjectExpression" === firstArg.type || "Literal" === firstArg.type && "number" == typeof firstArg.value || "UnaryExpression" === firstArg.type && "-" === firstArg.operator && firstArg.argument && "Literal" === firstArg.argument.type && "number" == typeof firstArg.argument.value ? (needsId = !0, depAction = "skip") : "MemberExpression" === firstArg.type && firstArg.object && firstArg.property && "Identifier" === firstArg.property.type && (needsId = !0, depAction = "empty"); else if ("ArrayExpression" === firstArg.type) needsId = !0, depAction = "skip"; else {
                                if ("Literal" !== firstArg.type || "string" != typeof firstArg.value) return;
                                needsId = !1, depAction = 2 === args.length && parse.isFnExpression(args[1]) ? (factoryNode = args[1], "scan") : "skip"
                            }
                            if ((range = {
                                foundId: void 0,
                                needsId: needsId,
                                ...
                                sourceUrlData: void 0
                            }).needsId) {
                                if (foundAnon) return logger.trace(path + " has more than one anonymous define. May be a built file from another build system like, Ender. Skipping normalization."), !(defineInfos = []);
                                foundAnon = range, defineInfos.push(range)
                            } else "scan" === depAction && (1 < (scanCount += 1) ? scanReset || (defineInfos = foundAnon ? [foundAnon] : [], scanReset = !0) : defineInfos.push(range))
                        }
                    }), defineInfos.length ? (defineInfos.reverse(), contentLines = contents.split("\n"), modLine = function (loc, contentInsertion) {
                        var startIndex = loc.start.column, lineIndex = loc.start.line - 1,
                        ...
                        var deps, contentInsertion = "", depString = "";
                        info.needsId && moduleName && (contentInsertion += "'" + moduleName + "',"), "scan" === info.depAction && (depString = (deps = parse.getAnonDepsFromNode(info.factoryNode)).length ? "[" + deps.map(function (dep) {
                            return "'" + dep + "'"
                        }) + "]" : "[]", depString += ",", info.factoryNode ? modLine(info.factoryNode.loc, depString) : contentInsertion += depString), contentInsertion && modLine(info.firstArgLoc, contentInsertion), namespace && !info.namespaceExists && modLine(info.defineLoc, namespace + "."), onFound && onFound(info)
                    }), applySourceUrl(contents = contentLines.join("\n"))) : applySourceUrl(contents)
                }, modifyConfig: function (fileContents, onConfig) {
                    var details = parse.findConfig(fileContents), config = details.config;
                    return (config = config && onConfig(config)) ? transform.serializeConfig(config, fileContents, details.range[0], details.range[1], {quote: details.quote}) : fileContents
                }, serializeConfig: function (config, fileContents, start, end, options) {
                    var indent, match, outDentRegExp, baseIndent = "", startString = fileContents.substring(0, start),
                        existingConfigString = fileContents.substring(start, end),
                        lineReturn = -1 === existingConfigString.indexOf("\r") ? "\n" : "\r\n",
                        lastReturnIndex = startString.lastIndexOf("\n");
                    return -1 === lastReturnIndex && (lastReturnIndex = 0), (match = baseIndentRegExp.exec(startString.substring(lastReturnIndex + 1, start))) && match[1] && (baseIndent = match[1]), (match = indentRegExp.exec(existingConfigString)) && match[1] && (indent = match[1]), indent = !indent || indent.length < baseIndent ? "  " : indent.substring(baseIndent.length), outDentRegExp = new RegExp("(" + lineReturn + ")" + indent, "g"), startString + function (str, indent, lineReturn) {
                        var regExp = bulkIndentRegExps[lineReturn];
                        return str.replace(regExp, "$&" + indent)
                    }(transform.objectToString(config, {
                        indent: indent,
                        lineReturn: lineReturn,
                        outDentRegExp: outDentRegExp,
                        quote: options && options.quote
                    }), baseIndent, lineReturn) + fileContents.substring(end)
                }, objectToString: function (obj, options, totalIndent) {
                    var startBrace, endBrace, nextIndent, first = !0, value = "", lineReturn = options.lineReturn,
                        indent = options.indent, outDentRegExp = options.outDentRegExp, quote = options.quote || '"';
                    return nextIndent = (totalIndent = totalIndent || "") + indent, null === obj ? value = "null" : void 0 === obj ? value = "undefined" : "number" == typeof obj || "boolean" == typeof obj ? value = obj : "string" == typeof obj ? value = quote + lang.jsEscape(obj) + quote : lang.isArray(obj) ? (lang.each(obj, function (item, i) {
                        value += (0 !== i ? "," + lineReturn : "") + nextIndent + transform.objectToString(item, options, nextIndent)
                    }), startBrace = "[", endBrace = "]") : lang.isFunction(obj) || lang.isRegExp(obj) ? value = obj.toString().replace(outDentRegExp, "$1") : (lang.eachProp(obj, function (v, prop) {
                        value += (first ? "" : "," + lineReturn) + nextIndent + (keyRegExp.test(prop) ? prop : quote + lang.jsEscape(prop) + quote) + ": " + transform.objectToString(v, options, nextIndent), first = !1
                    }), startBrace = "{", endBrace = "}"), startBrace && (value = startBrace + lineReturn + value + lineReturn + totalIndent + endBrace), value
                }
            }
            ...
            function create(obj, mixin) {
                Temp.prototype = obj;
                var prop, temp = new Temp;
                if (Temp.prototype = null, mixin) for (prop in mixin) mixin.hasOwnProperty(prop) && !temp.hasOwnProperty(prop) && (temp[prop] = mixin[prop]);
                return temp
            }
            ...
            var pragma = {
                ...
                namespace: function (fileContents, ns, onLifecycleName) {
                    return ns && (fileContents = fileContents.replace(pragma.configRegExp, "$1" + ns + ".$2$3("), fileContents = (fileContents = (fileContents = (fileContents = (fileContents = (fileContents = (fileContents = (fileContents = (fileContents = parse.renameNamespace(fileContents, ns)).replace(pragma.defineTernaryRegExp, "typeof " + ns + ".define === 'function' && " + ns + ".define.amd ? " + ns + ".define")).replace(pragma.defineJQueryRegExp, "typeof " + ns + ".define === 'function' && " + ns + ".define.amd && " + ns + ".define.amd.jQuery")).replace(pragma.defineHasRegExp, "typeof " + ns + ".define === 'function' && typeof " + ns + ".define.amd === 'object' && " + ns + ".define.amd")).replace(pragma.defineExistsAndAmdRegExp, "typeof " + ns + ".define !== 'undefined' && " + ns + ".define.amd")).replace(pragma.defineCheckRegExp, "typeof " + ns + ".define === 'function' && " + ns + ".define.amd")).replace(pragma.defineStringCheckRegExp, "typeof " + ns + ".define === 'function' && " + ns + ".define['amd']")).replace(pragma.defineTypeFirstCheckRegExp, "'function' === typeof " + ns + ".define && " + ns + ".define.amd")).replace(pragma.defineExistsRegExp, "typeof " + ns + ".define !== 'undefined'"), pragma.apiDefRegExp.test(fileContents) && -1 === fileContents.indexOf("if (!" + ns + " || !" + ns + ".requirejs)") && (fileContents = "var " + ns + ";(function () { if (!" + ns + " || !" + ns + ".requirejs) {\nif (!" + ns + ") { " + ns + " = {}; } else { require = " + ns + "; }\n" + fileContents + "\n" + ns + ".requirejs = requirejs;" + ns + ".require = require;" + ns + ".define = define;\n}\n}());"), pragma.nsWrapRegExp.test(fileContents) && (fileContents = "(function () {\nvar require = " + ns + ".require,requirejs = " + ns + ".requirejs,define = " + ns + ".define;\n" + (fileContents = fileContents.replace(pragma.nsWrapRegExp, "")) + "\n}());")), fileContents
                },
                process: function (fileName, fileContents, config, onLifecycleName, pluginCollector) {
                    var foundIndex = -1, startIndex = 0, lineEndIndex, conditionLine, matches, type, marker, condition,
                        isTrue, endRegExp, endMatches, endMarkerIndex, shouldInclude, startLength, lifecycleHas, deps,
                        i, dep, moduleName, collectorMod, lifecyclePragmas, pragmas = config.pragmas,
                        hasConfig = config.has, kwArgs = pragmas;
                    if (onLifecycleName && (lifecyclePragmas = config["pragmas" + onLifecycleName], lifecycleHas = config["has" + onLifecycleName], lifecyclePragmas && (pragmas = create(pragmas || {}, lifecyclePragmas)), lifecycleHas && (hasConfig = create(hasConfig || {}, lifecycleHas))), hasConfig && (fileContents = fileContents.replace(pragma.hasRegExp, function (match, test) {
                        return hasConfig.hasOwnProperty(test) ? !!hasConfig[test] : match
                    })), !config.skipPragmas) for (; -1 !== (foundIndex = fileContents.indexOf("//>>", startIndex));) if (lineEndIndex = fileContents.indexOf("\n", foundIndex), -1 === lineEndIndex && (lineEndIndex = fileContents.length - 1), startIndex = lineEndIndex + 1, conditionLine = fileContents.substring(foundIndex, lineEndIndex + 1), matches = conditionLine.match(pragma.conditionalRegExp), matches) {
                        type = matches[1], marker = matches[2], condition = matches[3], isTrue = !1;
                        try {
                            isTrue = !!eval("(" + condition + ")")
                        } catch (e) {
                            throw"Error in file: " + fileName + ". Conditional comment: " + conditionLine + " failed with this error: " + e
                        }
                        if (endRegExp = new RegExp("\\/\\/\\>\\>\\s*" + type + "End\\(\\s*['\"]" + marker + "['\"]\\s*\\)", "g"), endMatches = endRegExp.exec(fileContents.substring(startIndex, fileContents.length)), !endMatches) throw"Error in file: " + fileName + ". Cannot find end marker for conditional comment: " + conditionLine;
                        endMarkerIndex = startIndex + endRegExp.lastIndex - endMatches[0].length, lineEndIndex = fileContents.indexOf("\n", endMarkerIndex), -1 === lineEndIndex && (lineEndIndex = fileContents.length - 1), shouldInclude = "exclude" === type && !isTrue || "include" === type && isTrue, startLength = startIndex - foundIndex, fileContents = fileContents.substring(0, foundIndex) + (shouldInclude ? fileContents.substring(startIndex, endMarkerIndex) : "") + fileContents.substring(lineEndIndex + 1, fileContents.length), startIndex = foundIndex
                    }
                    if (config.optimizeAllPluginResources && pluginCollector) try {
                        if (deps = parse.findDependencies(fileName, fileContents), deps.length) for (i = 0; i < deps.length; i++) dep = deps[i], -1 !== dep.indexOf("!") && (moduleName = dep.split("!")[0], collectorMod = pluginCollector[moduleName], collectorMod = collectorMod || (pluginCollector[moduleName] = []), collectorMod.push(dep))
                    } catch (eDep) {
                        logger.error("Parse error looking for plugin resources in " + fileName + ", skipping.")
                    }
                    return config.keepAmdefine || (fileContents = fileContents.replace(pragma.amdefineRegExp, "")), "OnSave" === onLifecycleName && config.namespace && (fileContents = pragma.namespace(fileContents, config.namespace, onLifecycleName)), pragma.removeStrict(fileContents, config)
                }
            };
            return pragma
        }), "browser" === env && define("browser/optimize", {}), "node" === env && define("node/optimize", {}), "rhino" === env && define("rhino/optimize", ["logger", "env!env/file"], function (logger, file) {
            Array.prototype.reduce || (Array.prototype.reduce = function (fn) {
                var accumulator, i = 0, length = this.length;
                if (2 <= arguments.length) accumulator = arguments[1]; else if (length) {
                    for (; !(i in this);) i++;
                    accumulator = this[i++]
                }
                for (; i < length; i++) i in this && (accumulator = fn.call(void 0, accumulator, this[i], i, this));
                return accumulator
            });
            var JSSourceFilefromCode, mapRegExp = /"file":"[^"]+"/;
            try {
            ...
            return {
                closure: function (fileName, fileContents, outFileName, keepLines, config) {
                    config = config || {};
                    var result, mappings, optimized, baseName, writer, outBaseName, outFileNameMap, srcOutFileName,
                        concatNameMap, options, option, compiler, externExportsPath, filename, content,
                        jscomp = Packages.com.google.javascript.jscomp,
                        jsSourceFile = (Packages.com.google.common.flags, filename = String(fileName), content = String(fileContents), JSSourceFilefromCode.invoke(null, [filename, content])),
                        sourceListArray = new java.util.ArrayList, externList = new java.util.ArrayList,
                        Compiler = Packages.com.google.javascript.jscomp.Compiler,
                        CommandLineRunner = Packages.com.google.javascript.jscomp.CommandLineRunner;
                    for (option in logger.trace("Minifying file: " + fileName), baseName = new java.io.File(fileName).getName(), options = new jscomp.CompilerOptions, config.CompilerOptions) config.CompilerOptions[option] && (options[option] = config.CompilerOptions[option]);
                    if (options.prettyPrint = keepLines || options.prettyPrint, jscomp.CompilationLevel[config.CompilationLevel || "SIMPLE_OPTIMIZATIONS"].setOptionsForCompilationLevel(options), config.generateSourceMaps && ((mappings = new java.util.ArrayList).add(new com.google.javascript.jscomp.SourceMap.LocationMapping(fileName, baseName + ".src.js")), options.setSourceMapLocationMappings(mappings), options.setSourceMapOutputPath(fileName + ".map")), externList.addAll(CommandLineRunner.getDefaultExterns()), config.externExportsPath && (externExportsPath = config.externExportsPath, externList.add(jscomp.SourceFile.fromFile(externExportsPath))), Compiler.setLoggingLevel(Packages.java.util.logging.Level[config.loggingLevel || "WARNING"]), compiler = new Compiler, sourceListArray.add(jsSourceFile), (result = compiler.compile(externList, sourceListArray, options)).success) return optimized = String(compiler.toSource()), fileContents = config.generateSourceMaps && result.sourceMap && outFileName ? (outBaseName = new java.io.File(outFileName).getName(), srcOutFileName = outFileName + ".src.js", outFileNameMap = outFileName + ".map", file.exists(outFileNameMap) ? (concatNameMap = outFileNameMap.replace(/\.map$/, ".src.js.map"), file.saveFile(concatNameMap, file.readFile(outFileNameMap)), file.saveFile(srcOutFileName, fileContents.replace(/\/\# sourceMappingURL=(.+).map/, "/# sourceMappingURL=$1.src.js.map"))) : file.saveUtf8File(srcOutFileName, fileContents), writer = function (fileName, encoding) {
                        var outWriter, parentDir, outFile = new java.io.File(fileName);
                        if (!(parentDir = outFile.getAbsoluteFile().getParentFile()).exists() && !parentDir.mkdirs()) throw"Could not create directory: " + parentDir.getAbsolutePath();
                        return outWriter = encoding ? new java.io.OutputStreamWriter(new java.io.FileOutputStream(outFile), encoding) : new java.io.OutputStreamWriter(new java.io.FileOutputStream(outFile)), new java.io.BufferedWriter(outWriter)
                    }(outFileNameMap, "utf-8"), result.sourceMap.appendTo(writer, outFileName), writer.close(), file.saveFile(outFileNameMap, file.readFile(outFileNameMap).replace(mapRegExp, '"file":"' + baseName + '"')), optimized + "\n//# sourceMappingURL=" + outBaseName + ".map") : optimized;
                    throw new Error("Cannot closure compile file: " + fileName + ". Skipping it.")
                }
            }
        }), "xpconnect" === env && define("xpconnect/optimize", {}), define("optimize", ["lang", "logger", "env!env/optimize", "env!env/file", "parse", "pragma", "uglifyjs", "source-map"], function (lang, logger, envOptimize, file, parse, pragma, uglify, sourceMap) {
            "use strict";
            var optimize, cssImportRegExp = /\@import\s+(url\()?\s*([^);]+)\s*(\))?([\w, ]*)(;)?/gi,
                cssCommentImportRegExp = /\/\*[^\*]*@import[^\*]*\*\//g, cssUrlRegExp = /\url\(\s*([^\)]+)\s*\)?/g,
                protocolRegExp = /^\w+:/;
            sourceMap.SourceMapGenerator, sourceMap.SourceMapConsumer;

            function cleanCssUrlQuotes(url) {
                return "'" !== (url = url.replace(/\s+$/, "")).charAt(0) && '"' !== url.charAt(0) || (url = url.substring(1, url.length - 1)), url
            }

            function fixCssUrlPaths(fileName, path, contents, cssPrefix) {
                return contents.replace(cssUrlRegExp, function (fullMatch, urlMatch) {
                    var firstChar, hasProtocol, parts, i, fixedUrlMatch = cleanCssUrlQuotes(urlMatch);
                    for (firstChar = (fixedUrlMatch = fixedUrlMatch.replace(lang.backSlashRegExp, "/")).charAt(0), hasProtocol = protocolRegExp.test(fixedUrlMatch), "/" === firstChar || "#" === firstChar || hasProtocol ? hasProtocol || logger.trace(fileName + "\n  URL not a relative URL, skipping: " + urlMatch) : urlMatch = cssPrefix + path + fixedUrlMatch, i = (parts = urlMatch.split("/")).length - 1; 0 < i; i--) "." === parts[i] ? parts.splice(i, 1) : ".." === parts[i] && 0 !== i && ".." !== parts[i - 1] && (parts.splice(i - 1, 2), --i);
            ...
            return optimize = {
                jsFile: function (fileName, fileContents, outFileName, config, pluginCollector) {
                    fileContents = fileContents || file.readFile(fileName), fileContents = optimize.js(fileName, fileContents, outFileName, config, pluginCollector), file.saveUtf8File(outFileName, fileContents)
                }, js: function (fileName, fileContents, outFileName, config, pluginCollector) {
                    var optFunc, optConfig, parts = String(config.optimize).split("."), optimizerName = parts[0],
                        keepLines = "keepLines" === parts[1], licenseContents = "";
                    if (config = config || {}, fileContents = pragma.process(fileName, fileContents, config, "OnSave", pluginCollector), optimizerName && "none" !== optimizerName) {
                        if (!(optFunc = envOptimize[optimizerName] || optimize.optimizers[optimizerName])) throw new Error('optimizer with name of "' + optimizerName + '" not found for this environment');
                        optConfig = config[optimizerName] || {}, config.generateSourceMaps && (optConfig.generateSourceMaps = !!config.generateSourceMaps, optConfig._buildSourceMap = config._buildSourceMap);
                        try {
                            if (config.preserveLicenseComments) try {
                            ...
                            config.generateSourceMaps && licenseContents && (optConfig.preamble = licenseContents, licenseContents = ""), fileContents = licenseContents + optFunc(fileName, fileContents, outFileName, keepLines, optConfig), optConfig._buildSourceMap && optConfig._buildSourceMap !== config._buildSourceMap && (config._buildSourceMap = optConfig._buildSourceMap)
                        } catch (e) {
                            if (config.throwWhen && config.throwWhen.optimize) throw e;
                            logger.error(e)
                        }
                    } else config._buildSourceMap && (config._buildSourceMap = null);
                    return fileContents
                }, cssFile: function (fileName, outFileName, config) {
                    var startIndex, endIndex, buildText, comment, originalFileContents = file.readFile(fileName),
                        flat = function flattenCss(fileName, fileContents, cssImportIgnore, cssPrefix, included, topLevel) {
                            var endIndex = (fileName = fileName.replace(lang.backSlashRegExp, "/")).lastIndexOf("/"),
                                filePath = -1 !== endIndex ? fileName.substring(0, endIndex + 1) : "", importList = [],
                                skippedList = [];
                            return fileContents = fileContents.replace(cssCommentImportRegExp, ""), cssImportIgnore && "," !== cssImportIgnore.charAt(cssImportIgnore.length - 1) && (cssImportIgnore += ","), fileContents = fileContents.replace(cssImportRegExp, function (fullMatch, urlStart, importFileName, urlEnd, mediaTypes) {
                                if (mediaTypes && "all" !== mediaTypes.replace(/^\s\s*/, "").replace(/\s\s*$/, "")) return skippedList.push(fileName), fullMatch;
                                if (importFileName = cleanCssUrlQuotes(importFileName), cssImportIgnore && -1 !== cssImportIgnore.indexOf(importFileName + ",")) return fullMatch;
                                importFileName = importFileName.replace(lang.backSlashRegExp, "/");
                                try {
                                    var importEndIndex, importPath, flat,
                                        fullImportFileName = "/" === importFileName.charAt(0) ? importFileName : filePath + importFileName,
                                        importContents = file.readFile(fullImportFileName);
                                    return included[fullImportFileName] ? "" : (included[fullImportFileName] = !0, importContents = (flat = flattenCss(fullImportFileName, importContents, cssImportIgnore, cssPrefix, included)).fileContents, flat.importList.length && importList.push.apply(importList, flat.importList), flat.skippedList.length && skippedList.push.apply(skippedList, flat.skippedList), importPath = -1 !== (importEndIndex = importFileName.lastIndexOf("/")) ? importFileName.substring(0, importEndIndex + 1) : "", importContents = fixCssUrlPaths(importFileName, importPath = importPath.replace(/^\.\//, ""), importContents, cssPrefix), importList.push(fullImportFileName), importContents)
                                } catch (e) {
                                    return logger.warn(fileName + "\n  Cannot inline css import, skipping: " + importFileName), fullMatch
                                }
                            }), cssPrefix && topLevel && (fileContents = fixCssUrlPaths(fileName, "", fileContents, cssPrefix)), {
                                importList: importList,
                                skippedList: skippedList,
                                fileContents: fileContents
                            }
                        }(fileName, originalFileContents, config.cssImportIgnore, config.cssPrefix, {}, !0),
                    ...
                    try {
                        if (-1 === config.optimizeCss.indexOf(".keepComments")) for (startIndex = 0; -1 !== (startIndex = fileContents.indexOf("/*", startIndex));) {
                            if (-1 === (endIndex = fileContents.indexOf("*/", startIndex + 2))) throw"Improper comment in CSS file: " + fileName;
                            comment = fileContents.substring(startIndex, endIndex), startIndex = !config.preserveLicenseComments || -1 === comment.indexOf("license") && -1 === comment.indexOf("opyright") && -1 === comment.indexOf("(c)") ? (fileContents = fileContents.substring(0, startIndex) + fileContents.substring(endIndex + 2, fileContents.length), 0) : endIndex
                        }
                        fileContents = -1 === config.optimizeCss.indexOf(".keepLines") ? (fileContents = (fileContents = (fileContents = fileContents.replace(/[\r\n]/g, " ")).replace(/\s+/g, " ")).replace(/\{\s/g, "{")).replace(/\s\}/g, "}") : (fileContents = fileContents.replace(/(\r\n)+/g, "\r\n")).replace(/(\n)+/g, "\n"), -1 === config.optimizeCss.indexOf(".keepWhitespace") && (fileContents = (fileContents = (fileContents = (fileContents = (fileContents = (fileContents = fileContents.replace(/^[ \t]+/gm, "")).replace(/[ \t]+$/gm, "")).replace(/(;|:|\{|}|,)[ \t]+/g, "$1")).replace(/[ \t]+(\{)/g, "$1")).replace(/([ \t])+/g, "$1")).replace(/^[ \t]*[\r\n]/gm, ""))
                    } catch (e) {
                        fileContents = originalFileContents, logger.error("Could not optimized CSS file: " + fileName + ", error: " + e)
                    }
                    return file.saveUtf8File(outFileName, fileContents), buildText = "\n" + outFileName.replace(config.dir, "") + "\n----------------\n", flat.importList.push(fileName), buildText += flat.importList.map(function (path) {
                        return path.replace(config.dir, "")
                    }).join("\n"), {importList: flat.importList, buildText: buildText + "\n"}
                }, css: function (startDir, config) {
                    var i, fileName, result, fileList, buildText = "", importList = [],
                        shouldRemove = config.dir && config.removeCombined;
                    if (-1 !== config.optimizeCss.indexOf("standard")) {
                        if (fileList = file.getFilteredFileList(startDir, /\.css$/, !0)) for (i = 0; i < fileList.length; i++) fileName = fileList[i], logger.trace("Optimizing (" + config.optimizeCss + ") CSS file: " + fileName), buildText += (result = optimize.cssFile(fileName, fileName, config)).buildText, shouldRemove && (result.importList.pop(), importList = importList.concat(result.importList));
                        shouldRemove && importList.forEach(function (path) {
                            file.exists(path) && file.deleteFile(path)
                        })
                    }
                    return buildText
                }, optimizers: {
                    uglify: function (fileName, fileContents, outFileName, keepLines, config) {
                        var result, existingMap, resultMap, uconfig = {}, existingMapPath = outFileName + ".map",
                            baseName = fileName && fileName.split("/").pop();
                        config = config || {}, lang.mixin(uconfig, config, !0), uconfig.fromString = !0, config.preamble && (uconfig.output = {preamble: config.preamble}), config.generateSourceMaps && (outFileName || config._buildSourceMap) && (uconfig.outSourceMap = baseName + ".map", config._buildSourceMap ? (existingMap = JSON.parse(config._buildSourceMap), uconfig.inSourceMap = existingMap) : file.exists(existingMapPath) && (uconfig.inSourceMap = existingMapPath, existingMap = JSON.parse(file.readFile(existingMapPath)))), logger.trace("Uglify file: " + fileName);
                        try {
                            result = uglify.minify(fileContents, uconfig, baseName + ".src.js"), uconfig.outSourceMap && result.map ? (resultMap = result.map, existingMap || config._buildSourceMap || file.saveFile(outFileName + ".src.js", fileContents), fileContents = result.code, config._buildSourceMap ? config._buildSourceMap = resultMap : file.saveFile(outFileName + ".map", resultMap)) : fileContents = result.code
                        } catch (e) {
                            var errorString = e.toString(), isSyntaxError = /SyntaxError/.test(errorString);
            ...
            var allowRun = !0, hasProp = lang.hasProp, falseProp = lang.falseProp, getOwn = lang.getOwn,
                useStrictRegExp = /['"]use strict['"];/g, absoluteUrlRegExp = /^[\/\\]|^\w:/;
            return prim.hideResolutionConflict = !0, function () {
                if (allowRun) {
                    allowRun = !1;
                    var layer, pluginBuilderRegExp = /(["']?)pluginBuilder(["']?)\s*[=\:]\s*["']([^'"\s]+)["']/,
                        oldNewContext = require.s.newContext, oldDef, exports, module;
                    require._cacheReset = function () {
                        require._cachedRawText = {}, require._cachedFileContents = {}, require._cachedDefinesRequireUrls = {}
                    }, require._cacheReset(), require._isSupportedBuildUrl = function (url) {
                        return -1 === url.indexOf("://") && -1 === url.indexOf("?") && 0 !== url.indexOf("empty:") && 0 !== url.indexOf("//") || (layer.ignoredUrls[url] || (-1 === url.indexOf("empty:") && logger.info("Cannot optimize network URL, skipping: " + url), layer.ignoredUrls[url] = !0), !1)
                    }, require.s.newContext = function (name) {
                        var context = oldNewContext(name), oldEnable = context.enable,
                            moduleProto = context.Module.prototype, oldInit = moduleProto.init,
                            oldCallPlugin = moduleProto.callPlugin;
                        return "_" === name && (context.nextTick = function (fn) {
                            fn()
                        }, context.needFullExec = {}, context.fullExec = {}, context.plugins = {}, context.buildShimExports = {}, context.makeShimExports = function (value) {
                            return context.config.wrapShim ? function () {
                                var str = "return ";
                                return value.exports && -1 === value.exports.indexOf(".") && (str += "root." + value.exports + " = "), value.init && (str += "(" + value.init.toString().replace(useStrictRegExp, "") + ".apply(this, arguments))"), value.init && value.exports && (str += " || "), value.exports && (str += value.exports), str += ";"
                            } : function () {
                            ...
                            var id = depMap.id, parentId = parent && parent.map.id, needFullExec = context.needFullExec,
                                fullExec = context.fullExec, mod = getOwn(context.registry, id);
                            return mod && !mod.defined ? parentId && getOwn(needFullExec, parentId) && (needFullExec[id] = depMap) : (getOwn(needFullExec, id) && falseProp(fullExec, id) || parentId && getOwn(needFullExec, parentId) && falseProp(fullExec, id)) && context.require.undef(id), oldEnable.apply(context, arguments)
                        }, context.load = function (moduleName, url) {
                            var contents, pluginBuilderMatch, builderName, shim, shimExports;
                            0 === url.indexOf("empty:") && delete context.urlFetched[url], require._isSupportedBuildUrl(url) ? (url = normalizeUrlWithBase(context, moduleName, url), layer.buildPathMap[moduleName] = url, layer.buildFileToModule[url] = moduleName, hasProp(context.plugins, moduleName) && (context.needFullExec[moduleName] = !0), prim().start(function () {
                                if (!hasProp(require._cachedFileContents, url) || !falseProp(context.needFullExec, moduleName) && !getOwn(context.fullExec, moduleName)) return require._cacheReadAsync(url).then(function (text) {
                                    contents = text, !context.config.cjsTranslate || context.config.shim && lang.hasProp(context.config.shim, moduleName) || (contents = commonJs.convert(url, contents)), context.config.onBuildRead && (contents = context.config.onBuildRead(moduleName, url, contents)), contents = pragma.process(url, contents, context.config, "OnExecute");
                                    try {
                                        !layer.existingRequireUrl && parse.definesRequire(url, contents) && (layer.existingRequireUrl = url, require._cachedDefinesRequireUrls[url] = !0)
                                    } catch (e1) {
                                        throw new Error("Parse error using esprima for file: " + url + "\n" + e1)
                                    }
                                }).then(function () {
                                    return hasProp(context.plugins, moduleName) && (pluginBuilderMatch = pluginBuilderRegExp.exec(contents)) ? (builderName = context.makeModuleMap(pluginBuilderMatch[3], context.makeModuleMap(moduleName), null, !0).id, require._cacheReadAsync(context.nameToUrl(builderName))) : contents
                                }).then(function (text) {
                                    contents = text;
                                    try {
                                        falseProp(context.needFullExec, moduleName) && (contents = parse(moduleName, url, contents, {
                                            insertNeedsDefine: !0,
                                ...
                                contents = require._cachedFileContents[url], !layer.existingRequireUrl && require._cachedDefinesRequireUrls[url] && (layer.existingRequireUrl = url)
                            }).then(function () {
                                contents && eval(contents);
                                try {
                                    getOwn(context.needFullExec, moduleName) && (shim = getOwn(context.config.shim, moduleName), shim && shim.exports && (shimExports = eval(shim.exports), void 0 !== shimExports && (context.buildShimExports[moduleName] = shimExports))), context.completeLoad(moduleName)
                                } catch (e) {
                                    throw e.moduleTree || (e.moduleTree = []), e.moduleTree.push(moduleName), e
                                }
                            }).then(null, function (eOuter) {
                                throw eOuter.fileName || (eOuter.fileName = url), eOuter
                            }).end()) : context.completeLoad(moduleName)
                        }, context.execCb = function (name, cb, args, exports) {
                            var buildShimExports = getOwn(layer.context.buildShimExports, name);
                            return buildShimExports || (cb.__requireJsBuild || getOwn(layer.context.needFullExec, name) ? cb.apply(exports, args) : void 0)
                        }, moduleProto.init = function (depMaps) {
                            return context.needFullExec[this.map.id] && lang.each(depMaps, lang.bind(this, function (depMap) {
                                "string" == typeof depMap && (depMap = context.makeModuleMap(depMap, this.map.isDefine ? this.map : this.map.parentMap, !1, !0)), context.fullExec[depMap.id] || context.require.undef(depMap.id)
                            })), oldInit.apply(this, arguments)
                        }, moduleProto.callPlugin = function () {
                            var map = this.map, pluginMap = context.makeModuleMap(map.prefix), pluginId = pluginMap.id,
                                pluginMod = getOwn(context.registry, pluginId);
                            return context.plugins[pluginId] = !0, context.needFullExec[pluginId] = map, !falseProp(context.fullExec, pluginId) || pluginMod && !pluginMod.defined || context.require.undef(pluginMap.id), oldCallPlugin.apply(this, arguments)
                        }), context
                    }, delete require.s.contexts._, require._buildReset = function () {
                        var oldContext = require.s.contexts._;
                        return delete require.s.contexts._, require({}), layer = require._layer = {
                            buildPathMap: {},
                            buildFileToModule: {},
                            buildFilePaths: [],
                            pathAdded: {},
                    ...
                    }, require._buildReset(), oldDef = define, define = function (name) {
                        return "string" == typeof name && falseProp(layer.needsDefine, name) && (layer.modulesWithNames[name] = !0), oldDef.apply(require, arguments)
                    }, define.amd = oldDef.amd, require._readFile = file.readFile, require._fileExists = function (path) {
                        return file.exists(path)
                    }, require.onResourceLoad = function (context, map) {
                        var url, id = map.id;
                        context.plugins && lang.hasProp(context.plugins, id) && lang.eachProp(context.needFullExec, function (value, prop) {
                            if (!0 !== value && value.prefix === id && value.unnormalized) {
                                var map = context.makeModuleMap(value.originalName, value.parentMap);
                                context.needFullExec[map.id] = map
                            }
                        }), context.needFullExec && getOwn(context.needFullExec, id) && (context.fullExec[id] = map), map.prefix ? falseProp(layer.pathAdded, id) && (layer.buildFilePaths.push(id), layer.buildPathMap[id] = id, layer.buildFileToModule[id] = id, layer.modulesWithNames[id] = !0, layer.pathAdded[id] = !0) : map.url && require._isSupportedBuildUrl(map.url) && (url = normalizeUrlWithBase(context, id, map.url), !layer.pathAdded[url] && getOwn(layer.buildPathMap, id) && (layer.buildFilePaths.push(url), layer.pathAdded[url] = !0))
                    }, require.needsDefine = function (moduleName) {
                        layer.needsDefine[moduleName] = !0
                    }
                }

                function normalizeUrlWithBase(context, moduleName, url) {
                    return require.jsExtRegExp.test(moduleName) && !absoluteUrlRegExp.test(url) && (url = (context.config.dir || context.config.dirBaseUrl) + url), url
                }
            }
        }), define("commonJs", ["env!env/file", "parse"], function (file, parse) {
            "use strict";
            var commonJs = {
                useLog: !0, convertDir: function (commonJsPath, savePath) {
                    var fileList, i, fileName, convertedFileName, fileContents, jsFileRegExp = /\.js$/;
                    if (fileList = file.getFilteredFileList(commonJsPath, /\w/, !0), commonJsPath = commonJsPath.replace(/\\/g, "/"), savePath = savePath.replace(/\\/g, "/"), "/" === commonJsPath.charAt(commonJsPath.length - 1) && (commonJsPath = commonJsPath.substring(0, commonJsPath.length - 1)), "/" === savePath.charAt(savePath.length - 1) && (savePath = savePath.substring(0, savePath.length - 1)), fileList && fileList.length) for (i = 0; i < fileList.length; i++) convertedFileName = (fileName = fileList[i]).replace(commonJsPath, savePath), jsFileRegExp.test(fileName) ? (fileContents = file.readFile(fileName), fileContents = commonJs.convert(fileName, fileContents), file.saveUtf8File(convertedFileName, fileContents)) : file.copyFile(fileName, convertedFileName, !0); else commonJs.useLog && ("convert" === commonJsPath ? console.log("\n\n" + commonJs.convert(savePath, file.readFile(savePath))) : console.log("No files to convert in directory: " + commonJsPath))
                }, convert: function (fileName, fileContents) {
                    try {
                        var preamble = "", commonJsProps = parse.usesCommonJs(fileName, fileContents);
                        if (parse.usesAmdOrRequireJs(fileName, fileContents) || !commonJsProps) return fileContents;
                        (commonJsProps.dirname || commonJsProps.filename) && (preamble = 'var __filename = module.uri || "", __dirname = __filename.substring(0, __filename.lastIndexOf("/") + 1); '), fileContents = "define(function (require, exports, module) {" + preamble + fileContents + "\n});\n"
                    } catch (e) {
                        return console.log("commonJs.convert: COULD NOT CONVERT: " + fileName + ", so skipping it. Error was: " + e), fileContents
                    }
                    return fileContents
                }
            };
            ...
            "use strict";
            var build, lang = require("lang"), prim = require("prim"), logger = require("logger"),
                file = require("env!env/file"), parse = require("parse"), optimize = require("optimize"),
                pragma = require("pragma"), transform = require("transform"), requirePatch = require("requirePatch"),
                env = require("env"), commonJs = require("commonJs"),
                SourceMapGenerator = require("source-map").SourceMapGenerator, hasProp = lang.hasProp,
                getOwn = lang.getOwn, falseProp = lang.falseProp, endsWithSemiColonRegExp = /;\s*$/,
                endsWithSlashRegExp = /[\/\\]$/, resourceIsModuleIdRegExp = /^[\w\/\\\.]+$/,
            ...
            function addSemiColon(text, config) {
                return config.skipSemiColonInsertion || endsWithSemiColonRegExp.test(text) ? text : text + ";"
            }
            ...
            function makeWriteFile(namespace, layer) {
                function writeFile(name, contents) {
                    logger.trace("Saving plugin-optimized file: " + name), file.saveUtf8File(name, contents)
                }

                return writeFile.asModule = function (moduleName, fileName, contents) {
            ...
            function appendToFileContents(fileContents, singleContents, path, config, module, sourceMapGenerator) {
                ...
                if (sourceMapGenerator) {
                    for (refPath = config.out ? config.baseUrl : module && module._buildPath ? module._buildPath : "", sourceMapPath = 1 === (parts = path.split("!")).length ? build.makeRelativeFilePath(refPath, path) : (pluginId = parts.shift(), resourcePath = parts.join("!"), resourceIsModuleIdRegExp.test(resourcePath) ? build.makeRelativeFilePath(refPath, require.toUrl(resourcePath)) + "!" + pluginId : path), sourceMapLineNumber = fileContents.split("\n").length - 1, lineCount = singleContents.split("\n").length, i = 1; i <= lineCount; i += 1) sourceMapGenerator.addMapping({
                        generated: {
                            line: sourceMapLineNumber + i,
                            column: 0
                        }, original: {line: i, column: 0}, source: sourceMapPath
                    });
                    sourceMapGenerator.setSourceContent(sourceMapPath, singleContents)
                }
                return fileContents += singleContents
            }

            function stringDotToObj(result, name, value) {
                var parts = name.split(".");
                parts.forEach(function (prop, i) {
                    i === parts.length - 1 ? result[prop] = value : (falseProp(result, prop) && (result[prop] = {}), result = result[prop])
                })
            }

            function mixConfig(target, source, skipArrays) {
                var prop, value, isArray, targetValue;
                for (prop in source) hasProp(source, prop) && (value = source[prop], isArray = lang.isArray(value), "object" != typeof value || !value || isArray || lang.isFunction(value) || lang.isRegExp(value) ? isArray ? skipArrays || (targetValue = target[prop], lang.isArray(targetValue) ? target[prop] = targetValue.concat(value) : target[prop] = value) : target[prop] = value : "map" === prop ? (target.map || (target.map = {}), lang.deepMix(target.map, source.map)) : target[prop] = lang.mixin({}, target[prop], value, !0));
                lang.hasProp(target, "logLevel") && logger.logLevel(target.logLevel)
            }

            function flattenWrapFile(config, keyName, absFilePath) {
                var wrap = config.wrap, keyFileName = keyName + "File", keyMapName = "__" + keyName + "Map";
                if ("string" != typeof wrap[keyName] && wrap[keyFileName]) wrap[keyName] = "", "string" == typeof wrap[keyFileName] && (wrap[keyFileName] = [wrap[keyFileName]]), wrap[keyMapName] = [], wrap[keyFileName].forEach(function (fileName) {
                ...
                }); else if (null === wrap[keyName] || void 0 === wrap[keyName]) wrap[keyName] = ""; else {
                    if ("string" != typeof wrap[keyName]) throw new Error("wrap." + keyName + " or wrap." + keyFileName + " malformed");
                    wrap[keyName] = endsWithNewLine(wrap[keyName]), wrap[keyMapName] = [function (fileContents, cfg, sourceMapGenerator) {
            ...
            function normalizeWrapConfig(config, absFilePath) {
                try {
                    config.wrap && (!0 === config.wrap ? config.wrap = {
                        start: "(function () {\n",
                        end: "}());",
                    ...
                    } : (flattenWrapFile(config, "start", absFilePath), flattenWrapFile(config, "end", absFilePath)))
                } catch (wrapError) {
                    throw new Error("Malformed wrap config: " + wrapError.toString())
                }
            }

            return prim.nextTick = function (fn) {
                fn()
            }, require = requirejs, require._cacheReadAsync = function (path, encoding) {
                var d;
                return lang.hasProp(require._cachedRawText, path) ? ((d = prim()).resolve(require._cachedRawText[path]), d.promise) : file.readFileAsync(path, encoding).then(function (text) {
                    return require._cachedRawText[path] = text
                })
            }, build = function (args) {
                var buildFile, cmdConfig, errorMsg, errorStack, stackMatch, errorTree, i, j, errorMod,
                    stackRegExp = /( {4}at[^\n]+)\n/;
                return prim().start(function () {
                    if (!args || lang.isArray(args)) {
                        if (!args || args.length < 1) return void logger.error("build.js buildProfile.js\nwhere buildProfile.js is the name of the build file (see example.build.js for hints on how to make a build file).");
                        -1 === args[0].indexOf("=") && (buildFile = args[0], args.splice(0, 1)), (cmdConfig = build.convertArrayToObject(args)).buildFile = buildFile
                    } else cmdConfig = args;
                    return build._run(cmdConfig)
                }).then(null, function (e) {
                    var err;
                    if (errorMsg = e.toString(), errorTree = e.moduleTree, (stackMatch = stackRegExp.exec(errorMsg)) && (errorMsg += errorMsg.substring(0, stackMatch.index + stackMatch[0].length + 1)), errorTree && 0 < errorTree.length) {
                        for (errorMsg += "\nIn module tree:\n", i = errorTree.length - 1; -1 < i; i--) if (errorMod = errorTree[i]) {
                            for (j = errorTree.length - i; -1 < j; j--) errorMsg += "  ";
                            errorMsg += errorMod + "\n"
                        }
                        logger.error(errorMsg)
                    }
                    throw errorStack = e.stack, "string" == typeof args && -1 !== args.indexOf("stacktrace=true") ? errorMsg += "\n" + errorStack : !stackMatch && errorStack && (stackMatch = stackRegExp.exec(errorStack)) && (errorMsg += "\n" + stackMatch[0] || ""), (err = new Error(errorMsg)).originalError = e, err
                })
            }, build._run = function (cmdConfig) {
                var buildPaths, fileName, paths, i, baseConfig, config, modules, srcPath, buildContext, destPath,
                    moduleMap, parentModuleMap, context, resources, resource, plugin, fileContents,
                    pluginProcessed = {}, buildFileContents = "", pluginCollector = {};
                return prim().start(function () {
                    var prop;
                    if (requirePatch(), config = build.createConfig(cmdConfig), paths = config.paths, config.dir && !config.keepBuildDir && file.exists(config.dir) && file.deleteFile(config.dir), !config.out && !config.cssIn) if (file.copyDir(config.appDir || config.baseUrl, config.dir, /\w/, !0), buildPaths = {}, config.appDir) for (prop in paths) hasProp(paths, prop) && (buildPaths[prop] = paths[prop].replace(config.appDir, config.dir)); else for (prop in paths) hasProp(paths, prop) && (0 === paths[prop].indexOf(config.baseUrl) ? buildPaths[prop] = paths[prop].replace(config.baseUrl, config.dirBaseUrl) : (buildPaths[prop] = "empty:" === paths[prop] ? "empty:" : prop, 0 !== (srcPath = paths[prop]).indexOf("/") && -1 === srcPath.indexOf(":") && (srcPath = config.baseUrl + srcPath), destPath = config.dirBaseUrl + buildPaths[prop], "empty:" !== srcPath && (file.exists(srcPath) && file.isDirectory(srcPath) ? file.copyDir(srcPath, destPath, /\w/, !0) : (srcPath += ".js", destPath += ".js", file.copyFile(srcPath, destPath)))));
                    require({
                        baseUrl: config.baseUrl,
                        paths: paths,
                        packagePaths: config.packagePaths,
                        packages: config.packages
                    }), buildContext = require.s.contexts._, (modules = config.modules) && modules.forEach(function (module) {
                        if (module.name && (module._sourcePath = buildContext.nameToUrl(module.name), !(file.exists(module._sourcePath) || module.create || -1 !== module.name.indexOf("!") || config.rawText && lang.hasProp(config.rawText, module.name)))) throw new Error("ERROR: module path does not exist: " + module._sourcePath + " for module named: " + module.name + ". Path is relative to: " + file.absPath("."))
                    }), config.out ? (require(config), config.cssIn || (config.modules[0]._buildPath = "function" == typeof config.out ? "FUNCTION" : config.out)) : config.cssIn || (baseConfig = {
                        baseUrl: config.dirBaseUrl,
                        paths: buildPaths
                    }, lang.mixin(baseConfig, config), require(baseConfig), modules && modules.forEach(function (module) {
                        if (module.name) {
                            if (module._buildPath = buildContext.nameToUrl(module.name, null), module._buildPath === module._sourcePath && !config.allowSourceOverwrites) throw new Error("Module ID '" + module.name + "' has a source path that is same as output path: " + module._sourcePath + ". Stopping, config is malformed.");
                            module.create || config.rawText && lang.hasProp(config.rawText, module.name) || file.copyFile(module._sourcePath, module._buildPath)
                        }
                    })), config.optimizeCss && "none" !== config.optimizeCss && config.dir && (buildFileContents += optimize.css(config.dir, config))
                }).then(function () {
                    baseConfig = copyConfig(require.s.contexts._.config)
                }).then(function () {
                    var actions = [];
                    if (modules) return actions = modules.map(function (module, i) {
                        return function () {
                            return config._buildPathToModuleIndex[file.normalize(module._buildPath)] = i, build.traceDependencies(module, config, baseConfig).then(function (layer) {
                                module.layer = layer
                            })
                        }
                    }), prim.serial(actions)
                    ...
                    var actions;
                    if (modules) return actions = modules.map(function (module) {
                        return function () {
                            if (module.exclude) return module.excludeLayers = [], prim.serial(module.exclude.map(function (exclude, i) {
                                return function () {
                                    var found = build.findBuildModule(exclude, modules);
                                    if (!found) return build.traceDependencies({name: exclude}, config, baseConfig).then(function (layer) {
                    ...
                    if (modules) return prim.serial(modules.map(function (module) {
                        return function () {
                            return module.exclude && module.exclude.forEach(function (excludeModule, i) {
                                var excludeLayer = module.excludeLayers[i].layer, map = excludeLayer.buildFileToModule;
                                excludeLayer.buildFilePaths.forEach(function (filePath) {
                                    build.removeModulePath(map[filePath], filePath, module.layer)
                                })
                            }), module.excludeShallow && module.excludeShallow.forEach(function (excludeShallowModule) {
                                var path = getOwn(module.layer.buildPathMap, excludeShallowModule);
                                path && build.removeModulePath(excludeShallowModule, path, module.layer)
                            }), build.flattenModule(module, module.layer, config).then(function (builtModule) {
                                var finalText;
                                "FUNCTION" === module._buildPath ? (module._buildText = builtModule.text, module._buildSourceMap = builtModule.sourceMap) : (finalText = builtModule.text, builtModule.sourceMap && (finalText += "\n//# sourceMappingURL=" + module._buildPath.split("/").pop() + ".map", file.saveUtf8File(module._buildPath + ".map", builtModule.sourceMap)), file.saveUtf8File(module._buildPath + "-temp", finalText)), buildFileContents += builtModule.buildText
                            })
                        }
                    }))
                }).then(function () {
                    var moduleName, outOrigSourceMap, bundlesConfig = {},
                        bundlesConfigOutFile = config.bundlesConfigOutFile;
                    if (modules && (modules.forEach(function (module) {
                        var entryConfig, finalPath = module._buildPath;
                        if ("FUNCTION" !== finalPath) {
                            if (file.exists(finalPath) && file.deleteFile(finalPath), file.renameFile(finalPath + "-temp", finalPath), bundlesConfigOutFile) {
                                entryConfig = bundlesConfig[module.name] = [];
                                var bundleContents = file.readFile(finalPath), excludeMap = {};
                                excludeMap[module.name] = !0;
                                var parsedIds = parse.getAllNamedDefines(bundleContents, excludeMap);
                                entryConfig.push.apply(entryConfig, parsedIds)
                            }
                            config.removeCombined && !config.out && module.layer.buildFilePaths.forEach(function (path) {
                                var isLayer = modules.some(function (mod) {
                                    return mod._buildPath === path
                                }), relPath = build.makeRelativeFilePath(config.dir, path);
                                file.exists(path) && !isLayer && 0 !== relPath.indexOf("..") && file.deleteFile(path)
                            })
                        }
                        config.onModuleBundleComplete && config.onModuleBundleComplete(module.onCompleteData)
                    }), bundlesConfigOutFile)) {
                        var text = file.readFile(bundlesConfigOutFile);
                        text = transform.modifyConfig(text, function (config) {
                            return config.bundles || (config.bundles = {}), lang.eachProp(bundlesConfig, function (value, prop) {
                                config.bundles[prop] = value
                            }), config
                        }), file.saveUtf8File(bundlesConfigOutFile, text)
                    }
                    if (config.removeCombined && !config.out && config.dir && file.deleteEmptyDirs(config.dir), config.out && !config.cssIn) "FUNCTION" === (fileName = config.modules[0]._buildPath) ? (outOrigSourceMap = config.modules[0]._buildSourceMap, config._buildSourceMap = outOrigSourceMap, config.modules[0]._buildText = optimize.js((config.modules[0].name || config.modules[0].include[0] || fileName) + ".build.js", config.modules[0]._buildText, null, config), config._buildSourceMap && config._buildSourceMap !== outOrigSourceMap && (config.modules[0]._buildSourceMap = config._buildSourceMap, config._buildSourceMap = null)) : optimize.jsFile(fileName, null, fileName, config); else if (!config.cssIn) {
                        for (moduleName in file.getFilteredFileList(config.dir, /\.js$/, !0).forEach(function (fileName) {
                            var cfg, override, moduleIndex;
                            moduleName = (moduleName = fileName.replace(config.dir, "")).substring(0, moduleName.length - 3), (-1 < (moduleIndex = 0 === (moduleIndex = getOwn(config._buildPathToModuleIndex, fileName)) || 0 < moduleIndex ? moduleIndex : -1) || !config.skipDirOptimize || "all" === config.normalizeDirDefines || config.cjsTranslate) && (fileContents = file.readFile(fileName), !config.cjsTranslate || config.shim && lang.hasProp(config.shim, moduleName) || (fileContents = commonJs.convert(fileName, fileContents)), -1 === moduleIndex && (config.onBuildRead && (fileContents = config.onBuildRead(moduleName, fileName, fileContents)), "all" === config.normalizeDirDefines && (fileContents = build.toTransport(config.namespace, null, fileName, fileContents)), config.onBuildWrite && (fileContents = config.onBuildWrite(moduleName, fileName, fileContents))), cfg = (override = -1 < moduleIndex ? config.modules[moduleIndex].override : null) ? build.createOverrideConfig(config, override) : config, (-1 < moduleIndex || !config.skipDirOptimize) && optimize.jsFile(fileName, fileContents, fileName, cfg, pluginCollector))
                        }), context = require.s.contexts._, pluginCollector) if (hasProp(pluginCollector, moduleName)) for (parentModuleMap = context.makeModuleMap(moduleName), resources = pluginCollector[moduleName], i = 0; i < resources.length; i++) {
                            if (resource = resources[i], moduleMap = context.makeModuleMap(resource, parentModuleMap), falseProp(context.plugins, moduleMap.prefix)) {
                                if (context.plugins[moduleMap.prefix] = !0, !file.exists(require.toUrl(moduleMap.prefix + ".js"))) continue;
                                context.require([moduleMap.prefix]), moduleMap = context.makeModuleMap(resource, parentModuleMap)
                            }
                            falseProp(pluginProcessed, moduleMap.id) && ((plugin = getOwn(context.defined, moduleMap.prefix)) && plugin.writeFile && plugin.writeFile(moduleMap.prefix, moduleMap.name, require, makeWriteFile(config.namespace), context.config), pluginProcessed[moduleMap.id] = !0)
                        }
                        config.writeBuildTxt && file.saveUtf8File(config.dir + "build.txt", buildFileContents)
                    }
                    return config.cssIn && (buildFileContents += optimize.cssFile(config.cssIn, config.out, config).buildText), "function" == typeof config.out && config.out(config.modules[0]._buildText, config.modules[0]._buildSourceMap), buildFileContents ? (logger.info(buildFileContents), buildFileContents) : ""
                })
            }, build.objProps = {
                paths: !0,
                wrap: !0,
                ...
                return -1 !== index && (dotProp = prop.substring(0, index), hasProp(build.objProps, dotProp))
            }, build.convertArrayToObject = function (ary) {
                var i, separatorIndex, prop, value, result = {}, needArray = {
                    include: !0,
                    exclude: !0,
                    excludeShallow: !0,
                    insertRequire: !0,
                ...
                for (i = 0; i < ary.length; i++) {
                    if (-1 === (separatorIndex = ary[i].indexOf("="))) throw"Malformed name/value pair: [" + ary[i] + "]. Format should be name=value";
                    "true" === (value = ary[i].substring(separatorIndex + 1, ary[i].length)) ? value = !0 : "false" === value && (value = !1), prop = ary[i].substring(0, separatorIndex), getOwn(needArray, prop) && (value = value.split(",")), build.hasDotPropMatch(prop) ? stringDotToObj(result, prop, value) : result[prop] = value
                }
                return result
            }, build.makeAbsPath = function (path, absFilePath) {
                return absFilePath ? (0 !== path.indexOf("/") && -1 === path.indexOf(":") && (path = absFilePath + ("/" === absFilePath.charAt(absFilePath.length - 1) ? "" : "/") + path, path = file.normalize(path)), path.replace(lang.backSlashRegExp, "/")) : path
            }, build.makeAbsObject = function (props, obj, absFilePath) {
                var i, prop;
                if (obj) for (i = 0; i < props.length; i++) prop = props[i], hasProp(obj, prop) && "string" == typeof obj[prop] && (obj[prop] = build.makeAbsPath(obj[prop], absFilePath))
            }, build.makeAbsConfig = function (config, absFilePath) {
                var props, prop, i;
                for (props = ["appDir", "dir", "baseUrl"], i = 0; i < props.length; i++) getOwn(config, prop = props[i]) && ("baseUrl" === prop ? (config.originalBaseUrl = config.baseUrl, config.appDir ? config.baseUrl = build.makeAbsPath(config.originalBaseUrl, config.appDir) : config.baseUrl = build.makeAbsPath(config[prop], absFilePath)) : config[prop] = build.makeAbsPath(config[prop], absFilePath), config[prop] = endsWithSlash(config[prop]));
                build.makeAbsObject("stdout" === config.out ? ["cssIn"] : ["out", "cssIn"], config, absFilePath), build.makeAbsObject(["startFile", "endFile"], config.wrap, absFilePath), build.makeAbsObject(["externExportsPath"], config.closure, absFilePath)
                ...
                var i, dotLength, finalParts, length, targetParts, targetName, refParts = refPath.split("/"),
                    hasEndSlash = endsWithSlashRegExp.test(targetPath), dotParts = [];
                for (targetPath = file.normalize(targetPath), hasEndSlash && !endsWithSlashRegExp.test(targetPath) && (targetPath += "/"), targetName = (targetParts = targetPath.split("/")).pop(), refParts.pop(), length = refParts.length, i = 0; i < length && refParts[i] === targetParts[i]; i += 1) ;
                for (finalParts = targetParts.slice(i), dotLength = length - i, i = 0; -1 < i && i < dotLength; i += 1) dotParts.push("..");
                ...
                var buildFileContents, buildFileConfig, mainConfig, mainConfigFile, mainConfigPath, buildFile,
                    absFilePath, config = {}, buildBaseConfig = makeBuildBaseConfig();
                if (absFilePath = file.absPath("."), build.makeAbsConfig(cfg, absFilePath), build.makeAbsConfig(buildBaseConfig, absFilePath), lang.mixin(config, buildBaseConfig), lang.mixin(config, cfg, !0), lang.hasProp(config, "logLevel") && logger.logLevel(config.logLevel), config.buildFile) {
                    if (buildFile = file.absPath(config.buildFile), !file.exists(buildFile)) throw new Error("ERROR: build file does not exist: " + buildFile);
                    absFilePath = config.baseUrl = file.absPath(file.parent(buildFile)), buildFileContents = file.readFile(buildFile);
                    try {
                        buildFileContents = buildFileContents.replace(/\/\/\#[^\n\r]+[\n\r]*$/, "").trim().replace(/;$/, ""), buildFileConfig = eval("(" + buildFileContents + ")"), build.makeAbsConfig(buildFileConfig, absFilePath), mixConfig(config, buildFileConfig)
                    } catch (e) {
                        throw new Error("Build file " + buildFile + " is malformed: " + e)
                    }
                }
                if (mainConfigFile = config.mainConfigFile || buildFileConfig && buildFileConfig.mainConfigFile, mainConfigFile && ("string" == typeof mainConfigFile && (mainConfigFile = [mainConfigFile]), mainConfigFile.forEach(function (configFile) {
                    if (configFile = build.makeAbsPath(configFile, absFilePath), !file.exists(configFile)) throw new Error(configFile + " does not exist.");
                    try {
                    ...
                    mainConfig && (mainConfigPath = configFile.substring(0, configFile.lastIndexOf("/")), config.appDir && !mainConfig.appDir && (mainConfig.appDir = config.appDir), mainConfig.baseUrl || (mainConfig.baseUrl = mainConfigPath), build.makeAbsConfig(mainConfig, mainConfigPath), mixConfig(config, mainConfig))
                })), buildFileConfig && mixConfig(config, buildFileConfig, !0), mixConfig(config, cfg, !0), lang.eachProp(config.paths, function (value, prop) {
                    if (lang.isArray(value)) throw new Error("paths fallback not supported in optimizer. Please provide a build config path override for " + prop);
                    config.paths[prop] = build.makeAbsPath(value, config.baseUrl)
                }), hasProp(config, "baseUrl")) {
                    if (config.appDir) {
                        if (!config.originalBaseUrl) throw new Error("Please set a baseUrl in the build config");
                        config.dirBaseUrl = build.makeAbsPath(config.originalBaseUrl, config.dir)
                    } else config.dirBaseUrl = config.dir || config.baseUrl;
                    config.dirBaseUrl = endsWithSlash(config.dirBaseUrl)
                }
                if (config.bundlesConfigOutFile) {
                    if (!config.dir) throw new Error('bundlesConfigOutFile can only be used with optimizations that use "dir".');
                    config.bundlesConfigOutFile = build.makeAbsPath(config.bundlesConfigOutFile, config.dir)
                }
                if (config.out && "stdout" === config.out && (config.out = function (content) {
                    var e = env.get();
                    "rhino" === e ? new java.io.PrintStream(java.lang.System.out, !0, "UTF-8").println(content) : "node" === e ? process.stdout.write(content, "utf8") : console.log(content)
                }), config.main) throw new Error('"main" passed as an option, but the supported option is called "name".');
                ...
                if (config.cssIn) {
                    if (config.dir || config.appDir) throw new Error('cssIn is only for the output of single file CSS optimizations and is not compatible with "dir" or "appDir" configuration.');
                    if (!config.out) throw new Error('"out" option missing.')
                }
                if (config.cssIn || config.baseUrl || (config.baseUrl = "./"), !config.out && !config.dir) throw new Error('Missing either an "out" or "dir" config value. If using "appDir" for a full project optimization, use "dir". If you want to optimize to one file, use "out".');
                ...
                if ("uglify2" === config.optimize && (config.optimize = "uglify"), config.uglify2 && (config.uglify = config.uglify2, delete config.uglify2), config.generateSourceMaps) {
                    if (config.preserveLicenseComments && "none" !== config.optimize && "uglify" !== config.optimize) throw new Error("Cannot use preserveLicenseComments and generateSourceMaps together, unless optimize is set to 'uglify'. Either explicitly set preserveLicenseComments to false (default is true) or turn off generateSourceMaps. If you want source maps with license comments, see: http://requirejs.org/docs/errors.html#sourcemapcomments");
                    if ("none" !== config.optimize && "closure" !== config.optimize && "uglify" !== config.optimize) throw new Error('optimize: "' + config.optimize + '" does not support generateSourceMaps.')
                }
                if (!config.name && !config.include || config.modules) {
                    if (config.modules && config.out) throw new Error('If the "modules" option is used, then there should be a "dir" option set and "out" should not be used since "out" is only for single file optimization output.');
                    if (config.modules && config.name) throw new Error('"name" and "modules" options are incompatible. Either use "name" if doing a single file optimization, or "modules" if you want to target more than one file for optimization.')
                } else config.modules = [{
                    name: config.name,
                    out: config.out,
                ...
                if (config.out && !config.cssIn && (cfg.optimizeCss || (config.optimizeCss = "none")), config.cssPrefix ? config.cssPrefix = endsWithSlash(config.cssPrefix) : config.cssPrefix = "", config.modules && config.modules.length && config.modules.forEach(function (mod) {
                    if (lang.isArray(mod) || "string" == typeof mod || !mod) throw new Error("modules config item is malformed: it should be an object with a 'name' property.");
                    config.stubModules && (mod.stubModules = config.stubModules.concat(mod.stubModules || [])), mod.stubModules && (mod.stubModules._byName = {}, mod.stubModules.forEach(function (id) {
                        mod.stubModules._byName[id] = !0
                    })), "string" == typeof mod.include && (mod.include = [mod.include]), mod.override && normalizeWrapConfig(mod.override, absFilePath)
                }), normalizeWrapConfig(config, absFilePath), config.context) throw new Error('The build argument "context" is not supported in a build. It should only be used in web pages.');
                return hasProp(config, "normalizeDirDefines") || ("none" === config.optimize || config.skipDirOptimize ? config.normalizeDirDefines = "skip" : config.normalizeDirDefines = "all"), hasProp(config, "fileExclusionRegExp") ? "string" == typeof config.fileExclusionRegExp ? file.exclusionRegExp = new RegExp(config.fileExclusionRegExp) : file.exclusionRegExp = config.fileExclusionRegExp : hasProp(config, "dirExclusionRegExp") && (file.exclusionRegExp = config.dirExclusionRegExp), config.deps && (config._depsInclude = config.deps), delete config.deps, delete config.jQuery, delete config.enforceDefine, delete config.urlArgs, config
            }, build.findBuildModule = function (moduleName, modules) {
                var i, module;
                for (i = 0; i < modules.length; i++) if ((module = modules[i]).name === moduleName) return module;
                return null
            }, build.removeModulePath = function (module, path, layer) {
                var index = layer.buildFilePaths.indexOf(path);
                -1 !== index && layer.buildFilePaths.splice(index, 1)
            }, build.traceDependencies = function (module, config, baseLoaderConfig) {
                var include, override, layer, context, rawTextByIds, syncChecks = {rhino: !0, node: !0, xpconnect: !0},
                    deferred = prim();

                function includeFinished(value) {
                    var hasError = !1;
                    if (syncChecks[env.get()]) try {
                        build.checkForErrors(context, layer)
                    } catch (e) {
                        hasError = !0, deferred.reject(e)
                    }
                    hasError || deferred.resolve(value)
                }

                return require._buildReset(), layer = require._layer, context = layer.context, baseLoaderConfig && require(copyConfig(baseLoaderConfig)), logger.trace("\nTracing dependencies for: " + (module.name || ("function" == typeof module.out ? "FUNCTION" : module.out))), include = (include = config._depsInclude || []).concat(module.name && !module.create ? [module.name] : []), module.include && (include = include.concat(module.include)), module.override && (override = baseLoaderConfig ? build.createOverrideConfig(baseLoaderConfig, module.override) : copyConfig(module.override), require(override)), (rawTextByIds = require.s.contexts._.config.rawText) && lang.eachProp(rawTextByIds, function (contents, id) {
                    var url = require.toUrl(id) + ".js";
                    require._cachedRawText[url] = contents
                }), deferred.reject.__requireJsBuild = !0, includeFinished.__requireJsBuild = !0, require(include, includeFinished, deferred.reject), syncChecks[env.get()] && build.checkForErrors(context, layer), deferred.promise.then(function () {
                    return module.override && baseLoaderConfig && require(copyConfig(baseLoaderConfig)), build.checkForErrors(context, layer), layer
                })
            }, build.checkForErrors = function (context, layer) {
                var id, prop, mod, idParts, pluginId, errMessage = "", failedPluginMap = {}, failedPluginIds = [],
                    errIds = [], errUrlMap = {}, errUrlConflicts = {}, hasErrUrl = !1, hasUndefined = !1,
                    defined = context.defined, registry = context.registry;

                function populateErrUrlMap(id, errUrl, skipNew) {
                    errUrl && (skipNew || errIds.push(id), errUrlMap[errUrl] ? (hasErrUrl = !0, errUrlConflicts[errUrl] || (errUrlConflicts[errUrl] = [], errUrlConflicts[errUrl].push(errUrlMap[errUrl])), errUrlConflicts[errUrl].push(id)) : skipNew || (errUrlMap[errUrl] = id))
                }

                for (id in registry) hasProp(registry, id) && 0 !== id.indexOf("_@r") && (hasUndefined = !0, mod = getOwn(registry, id), pluginId = (idParts = id.split("!"))[0], -1 === id.indexOf("_unnormalized") && mod && mod.enabled && populateErrUrlMap(id, mod.map.url), !hasProp(layer.modulesWithNames, id) && 1 < idParts.length && (falseProp(failedPluginMap, pluginId) && failedPluginIds.push(pluginId), (failedPluginMap[pluginId] || (failedPluginMap[pluginId] = [])).push(id + (mod.error ? ": " + mod.error : ""))));
                if (hasUndefined) for (id in defined) hasProp(defined, id) && -1 === id.indexOf("!") && populateErrUrlMap(id, require.toUrl(id) + ".js", !0);
                if (errIds.length || failedPluginIds.length) {
                    if (failedPluginIds.length && (errMessage += "Loader plugin" + (1 === failedPluginIds.length ? "" : "s") + " did not call the load callback in the build:\n" + failedPluginIds.map(function (pluginId) {
                ...
                var cfg = copyConfig(config), oride = copyConfig(override);
                return lang.eachProp(oride, function (value, prop) {
                    hasProp(build.objProps, prop) ? (cfg[prop] = {}, lang.mixin(cfg[prop], config[prop], !0), lang.mixin(cfg[prop], override[prop], !0)) : cfg[prop] = override[prop]
                }), cfg
            }, build.flattenModule = function (module, layer, config) {
                var fileContents, sourceMapGenerator, sourceMapBase, buildFileContents = "";
                return prim().start(function () {
                    var reqIndex, currContents, fileForSourceMap, moduleName, shim, packageName, parts, builder,
                        writeApi, namespace, namespaceWithDot, stubModulesByName, context = layer.context,
                        onLayerEnds = [], onLayerEndAdded = {}, pkgsMainMap = {};
                    return module.override && (config = build.createOverrideConfig(config, module.override)), namespace = config.namespace || "", namespaceWithDot = namespace ? namespace + "." : "", stubModulesByName = module.stubModules && module.stubModules._byName || {}, module.onCompleteData = {
                        name: module.name,
                        path: config.dir ? module._buildPath.replace(config.dir, "") : module._buildPath,
                        included: []
                    }, buildFileContents += "\n" + module.onCompleteData.path + "\n----------------\n", layer.existingRequireUrl && -1 !== (reqIndex = layer.buildFilePaths.indexOf(layer.existingRequireUrl)) && (layer.buildFilePaths.splice(reqIndex, 1), layer.buildFilePaths.unshift(layer.existingRequireUrl)), config.generateSourceMaps && (sourceMapBase = config.dir || config.baseUrl, fileForSourceMap = "FUNCTION" === module._buildPath ? (module.name || module.include[0] || "FUNCTION") + ".build.js" : config.out ? module._buildPath.split("/").pop() : module._buildPath.replace(sourceMapBase, ""), sourceMapGenerator = new SourceMapGenerator({file: fileForSourceMap})), lang.eachProp(layer.context.config.pkgs, function (value, prop) {
                        pkgsMainMap[value] = prop
                    }), fileContents = "", config.wrap && config.wrap.__startMap && config.wrap.__startMap.forEach(function (wrapFunction) {
                        fileContents = wrapFunction(fileContents, config, sourceMapGenerator)
                    }), prim.serial(layer.buildFilePaths.map(function (path) {
                        return function () {
                            var singleContents = "";
                            return moduleName = layer.buildFileToModule[path], packageName = getOwn(pkgsMainMap, moduleName), prim().start(function () {
                                return parts = context.makeModuleMap(moduleName), (builder = parts.prefix && getOwn(context.defined, parts.prefix)) ? (builder.onLayerEnd && falseProp(onLayerEndAdded, parts.prefix) && (onLayerEnds.push(builder), onLayerEndAdded[parts.prefix] = !0), void (builder.write && ((writeApi = function (input) {
                                    singleContents += "\n" + addSemiColon(input, config), config.onBuildWrite && (singleContents = config.onBuildWrite(moduleName, path, singleContents))
                                }).asModule = function (moduleName, input) {
                                    singleContents += "\n" + addSemiColon(build.toTransport(namespace, moduleName, path, input, layer, {useSourceUrl: layer.context.config.useSourceUrl}), config), config.onBuildWrite && (singleContents = config.onBuildWrite(moduleName, path, singleContents))
                                }, builder.write(parts.prefix, parts.name, writeApi, {
                                    name: module.onCompleteData.name,
                                    path: module.onCompleteData.path
                                })))) : prim().start(function () {
                                    return hasProp(stubModulesByName, moduleName) ? hasProp(layer.context.plugins, moduleName) ? 'define({load: function(id){throw new Error("Dynamic load not allowed: " + id);}});' : "define({});" : require._cacheReadAsync(path)
                                }).then(function (text) {
                                    var hasPackageName;
                                    currContents = text, !config.cjsTranslate || config.shim && lang.hasProp(config.shim, moduleName) || (currContents = commonJs.convert(path, currContents)), config.onBuildRead && (currContents = config.onBuildRead(moduleName, path, currContents)), packageName && (hasPackageName = packageName === parse.getNamedDefine(currContents)), namespace && (currContents = pragma.namespace(currContents, namespace)), currContents = build.toTransport(namespace, moduleName, path, currContents, layer, {useSourceUrl: config.useSourceUrl}), packageName && !hasPackageName && (currContents = addSemiColon(currContents, config) + "\n", currContents += namespaceWithDot + "define('" + packageName + "', ['" + moduleName + "'], function (main) { return main; });\n"), config.onBuildWrite && (currContents = config.onBuildWrite(moduleName, path, currContents)), singleContents += addSemiColon(currContents, config)
                                })
                            }).then(function () {
                                var shimDeps, shortPath = path.replace(config.dir, "");
                                module.onCompleteData.included.push(shortPath), buildFileContents += shortPath + "\n", moduleName && falseProp(layer.modulesWithNames, moduleName) && !config.skipModuleInsertion && ((shim = config.shim && (getOwn(config.shim, moduleName) || packageName && getOwn(config.shim, packageName))) ? (shimDeps = lang.isArray(shim) ? shim : shim.deps, config.wrapShim ? singleContents = "(function(root) {\n" + namespaceWithDot + 'define("' + moduleName + '", ' + (shimDeps && shimDeps.length ? build.makeJsArrayString(shimDeps) + ", " : "[], ") + "function() {\n  return (function() {\n" + singleContents + "\n" + (shim.exportsFn ? shim.exportsFn() : "") + "\n  }).apply(root, arguments);\n});\n}(this));\n" : singleContents += "\n" + namespaceWithDot + 'define("' + moduleName + '", ' + (shimDeps && shimDeps.length ? build.makeJsArrayString(shimDeps) + ", " : "") + (shim.exportsFn ? shim.exportsFn() : "function(){}") + ");\n") : singleContents += "\n" + namespaceWithDot + 'define("' + moduleName + '", function(){});\n'), fileContents = appendToFileContents(fileContents, singleContents += "\n", path, config, module, sourceMapGenerator)
                            })
                        }
                    })).then(function () {
                        onLayerEnds.length && onLayerEnds.forEach(function (builder, index) {
                            var path;
                            "string" == typeof module.out ? path = module.out : "string" == typeof module._buildPath && (path = module._buildPath), builder.onLayerEnd(function (input) {
                            ...
                            }, {name: module.name, path: path})
                        }), module.create && (fileContents = appendToFileContents(fileContents, "\n" + namespaceWithDot + 'define("' + module.name + '", function(){});\n', "module-create.js", config, module, sourceMapGenerator)), module.insertRequire && (fileContents = appendToFileContents(fileContents, "\n" + namespaceWithDot + 'require(["' + module.insertRequire.join('", "') + '"]);\n', "module-insertRequire.js", config, module, sourceMapGenerator))
                    })
                }).then(function () {
                    return config.wrap && config.wrap.__endMap && config.wrap.__endMap.forEach(function (wrapFunction) {
                        fileContents = wrapFunction(fileContents, config, sourceMapGenerator)
                    }), {
                        text: fileContents,
                        buildText: buildFileContents,
                        sourceMap: sourceMapGenerator ? JSON.stringify(sourceMapGenerator.toJSON(), null, "  ") : void 0
                    }
                })
                ...
                return baseUrl && (path = path.replace(baseUrl, "")), transform.toTransport(namespace, moduleName, path, contents, function (info) {
                    layer && (info.needsId || info.foundId === moduleName) && (layer.modulesWithNames[moduleName] = !0)
                }, options)
            }, build
        })
    }
    ...
    function setBaseUrl(fileName) {
        -1 !== (dir = fileName.replace(/\\/g, "/")).indexOf("/") && ((dir = dir.split("/")).pop(), dir = dir.join("/"), exec("require({baseUrl: '" + dir.replace(/[\\"']/g, "\\$&") + "'});"))
    }

    function createRjsApi() {
        requirejs.optimize = function (config, callback, errback) {
            loadedOptimizedLib || (loadLib(), loadedOptimizedLib = !0);
            requirejs({context: "build"}, ["build", "logger", "env!env/quit"], function (build, logger, quit) {
                function done(result) {
                    if (requirejs._buildReset && (requirejs._buildReset(), requirejs._cacheReset()), result instanceof Error) throw result;
                    return result
                }

                config.logLevel = config.hasOwnProperty("logLevel") ? config.logLevel : logger.SILENT, requirejs._buildReset && (requirejs._buildReset(), requirejs._cacheReset()), errback = errback || function (err) {
                    console.log(err), quit(1)
                }, build(config).then(done, done).then(callback, errback)
            })
        }, requirejs.tools = {
            useLib: function (contextName, callback) {
                callback || (callback = contextName, contextName = "uselib"), useLibLoaded[contextName] || (loadLib(), useLibLoaded[contextName] = !0);
    ...
    "undefined" != typeof process && process.versions && process.versions.node ? (env = "node", fs = require("fs"), vm = require("vm"), path = require("path"), existsForNode = fs.existsSync || path.existsSync, nodeRequire = require, nodeDefine = define, reqMain = require.main, require = void 0, define = void 0, readFile = function (path) {
    ...
    }, fileName = process.argv[2], fileName && 0 === fileName.indexOf("-") && (commandOption = fileName.substring(1), fileName = process.argv[3])) : "undefined" != typeof Packages ? (env = "rhino", fileName = args[0], fileName && 0 === fileName.indexOf("-") && (commandOption = fileName.substring(1), fileName = args[1]), "undefined" != typeof importPackage ? (rhinoContext = Packages.org.mozilla.javascript.ContextFactory.getGlobal().enterContext(), exec = function (string, name) {
        return rhinoContext.evaluateString(this, string, name, 0, null)
    }) : (exec = function (string, name) {
        load({script: string, name: name})
    }, readFile = readFully), exists = function (fileName) {
        return new java.io.File(fileName).exists()
    }, void 0 === console && (console = {
        log: function () {
            print.apply(void 0, arguments)
        }
    })) : "undefined" != typeof navigator && "undefined" != typeof document || "undefined" != typeof importScripts && "undefined" != typeof self ? (env = "browser", readFile = function (path) {
        return fs.readFileSync(path, "utf8")
    }, exec = function (string) {
        return eval(string)
    }, exists = function () {
        return console.log("x.js exists not applicable in browser env"), !1
    }) : "undefined" != typeof Components && Components.classes && Components.interfaces && (env = "xpconnect", Components.utils.import("resource://gre/modules/FileUtils.jsm"), Cc = Components.classes, Ci = Components.interfaces, fileName = args[0], fileName && 0 === fileName.indexOf("-") && (commandOption = fileName.substring(1), fileName = args[1]), xpcUtil = {
        isWindows: "@mozilla.org/windows-registry-key;1" in Cc, cwd: function () {
            ...
            for ("/" !== firstChar && "\\" !== firstChar && -1 === path.indexOf(":") && (path = xpcUtil.cwd() + "/" + path), ary = path.replace(/\\/g, "/").split("/"), i = 0; i < ary.length; i += 1) "." === (part = ary[i]) ? (ary.splice(i, 1), --i) : ".." === part && (ary.splice(i - 1, 2), i -= 2);
            return ary.join("/")
        }, xpfile: function (path) {
            var fullPath;
            try {
                return fullPath = xpcUtil.normalize(path), xpcUtil.isWindows && (fullPath = fullPath.replace(/\//g, "\\")), new FileUtils.File(fullPath)
            } catch (e) {
                throw new Error((fullPath || path) + " failed: " + e)
            }
        }, readFile: function (path, encoding) {
            encoding = encoding || "utf-8";
            var inStream, convertStream, readData = {}, fileObj = xpcUtil.xpfile(path);
            try {
                return (inStream = Cc["@mozilla.org/network/file-input-stream;1"].createInstance(Ci.nsIFileInputStream)).init(fileObj, 1, 0, !1), (convertStream = Cc["@mozilla.org/intl/converter-input-stream;1"].createInstance(Ci.nsIConverterInputStream)).init(inStream, encoding, inStream.available(), Ci.nsIConverterInputStream.DEFAULT_REPLACEMENT_CHARACTER), convertStream.readString(inStream.available(), readData), readData.value
            } catch (e) {
                throw new Error((fileObj && fileObj.path || "") + ": " + e)
            } finally {
                convertStream && convertStream.close(), inStream && inStream.close()
            }
        }
    }, readFile = xpcUtil.readFile, exec = function (string) {
        return eval(string)
    }, exists = function (fileName) {
        ...
        function each(ary, func) {
            var i;
            if (ary) for (i = 0; i < ary.length && (!ary[i] || !func(ary[i], i, ary)); i += 1) ;
        }

        function eachReverse(ary, func) {
            var i;
            if (ary) for (i = ary.length - 1; -1 < i && (!ary[i] || !func(ary[i], i, ary)); --i) ;
        }
        ...
        if (void 0 === define) {
            ...
            void 0 === require || isFunction(require) || (cfg = require, require = void 0), req = requirejs = function (deps, callback, errback, optional) {
                ...
                if (isBrowser) return (node = req.createNode(config, moduleName, url)).setAttribute("data-requirecontext", context.contextName), node.setAttribute("data-requiremodule", moduleName), !node.attachEvent || node.attachEvent.toString && node.attachEvent.toString().indexOf("[native code") < 0 || isOpera ? (node.addEventListener("load", context.onScriptLoad, !1), node.addEventListener("error", context.onScriptError, !1)) : (useInteractive = !0, node.attachEvent("onreadystatechange", context.onScriptLoad)), node.src = url, config.onNodeCreated && config.onNodeCreated(node, config, moduleName, url), currentlyAddingScript = node, baseElement ? head.insertBefore(node, baseElement) : head.appendChild(node), currentlyAddingScript = null, node;
                if (isWebWorker) try {
                    setTimeout(function () {
                    }, 0), importScripts(url), context.completeLoad(moduleName)
                } catch (e) {
                    context.onError(makeError("importscripts", "importScripts failed for " + moduleName + " at " + url, e, [moduleName]))
                }
            }, isBrowser && !cfg.skipDataMain && eachReverse(scripts(), function (script) {
                if (head = head || script.parentNode, dataMain = script.getAttribute("data-main")) return mainScript = dataMain, cfg.baseUrl || -1 !== mainScript.indexOf("!") || (mainScript = (src = mainScript.split("/")).pop(), subPath = src.length ? src.join("/") + "/" : "./", cfg.baseUrl = subPath), mainScript = mainScript.replace(jsSuffixRegExp, ""), req.jsExtRegExp.test(mainScript) && (mainScript = dataMain), cfg.deps = cfg.deps ? cfg.deps.concat(mainScript) : [mainScript], !0
        ...
        function newContext(contextName) {
            ...
            function normalize(name, baseName, applyMap) {
                var mapValue, nameParts, i, j, nameSegment, lastIndex, foundMap, foundI, foundStarMap, starI,
                    baseParts = baseName && baseName.split("/"), map = config.map, starMap = map && map["*"];
                if (name && (lastIndex = (name = name.split("/")).length - 1, config.nodeIdCompat && jsSuffixRegExp.test(name[lastIndex]) && (name[lastIndex] = name[lastIndex].replace(jsSuffixRegExp, "")), "." === name[0].charAt(0) && baseParts && (name = baseParts.slice(0, baseParts.length - 1).concat(name)), function (ary) {
                    var i, part;
                    for (i = 0; i < ary.length; i++) if ("." === (part = ary[i])) ary.splice(i, 1), --i; else if (".." === part) {
                        if (0 === i || 1 === i && ".." === ary[2] || ".." === ary[i - 1]) continue;
                        0 < i && (ary.splice(i - 1, 2), i -= 2)
                    }
                }(name), name = name.join("/")), applyMap && map && (baseParts || starMap)) {
                    outerLoop:for (i = (nameParts = name.split("/")).length; 0 < i; --i) {
                        if (nameSegment = nameParts.slice(0, i).join("/"), baseParts) for (j = baseParts.length; 0 < j; --j) if (mapValue = (mapValue = getOwn(map, baseParts.slice(0, j).join("/"))) && getOwn(mapValue, nameSegment)) {
            ...
            function hasPathFallback(id) {
                ...
                return pathConfig && isArray(pathConfig) && 1 < pathConfig.length && (pathConfig.shift(), context.require.undef(id), context.makeRequire(null, {skipMap: !0})([id]), 1)
            }
            ...
            function makeModuleMap(name, parentModuleMap, isNormalized, applyMap) {
                var url, pluginModule, suffix, nameParts, prefix = null,
                    parentName = parentModuleMap ? parentModuleMap.name : null, originalName = name, isDefine = !0,
                    normalizedName = "";
            ...
            function getModule(depMap) {
                ...
                return mod = mod || (registry[id] = new context.Module(depMap))
            }
            ...
            function checkLoaded() {
                var err, usingPathFallback, waitInterval = 1e3 * config.waitSeconds,
                    ...
                    reqCalls = [], stillLoading = !1, needCycleCheck = !0;
                if (!inCheckLoaded) {
                    if (inCheckLoaded = !0, eachProp(enabledRegistry, function (mod) {
                        var map = mod.map, modId = map.id;
                        if (mod.enabled && (map.isDefine || reqCalls.push(mod), !mod.error)) if (!mod.inited && expired) hasPathFallback(modId) ? stillLoading = usingPathFallback = !0 : (noLoads.push(modId), removeScript(modId)); else if (!mod.inited && mod.fetched && map.isDefine && (stillLoading = !0, !map.prefix)) return needCycleCheck = !1
                    }), expired && noLoads.length) return (err = makeError("timeout", "Load timeout for modules: " + noLoads, null, noLoads)).contextName = context.contextName, onError(err);
            ...
            function intakeDefines() {
                ...
                for (takeGlobalQueue(); defQueue.length;) {
                    if (null === (args = defQueue.shift())[0]) return onError(makeError("mismatch", "Mismatched anonymous define() module: " + args[args.length - 1]));
                    callGetModule(args)
                }
                context.defQueueMap = {}
            }
            ...
            return handlers = {
                ...
                init: function (depMaps, factory, errback, options) {
                    ...
                    this.depMatched[i] || (this.depMatched[i] = !0, --this.depCount, this.depExports[i] = depExports)
                }, fetch: function () {
                    if (!this.fetched) {
                        this.fetched = !0, context.startTime = (new Date).getTime();
                        var map = this.map;
                        if (!this.shim) return map.prefix ? this.callPlugin() : this.load();
                        context.makeRequire(this.map, {enableBuildCallback: !0})(this.shim.deps || [], bind(this, function () {
                    ...
                    this.depMaps.push(pluginMap), on(pluginMap, "defined", bind(this, function (plugin) {
                        var load, normalizedMap, normalizedMod, bundleId = getOwn(bundlesMap, this.map.id),
                            ...
                            localRequire = context.makeRequire(map.parentMap, {enableBuildCallback: !0});
                        return this.map.unnormalized ? (plugin.normalize && (name = plugin.normalize(name, function (name) {
                            return normalize(name, parentName, !0)
                        }) || ""), on(normalizedMap = makeModuleMap(map.prefix + "!" + name, this.map.parentMap, !0), "defined", bind(this, function (value) {
                    ...
                    (enabledRegistry[this.map.id] = this).enabled = !0, this.enabling = !0, each(this.depMaps, bind(this, function (depMap, i) {
                        var id, mod, handler;
                    ...
                    (cbs = cbs || (this.events[name] = [])).push(cb)
                ...
                configure: function (cfg) {
                    ...
                    eachProp(cfg, function (value, prop) {
                        ...
                        var name;
                        name = (pkgObj = "string" == typeof pkgObj ? {name: pkgObj} : pkgObj).name, pkgObj.location && (config.paths[name] = pkgObj.location), config.pkgs[name] = pkgObj.name + "/" + (pkgObj.main || "main").replace(currDirRegExp, "").replace(jsSuffixRegExp, "")
                ...
                makeRequire: function (relMap, options) {
                    function localRequire(deps, callback, errback) {
                        var id, requireMod;
                        return options.enableBuildCallback && callback && isFunction(callback) && (callback.__requireJsBuild = !0), "string" == typeof deps ? isFunction(callback) ? onError(makeError("requireargs", "Invalid require call"), errback) : relMap && hasProp(handlers, deps) ? handlers[deps](registry[relMap.id]) : req.get ? req.get(context, deps, relMap, localRequire) : (id = makeModuleMap(deps, relMap, !1, !0).id, hasProp(defined, id) ? defined[id] : onError(makeError("notloaded", 'Module name "' + id + '" has not been loaded yet for context: ' + contextName + (relMap ? "" : ". Use require([])")))) : (intakeDefines(), context.nextTick(function () {
                            intakeDefines(), (requireMod = getModule(makeModuleMap(null, relMap))).skipMap = options.skipMap, requireMod.init(deps, callback, errback, {enabled: !0}), checkLoaded()
                        }), localRequire)
                    }
                ...
                nameToUrl: function (moduleName, ext, skipExt) {
                    var paths, syms, i, url, parentPath, bundleId, pkgMain = getOwn(config.pkgs, moduleName);
                    ...
                    if (req.jsExtRegExp.test(moduleName)) url = moduleName + (ext || ""); else {
                        for (paths = config.paths, i = (syms = moduleName.split("/")).length; 0 < i; --i) if (parentPath = getOwn(paths, syms.slice(0, i).join("/"))) {
                            isArray(parentPath) && (parentPath = parentPath[0]), syms.splice(0, i, parentPath);
                            break
                        }
                        url = syms.join("/"), url = ("/" === (url += ext || (/^data\:|^blob\:|\?/.test(url) || skipExt ? "" : ".js")).charAt(0) || url.match(/^[\w\+\.\-]+:/) ? "" : config.baseUrl) + url
                    }
        ...
        function getInteractiveScript() {
            return interactiveScript && "interactive" === interactiveScript.readyState || eachReverse(scripts(), function (script) {
                if ("interactive" === script.readyState) return interactiveScript = script
            }), interactiveScript
        }
    }(this, "undefined" == typeof setTimeout ? void 0 : setTimeout), this.requirejsVars = {
        require: require,
        requirejs: require,
        define: define
        ...
        require.load = function (context, moduleName, url) {
            var xhr = new XMLHttpRequest;
            xhr.open("GET", url, !0), xhr.send(), xhr.onreadystatechange = function () {
                4 === xhr.readyState && (exec(xhr.responseText), context.completeLoad(moduleName))
            }
        }
    }() : "rhino" === env ? function () {
        "use strict";
        require.load = function (context, moduleName, url) {
            load(url), context.completeLoad(moduleName)
        }
    }() : "node" === env ? (this.requirejsVars.nodeRequire = nodeRequire, require.nodeRequire = nodeRequire, function () {
        var nodeReq = requirejsVars.nodeRequire, req = requirejsVars.require, def = requirejsVars.define,
            fs = nodeReq("fs"), path = nodeReq("path"), vm = nodeReq("vm"), exists = fs.existsSync || path.existsSync,
        ...
        function makeError(message, moduleName) {
            ...
            return err.requireModules = [moduleName], err
        }

        req.get = function (context, moduleName, relModuleMap, localRequire) {
            "require" !== moduleName && "exports" !== moduleName && "module" !== moduleName || context.onError(makeError("Explicit require of " + moduleName + " is not allowed.", moduleName));
            var ret, oldTick, moduleMap = context.makeModuleMap(moduleName, relModuleMap, !1, !0);
            if (moduleName = moduleMap.id, hasProp(context.defined, moduleName)) ret = context.defined[moduleName]; else if (void 0 === ret) {
                oldTick = context.nextTick, context.nextTick = syncTick;
                try {
                    moduleMap.prefix ? (localRequire([moduleMap.originalName]), moduleName = (moduleMap = context.makeModuleMap(moduleMap.originalName, relModuleMap, !1, !0)).id) : (req.load(context, moduleName, moduleMap.url), context.enable(moduleMap, relModuleMap)), context.require([moduleName]), ret = context.defined[moduleName]
                } finally {
            ...
            if (!config.shim[moduleName] || config.suppress && config.suppress.nodeShim || console.warn("Shim config not supported in Node, may or may not work. Detected for module: " + moduleName), exists(url)) {
                contents = fs.readFileSync(url, "utf8"), contents = req.makeNodeWrapper(contents);
                try {
                    vm.runInThisContext(contents, fs.realpathSync(url))
                } catch (e) {
                    return (err = new Error("Evaluating " + url + ' as module "' + moduleName + '" failed with error: ' + e)).originalError = e, err.moduleName = moduleName, err.requireModules = [moduleName], err.fileName = url, context.onError(err)
                }
            } else def(moduleName, function () {
                var dirName, map = hasProp(context.registry, moduleName) && context.registry[moduleName].map,
                    parentMap = map && map.parentMap, originalName = map && map.originalName;
                "." === originalName.charAt(0) && parentMap && ((dirName = parentMap.url.split("/")).pop(), originalName = dirName.join("/") + "/" + originalName);
                try {
                    return (context.config.nodeRequire || req.nodeRequire)(originalName)
                } catch (e) {
                    throw(err = new Error('Tried loading "' + moduleName + '" at ' + url + " then tried node's require(\"" + originalName + '") and it failed with error: ' + e)).originalError = e, err.moduleName = originalName, err.requireModules = [moduleName], err
                }
            });
            context.completeLoad(moduleName)
        }, req.exec = function (text) {
            return text = req.makeNodeWrapper(text), eval(text)
        }
    }()) : "xpconnect" === env && function () {
        "use strict";
        require.load = function (context, moduleName, url) {
            load(url), context.completeLoad(moduleName)
        }
    }(), "o" === commandOption || fileName && jsSuffixRegExp.test(fileName) || (fileName = "main.js"), "node" === env && reqMain !== module ? (setBaseUrl(path.resolve(reqMain ? reqMain.filename : ".")), createRjsApi(), module.exports = requirejs) : "browser" === env ? (setBaseUrl(location.href), createRjsApi()) : "rhino" !== env && "xpconnect" !== env || "undefined" == typeof requirejsAsLib || !requirejsAsLib ? "o" === commandOption ? (loadLib(), require({
        ...
        build(args).then(function () {
            ...
            logger.error(err), quit(1)
        })
    })) : "v" === commandOption ? console.log("r.js: " + version + ", RequireJS: " + this.requirejsVars.require.version + ", UglifyJS: 2.8.29") : "convert" === commandOption ? (loadLib(), this.requirejsVars.require(["env!env/args", "commonJs", "env!env/print"], function (args, commonJs, print) {
        var srcDir, outDir;
        srcDir = args[0], outDir = args[1], srcDir && outDir ? commonJs.convertDir(args[0], args[1]) : print("Usage: path/to/commonjs/modules output/dir")
    })) : ("lib" === commandOption && loadLib(), setBaseUrl(fileName), exists(fileName) ? exec(readFile(fileName), fileName) : showHelp()) : (setBaseUrl(fileName), createRjsApi())
}("undefined" != typeof console ? console : void 0, "undefined" != typeof Packages || "undefined" == typeof window && "undefined" != typeof Components && Components.interfaces ? Array.prototype.slice.call(arguments, 0) : [], "undefined" != typeof readFile ? readFile : void 0);
